...
**318 rows move. 1624 declared edit lines**: `import rewrites` 708, `path
constants` 242, `adapter calls` 674. **159 rows carry `edits:`** — the 158
...
carrier count stand where slice S5 left them.
...
when this paragraph was written, the 1624th now): a replica row names no
...
from the start — 794 of them at this ruling's own landing, 1624 now.
