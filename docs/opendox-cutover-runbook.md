...
**318 rows move. 2462 declared edit lines**: `import rewrites` 726, `path
constants` 246, `adapter calls` 1490. **176 rows carry `edits:`** — the 175
...
sum to 2460 rather than 2462. **The two are owed differently, and a replica row
...
| --- | ---: | ---: | ---: | --- |
| `opendox_code` | 123 | 35 / 88 | 1675 | `src/opendox`, `tests` |
...
declared-line cell alone once more, to `1675`: its eight `adapter calls` lines
fall on five rows that leg already edits, so no `verbatim / edited` cell and no
row count moves with them.
The four destination figures now sum to 2460, and the TWO lines the replica row
...
from the start — 794 of them at this ruling's own landing, 2462 now.
