...
| 11 | Contract-copy drift | Each pinned consumer's `stack.yaml` `contract_ref:` against openxFactory's own HEAD — `warning` where the declared pin differs at all, a plain inequality that a lag, a lead, and an unrelated commit trip alike. What it compares is the DECLARED PIN; the token-set comparison of the copies themselves is this family generalized, and it belongs to the neutrality-drift lane below |
