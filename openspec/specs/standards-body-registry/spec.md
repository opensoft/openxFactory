...
Govern `contracts/policies/standards-bodies.yaml` as the neutral register of
external standards bodies that domain terminology crosswalks resolve against:
what a body must say when it CLAIMS primary-source verification (id, steward,
term kind, current version, source URL, verification date, confidence), how a
historical id stays resolvable rather than being silently renamed, how an
operator override that disagrees with a steward's licensing evidence is recorded
as an explicit unverified exception rather than laundered into permission, and
how a conditional reuse grant carries its attribution. The obligation is keyed
to the verification CLAIM and not to `status: current`, so the register is
enforceable on every record that asserts currency and silent about the records
its own header discloses as unverified. Crosswalks reading this register stay
DESCRIPTIVE: resolving a body id exposes its term kind and licensing posture and
confers no archetype, permission, credential tier or terminal-action authority
on a worker.
## Requirements
