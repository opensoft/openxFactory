# packet-citation-report Specification

## Purpose

Make the remainder of a corpus's own packet citations READABLE. A repository
whose `openspec/changes/…` citations have a resolution rule still carries the
citations that rule cannot resolve, and a remainder nobody reads is a rule
whose coverage nobody knows. This capability puts a corpus-wide REPORT over
that remainder. The report derives its file population and its token grammar
from a recipe STATED IN THE REQUIREMENT rather than left to an implementation,
so a figure it prints can be reproduced by hand. It counts the remainder in
TOKENS and in IDENTITIES both, because a reader who repairs one identity may
clear several lines. It FLAGS a suspected cross-repository citation and never
drops one, this tree holding no repository vocabulary with which to answer for
another's corpus. It classifies only what it can decide mechanically from a
path or from its own normalization, and says `unclassified` honestly for what
needs a reading of intent. And it REPAIRS NOTHING AND GATES NOTHING: it exits
successfully whatever it finds, a non-zero exit meaning the report COULD NOT
RUN rather than that it found something. Promoting such a report to a gate is
a separate act on a separate word, and a severity decision that FOLLOWS a
measurement of the population it would red rather than preceding it.
