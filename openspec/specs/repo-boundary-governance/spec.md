...
`openxFactory` SHALL be the canonical repository for factory-level workflow
...
- **THEN** the install repo MUST link to the canonical `openxFactory` policy rather than become the policy source of truth
