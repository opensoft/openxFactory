...
- [ ] 1.1 Write `docs/doc-health.md` (`Status: ratified`, `Ratified by:`
...
- [ ] 1.2 Link `docs/doc-health.md` from the README documentation index.

## 2. Ideation Pointers (staged -> proposed gate)

- [ ] 2.1 Update `ideation/staging/doc-health-checks/nightly-run-shape.md`:
...
- [ ] 2.2 Update `ideation/staging/doc-health-checks/status-check-rules.md`:
      reference this change as the contract now carrying its checks.
- [ ] 2.3 Update `ideation/staging/doc-health-checks/tag-hygiene-rules.md`:
      reference this change as the contract now carrying its checks (the
      grammar itself stays owned by `document-lifecycle`).

## 3. Implementation Handoff

- [ ] 3.1 Write `ideation/staging/doc-health-checks/implementation-handoff.md`
...
- [ ] 4.1 `OPENSPEC_TELEMETRY=0 openspec validate add-doc-health-contract
      --strict` and `openspec validate --all --strict` pass from the
      openxFactory root.
- [ ] 4.2 Grep-verify the `xspec:` grammar is stated only by
