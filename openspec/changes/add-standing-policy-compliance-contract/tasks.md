...
- [ ] 2.2 Encode closed decision outcomes, stable allowance IDs, immutable
      approval facts, additive revocation facts and unique registry lookup.
- [ ] 2.3 Require bindings/decisions to carry allowance IDs only and reject
      embedded allowance payloads as authority evidence.
- [ ] 2.4 Add `.template.yaml` files for all four record kinds and ensure every
      YAML template carries `schema_version` and `kind`.
- [ ] 2.5 Add a codexFactory-shaped first-conformer vocabulary example carrying
      the five ratified standing-policy classes without making those five the
      neutral closed set.
...
- [ ] 3.2 Validate source-reference/digest presence, class/issuer closure,
      registry-ID uniqueness, allowance scope/validity, immutable approval
      facts and additive revocation fields.
- [ ] 3.3 Validate decision/content digest binding, closed outcomes,
      correlation facts and allowance-resolution evidence.
- [ ] 3.4 Add packaged positive examples for no-class allow, current-allowance
      allow, ambiguous-reference review and revoked-allowance block.
- [ ] 3.5 Add negative examples for duplicate IDs, embedded allowance payloads,
      unauthorized issuers, stale policy digests, missing evidence fields and
      classifier override of a deterministic finding.
- [ ] 3.6 Prove every negative fixture fails for its intended reason and every
...
- [ ] 5.1 Raise codexFactory `add-intent-compliance-gate` against the released
      ...
      to the approved-intent binding.
