...
- [x] 2.3 Implement strict openxFactory validators for catalog schema, complete coverage, unique identity, source freshness, taxonomy resolution, override standing, immutable paths, and baseline-mode exceptions. (Added 2026-07-13 as `scripts/validate-document-catalog.py`: schema conformance over all 23 packaged examples (12 valid confirmed valid, 11 negative confirmed invalid — `python3 scripts/validate-document-catalog.py` exits 0) plus seven cross-cutting checks — `check_complete_coverage`/`check_unique_identity`/`check_source_freshness`/`check_taxonomy_registries`+`check_topic_tag_resolution`+`check_capability_resolution`+`check_taxonomy_digest_consistency`/`check_override_standing`/`check_immutable_path_layout` — each proven to accept the valid shape and reject its own violation by `tests/document_catalog/test_validate_document_catalog.py` (33 passed). `--baseline` downgrades missing-coverage findings to warnings per the disclosed-baseline scenario; `--inventory` supplies the external shared-inventory input this validator does not itself own. Override-standing authority-class matching and the taxonomy-digest formula were cross-checked, read-only, against the in-flight codexFactory realization (`001-document-catalog` worktree, `scripts/doc_health/catalog.py`/`document_catalog.py`) without copying its implementation; digest bit-for-bit recomputation is deliberately left to that codexFactory ownership (task 3.3) — see this task's completion note in the change's final report.)
