...
The system SHALL migrate canonical factory policy into `openWorkflow` using
copy-first feature slices.

#### Scenario: Source policy is migrated
- **WHEN** policy currently lives in an install or proof repo
- **THEN** the canonical policy MUST be copied or summarized into `openWorkflow` before the source copy is deleted or marked legacy

#### Scenario: Source repo keeps implementation context
- **WHEN** a policy source also contains install-specific context
- **THEN** `openWorkflow` MUST keep canonical policy while the install repo MAY retain implementation notes that link back to `openWorkflow`
...
`openWorkflow` SHALL own the canonical cross-factory role and authority model.
...
- **THEN** their canonical responsibility, authority, and escalation boundaries MUST live in `openWorkflow`

#### Scenario: Install repo uses role policy
- **WHEN** an install repo needs role-specific implementation behavior
- **THEN** it MUST link to the canonical `openWorkflow` role policy and document only subsystem-specific implementation details

### Requirement: Canonical stage and merge policy
`openWorkflow` SHALL own canonical Spec Kit stage ownership, clarification
...
- **THEN** the canonical ownership and consultation policy MUST live in `openWorkflow`

#### Scenario: Merge authority is migrated
- **WHEN** PR admission, merge readiness, merge risk, or human escalation rules are defined
- **THEN** the canonical policy MUST live in `openWorkflow`
...
- **WHEN** a canonical policy document is created in `openWorkflow`
