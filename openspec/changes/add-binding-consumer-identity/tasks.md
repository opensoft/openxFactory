...
- [ ] 1.1 Add `consumer:` to each entry of `credential_bindings` in
  ...
  **DESCRIBED, UNCONSTRAINED property**: no `type`, no `required:`, no
  ...
  `instantiation_stub`.
- [ ] 1.1a **DO NOT CONSTRAIN THE BLOCK AT THIS CUT — NOT ONE OF THE THREE
...
- [ ] 1.1b **THE BUILT-AND-DRIVEN FIXTURE FOR THIS TASK, written WITH the task
...
- [ ] 1.2 The block stays OPTIONAL on the binding and the binding object stays
  UNCLOSED. Closing the binding object is a further breaking act and is NOT this
  change.
- [ ] 1.3 **Constrain the `credential_bindings` MAP KEY** to the identity-brokering
...
- [ ] 1.5 **Give `requirements_document_ref` a grammar**: repository-relative,
...
- [ ] 1.6 **The stub exemption is the declared const-true `instantiation_stub`
...
- [ ] 1.7 Write the block's `description` to carry its own reasoning, on this
...
- [ ] 2.3a **The arity is EVERY PAIR, and the inherited shape is REPLACED not
...
- [ ] 2.3b **An unreadable access mode makes the lift UNAVAILABLE**, never "equal
...
- [ ] 2.4 `shared-authority-identity` — NEW error: two bindings declaring
...
- [ ] 2.5 **Resolution, constrained.** The QUALIFIED `requirement_ref` resolves
...
- [ ] 2.6 **Screen the two new sinks.** `_looks_like_raw_secret` reads
...
- [ ] 2.7 A mutation round over §2.3–§2.5, one mutant per condition, each
...
- [ ] 3.2 NEGATIVES, one per named refusal: same fetch identity with different
  ...
  expectation.
- [ ] 3.2a **A RECORD WEARING A STUB NAME IS A NEGATIVE.** A binding carrying
...
- [ ] 3.3 **WARNING FIXTURES NEED A HOME THAT DOES NOT EXIST YET, and building it
...
- [ ] 4.3 `docs/credential-access-model.md:219-225` — the worked binding example,
  ...
  standing.
- [ ] 4.4 `contracts/avatar-client/broker-server-key-binding.template.yaml` —
...
- [ ] 4.5 IN/OUT for the remaining nine paths the command returns:
...
- [ ] 4.6 RE-RUN THE SWEEP AT REALIZATION rather than trusting this list. The
...
- [ ] 5.1a **THE ENTRY GATES THE MAJOR ON THE DEGRADED MODE.** It states that
  ...
  second branch, taken.
- [ ] 5.1b **FIRST CHECK WHETHER THE MECHANISM IS TRUSTWORTHY.** Three entries in
...
- [ ] 5.4 **COORDINATE WITH THE OTHER WRITERS — MACHINE-CHECKED, BECAUSE A
