...
- [ ] 1.1 `contracts/schemas/gate-action-record.schema.yaml`: the `action`
...
- [ ] 1.2 Same schema: broaden the EXISTING optional `target.document`
...
- [ ] 1.3 Same schema: one `allOf` conditional requiring
...
- [ ] 1.4 Confirm the artifact `kind` enum is NOT grown in v1 (design D12,
...
- [ ] 1.5 Validate: `OPENSPEC_TELEMETRY=0 openspec validate --all --strict`
...
- [ ] 2.1 Lift the SVG bullseye renderer (`bullseye(model)` plus its `svg()`
      ...
      geometry untouched.
- [ ] 2.2 The widget takes an optional `onActivate` for the CENTRE-ring hit
...
- [ ] 2.3 `views/lens.js` consumes the widget with NO signature change to
      its own callers; the existing lens tests stay green unmodified.

## 3. Workbench lens panel (codexFactory)

- [ ] 3.1 `views/staging-workbench.js` lens panel mounts the shared widget
      ...
      counts.
- [ ] 3.2 Move the checked-keyword set out of the panel's `draw()` closure
...
- [ ] 3.3 Node-harness model tests for the seeding/reset rule; renderer
...
- [ ] 4.1 `gate_routes.py`: `create-document` beside propose and the two
...
- [ ] 4.2 Payload validation/normalization at the route:
...
- [ ] 4.3 Gate-action record: action `create-document`, `target.document` =
...
- [ ] 4.4 Refusal surface: an EXISTING target refuses as `SOURCE_EDIT`
...
- [ ] 4.5 `cli.py gate create-document` parity subcommand beside the
...
- [ ] 5.1 Thread `caps` and a `fetcher` into `mountStagingWorkbench` from
      `app.js` — it receives NEITHER today (design D5 consequence);
      `app.js` already holds `caps` from the one capability probe.
- [ ] 5.2 NEW sibling transport module holding the create POST, using the
...
- [ ] 5.3 Create-payload seeding in the PURE model module (design D7): the
...
- [ ] 5.4 The `docs` tab affordance: a pane actions-row button opening the
      dialog with every seeded value editable.
- [ ] 5.5 The `lens` tab affordances: a forming-set pane button AND the
...
- [ ] 6.1 Route + CLI tests: a create lands the document and its record;
      ...
      grown schema.
- [ ] 6.2 Full dashboard suite green, INCLUDING the two pinning tests
...
- [ ] 6.3 Playwright smoke: open the workbench on a cluster, a possible,
...
- [ ] 6.4 Degraded-posture check on the served static image: the pill reads
      ...
      snapshot alone.
- [ ] 6.5 Boundary check: the boundary validator still reports no
