...
- [ ] 6.4 Notify/unblock `implement-avatar-client-lab` §9 (its 9.3 pin-resync is satisfied by 6.1; its 4.4 manifest registration satisfied by 5.1); archive THIS change per release-realization once 6.2's green run is recorded. — PARKED on codexFactory PR #17 merging (Tier-1 approval: Brett); on merge, realization is complete → archive this change and sweep `implement-avatar-client-lab` §9.
