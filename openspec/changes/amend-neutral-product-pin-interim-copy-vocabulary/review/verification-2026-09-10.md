# Verification record: amend-neutral-product-pin-interim-copy-vocabulary, RE-RUN 2026-09-10
...
FORWARD** from the 2026-09-09 capture; where a figure is unchanged from it, it is
unchanged because it was measured again and came out the same.
...
`Decision date:`, `approved_on` and the ratification record's file name stay
2026-09-09. The two CORRECTIVE rulings — *"Keep the clause, fix the accounting"*
and *"Follow canon: Status: ratified"* — were given at 2026-09-10T00:20:16Z, and
this re-run is their evidence. The UTC day boundary is crossed by the
CORRECTION, never by the ratification.
...
| `review/verification-2026-09-09.md` (the 09-09 capture, preserved) | `Status: record` | `Ratified by:` |
| `review/verification-2026-09-10.md` (this file) | `Status: record` | `Ratified by:` |
...
**THE FAMILY IS SILENT ABOUT `review/` RECORDS, AND THAT SILENCE IS A COVERAGE
GAP RATHER THAN A LICENCE — WHICH IS WHY THIS FILE SITS AT A NEW PATH.**
`govern-openspec-corpus-membership` declares two document sets.
`fam_record_immutability` iterates `ctx.docs`, the GOVERNED CORPUS —
`contracts/`, `docs/`, `examples/`, `ideation/`, `templates/` — while a `review/`
record under an OpenSpec change packet lives in the LIFECYCLE SCAN SET
...
integrity. Record immutability is not one of them, and all four criticals above
are `docs/` documents. **THE CONTRACT BINDS ANYWAY.** `document-lifecycle`'s
`record` rule is written over the DOCUMENT and not over the checker: *"a second
run of such a generator writes a different path rather than rewriting the same
one"*. An earlier draft of this re-run rewrote `verification-2026-09-09.md` in
place and the family said nothing — Codex's P1 (**T7**) named exactly that
inference as unsound, and it is right. The 09-09 capture is restored to its
`d0ddb126` bytes (`git diff d0ddb126 -- …/review/verification-2026-09-09.md` is
**EMPTY**) and this second run is a second path. The `record-immutability` family
gap is filed rather than relied on.
...
The re-derive commit `27b31718` touches three:
...
The T7/T8 disposition commit carrying THIS file **restores** the third of those
to its `d0ddb126` bytes and adds this record beside it:

```
README.md
openspec/changes/amend-neutral-product-pin-interim-copy-vocabulary/tasks.md
openspec/changes/amend-neutral-product-pin-interim-copy-vocabulary/review/ratification-2026-09-09.md
openspec/changes/amend-neutral-product-pin-interim-copy-vocabulary/review/verification-2026-09-09.md   (RESTORED)
openspec/changes/amend-neutral-product-pin-interim-copy-vocabulary/review/verification-2026-09-10.md   (NEW)
```
...
**BOTH REFUSALS ARE ARGUED ON THE MERITS AND BOTH ALSO SIT BEHIND WORKING RULE
3**: the delta's normative units are RATIFIED text, and a ratified requirement
changes only by a ratified change — which is the same limit the refusal on #780
respected and the reason this packet exists at all. **This lane encodes,
corrects and freezes; it does not merge.**
