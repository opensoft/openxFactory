...
block raises `co_modified` 104 → 105 and `active_co_modified` 18 → 19, and
`change_ids` 154 → 155 — its `- 1` reading, which subtracts the pin's own owning change, moving 153 → 154. (The FIRST measurement read 153 → 154
absolute, before #555's ADDED-only landing was merged into this branch; #555
moved the absolute population and the SOLE-modifier readings and left the
co-modified ones, which is the opposite half of the same rule.) That is the pin behaving exactly as designed — its
