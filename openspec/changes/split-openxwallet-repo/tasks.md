...
- [ ] 5.1 Each of the eight manifest rows at `contracts/manifest.yaml:1967-2082`
...
- [ ] 5.2 **NO `removed_at` on the row.** `:245-247` puts the removal version and
...
- [ ] 5.3 `contracts/CHANGELOG.md` gains the deprecation entry stating BOTH
...
- [ ] 5.4 The emitter is `scripts/check-openxfactory-pin.py` — the only candidate
      with a warning tier (`PASS, WARN, ERROR, SKIP` at `:35`, returned by
      `classify()` at `:43`). It warns when a domain pins a bundle carrying
      `relocating:` rows.
- [ ] 5.5 Record why `scripts/validate-domain-openxfactory-pins.py` is NOT the
      ...
      avoid.
- [ ] 5.6 **`scripts/validate-openxwallet.py` is NOT edited in openxFactory by
...
- [ ] 5.7 `contracts/releases/<the P2.5 minor>.digests.yaml` over a release
...
- [ ] 5.9 Evidence row: the eight `relocating:` rows, the CHANGELOG migration
      note, `check-openxfactory-pin.py`'s WARN output, AND the minor's own
      `contracts/releases/<tag>.digests.yaml`.
- [ ] 5.10 Rollback recorded before the step: a published bundle is not
