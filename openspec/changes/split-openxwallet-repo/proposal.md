...
| --- | --- | --- | --- | --- | --- |
| P2 | carve + scaffold | `opensoft/openXwallet` | | | first green `wallet-validation`, plus the carve-completeness check over the twelve path sets (`specs/006-openxwallet-contracts/evidence/` and `contracts/openxwallet/examples/negative/` named explicitly) |
| P2 | runnable half | `opensoft/openXwallet` | | | first green `pytest-suite`, plus the byte-identity proof of the eight rows **against the NAMED CARVE COMMIT** |
| P2 | vendored-schema verify (V8) | `opensoft/openXwallet` | | | the `wallet-validation` log line showing the vendored `contracts/schemas/hermes-job-envelope.schema.yaml` digest-verified against `contract_pin.yaml` BEFORE the validator ran, plus a red run with a mutated copy |
| P2b | `wallet-v1.1` — nested-repository prune + register-read note | `opensoft/openXwallet` | | | the one auditable additive-minor diff, green `wallet-validation` and `pytest-suite` at the tag, a run proving a YAML file inside a nested repository is no longer swept, AND a green LedgerxFactory estate run on the v1.1 reader (its `--strict` invocation must stay green, and the register-read line must be a NOTE) |
| P2.5 | deprecation minor | `opensoft/openxFactory` | | | the eight `relocating:` rows, the CHANGELOG migration note, the emitting checker's warning output, AND the minor's own `contracts/releases/<tag>.digests.yaml` |
| P3 | consume + shed (major) | `opensoft/openxFactory` | | | green `wallet-validation` (the token, from the renamed `openxwallet-consumer-gate.yml`) on P3's own head; the digests RE-VERIFIED post-rebase immediately before merge |
| P3 | the reader actually RAN (V7) | `opensoft/openxFactory` | | | the consumer gate's log carrying the `repo scan: N openxWallet artifact` note AND carrying neither `no intake register at this tree` nor any `register-*` finding code — the positive conjunction the pinned reader actually produces, since `check_register` names the register path on failure only — plus the `wallet-v1.1` register-read NOTE naming the resolved path, **plus the task-2.6 red-proof run id**. A green check that proves no register was opened is a vacuous pass |
| P3 | `pytest-suite` under nested submodules (V9) | `opensoft/openxFactory` | | | the pinned **PASS / SKIP counts**, not collection alone, so a silently skipped `tests/trust-anchor/` is distinguishable from a passing one |
| P3b | codexFactory floor widening (V6) | `opensoft/codexFactory` | | | the `openxfactory-review-authority-floor.yaml` diff adding `contracts/openxwallet-pin.yaml` and `openXwallet`, plus the merged pull request |
| P4 | aggregation | `opensoft/xFactory` | | | plus the root-gitlink-equals-nested-gitlink check (V3) |
| P4b | root-level governed-repo recognition (V10) | `opensoft/openxFactory` | | | `xf-ideation-openxwallet` exists after one `sync-notebooklm-books.py . --apply` |
| P5a.1 | forward-compatible finder (before P2) | `opensoft/LedgerxFactory` | | | green estate run resolving through the THREE-candidate finder, on a tree where only candidate three exists |
| P5a.2 | `stack.yaml` bump to the P2.5 minor (after P2.5, before P3) | `opensoft/LedgerxFactory` | | | green estate run with the `relocating:` warning OBSERVED — the pinned checker's output in the run log, not the manifest rows |
| P5b | post-move repoints + declared wallet pin | `opensoft/LedgerxFactory` | | | |
| P5b | supporting-doc repoint | `opensoft/OpsxFactory` | | | |

**Two RULESET STATES are their own evidence rows**, because a repository setting is
...
| --- | --- | --- |
| new | `opensoft/openXwallet` | `wallet-validation` — created in EVALUATE mode, promoted to ACTIVE after the bootstrap pull request's first report. Day-one ACTIVE is impossible: a check must have reported once before it is selectable, so "required from day one" is recorded here as unachievable and the bootstrap sequence replaces it |
| 21538893 (**UNCHANGED**) | `opensoft/openxFactory` | `wallet-validation` — unchanged, because the token is the job id and P3 retains it in `openxwallet-consumer-gate.yml`. Evidence is TWO artifacts: the ruleset output showing the same single token as before the wave, AND the P3 pull request's own green `wallet-validation` run, produced by the NEW workflow file |
