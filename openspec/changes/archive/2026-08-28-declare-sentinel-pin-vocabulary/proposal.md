...
target_release: implemented — realized on the openxFactory main line as pull request #457 ("Realize declare-sentinel-pin-vocabulary: a non-commit pin value is classified, not skipped"), merged 2026-08-28T04:57:04Z as merge commit `157e423dfb62b1405d87b49f9681d729526a7c42`, re-verified AT THIS ACT an ancestor of `origin/main` (`git merge-base --is-ancestor`, exit 0) and a REAL TWO-PARENT MERGE (`52999f011cc99224e564d058842171e64669d27b` + `9142b5e3085c271f15b8cb0e8221762eebb97e55`) rather than a rewrite — read out of `git cat-file -p` rather than off the pull request page. GREEN THERE, on the final head `9142b5e3085c271f15b8cb0e8221762eebb97e55`, all four checks read back from the check-runs API at this act: `pytest-suite` **success** 04:40:50Z → 04:56:06Z (15m16s), with the counts read out of the job log of run 98756219935 rather than off a summary — **selected 7524, passed 7503, skipped 21, failures 0, errors 0**, clearing the workflow's own floors by 434 selected and 433 passed; `wallet-validation` **success** 04:40:48Z → 04:41:13Z; `merge-master-approval` **success** 04:40:46Z → 04:41:04Z; `copilot-pull-request-reviewer` **success**. LOCALLY ON THE MERGED TREE at this act: `OPENSPEC_TELEMETRY=0 openspec validate --all --strict` **78 passed / 0 failed** before the act and **77 passed / 0 failed** after (this packet having left the active set), `python3 -m pytest tests/doc-health -q` under `set -o pipefail` **1249 passed / 0 failed** with the exit code read rather than inferred (the 1248 measured on the realization branch plus one main grew after it), and the declared pin class reporting **66 declared pin sites across 23 class members — 50 reachable, 0 orphaned, 1 lost (declared unrecoverable, 0 awaiting a superseding record), 0 inconclusive; 0 uncovered, 0 vanished, 0 future members now carrying pins; 7 legal non-pins, 0 undeclared non-commit values, 3 recognized legacy absences, 0 unused vocabulary members** — `clean` true, exit 0 — IDENTICAL before and after the packet moved, which is the answer this act had to check rather than assume, the class's own inventory sweeping `openspec/changes/**` under four live-plus-archive glob pairs. THE DELIVERED SURFACE: a registry module `scripts/doc_health/pin_sentinels.py` beside the pin class (five conditions, five members, each stating its condition, standing and emitters, with absence declared as a recognized legacy state and NOT a member); a widened sweep in `pin_class.py` that classifies a declared sentinel as a LEGAL NON-PIN and an undeclared non-commit value as a defect, run as a SEPARATE pass whose `_field_re` and `_VOCAB_RE` are byte-unchanged; a `status --porcelain` dirty-tree sentinel branch in `git_generation()` scoped to the corpus pathspecs; the three drifted `proposal-support.py` guards reconciled against the declaration; and 33 new tests in `tests/doc-health/test_sentinel_vocabulary.py` with the 53 in `test_pin_reachability.py` green and unedited. NO CONTRACT BUNDLE IS OWED and it stayed unowed, re-affirmed by PARSE rather than by `grep`: all 40 `contracts/releases/*.digests.yaml` inventories were walked into their 192 distinct member paths and NONE of the seven touched files appears in any of them, so no schema moved, no digest set changed and no release tag is owed. ONE DIVERGENCE FROM THE REALIZATION INSTRUCTION STANDS AND IS RECORDED HERE RATHER THAN LEFT IN THE TASK FILE ALONE: `repo_revision()` was deliberately NOT given a dirty-tree sentinel branch, because that mover has no untrue pin to repair — it compares every source file's sha256 against the committed blob at the revision it stamped and RAISES rather than writing a manifest whose pin the content contradicts, so it already satisfies the generator obligation BY REFUSING, and adding the branch would loosen a live guard no ruling asked to loosen. Brett was shown that flag on 2026-08-28 and pre-authorized the merge-plus-archive sequence WITH IT STANDING, so it archives as a STANDING DIVERGENCE-BY-MEASUREMENT rather than as an unnoticed gap; the reasoning is at `tasks.md` § 2.7. REWRITTEN AT THE ARCHIVE, 2026-08-28, from the forward-looking value that follows, under the `implemented — <evidence>` correction shape ruled 2026-08-22 for register C1 of `docs/archive-record-discrepancies.md` and applied at all three sibling archives: a value that names its evidence as still owed is self-contradicting on an archived change, because the archive gate admits a change only on the evidence such a value says is outstanding. The authored text is kept verbatim rather than discarded, because its bundle measurement is the argument for why nothing was owed. AS AUTHORED: implemented — the openxFactory main line. This surface cuts NO contract bundle, measured rather than assumed: `grep`ing every `contracts/releases/*.digests.yaml` inventory for `pin_class`, `bootstrap-ideation-cross-reference` and `proposal-support` returns nothing, so no schema moves, no digest set changes, and no release tag is owed. The archive gate is therefore merge-plus-green on main: `python3 -m pytest tests/doc-health` green under `set -o pipefail`, `OPENSPEC_TELEMETRY=0 openspec validate --all --strict` green, and the declared class reporting its sentinel sites as legal non-pins with the vocabulary check green in both directions. THE CHANGE SHIPS ACTIVE and archives only after that.
Status: ratified
