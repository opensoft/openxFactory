...
- [x] 9.4 Archive this change once merge evidence and a green run exist on both code surfaces. — Archived 2026-08-04: merge evidence recorded at 9.3 (openxFactory 558c254 + adopt-change realization; codexFactory PR #17 merge 0fed12c; aggregation pointer sync 5dfc06f) and the green-run gate discharged by the 9.1 disposition (Linux-bench green + portable suite + WCAG exception register accepted as v1; Windows/web deferred).
...
This document's citation takes derivation route (b), the archive act itself, because no explicit ratification act appears anywhere on the record: the archive commit `25d8e1c` applied this change's spec delta into the canonical specs, and a change whose spec deltas have PROMOTED is ratified by construction — the reasoning `bdd09c2` recorded and `openspec/changes/archive/2026-08-22-add-doxbench-editing-phase-b/proposal.md` cites as its own. The three-way floor is cleared on the APPROVER axis, the DATE axis and a resolvable RECORD PATH, measured through `doc_health.families` before the line was written, not assumed.

**What Brett's named decision here is and is not.** The archive commit names a disposition ruled by Brett on 2026-08-04, and this change's tasks.md 9.1 records it in full. Read literally, that disposition discharged a REALIZATION gate — it accepted the Linux bench, the portable suite and the WCAG exception register as the v1 realization — and the word "ratified" appears nowhere on this record. The citation therefore names it as the word the archive act was taken on, not as the ratification itself, and rests on the promotion. The gap is stated rather than smoothed, as C2 stated the same gap for `add-lens-gate-verbs`.
