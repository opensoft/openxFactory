...
target_release: implemented — realized on the openxFactory main line as pull request #429, merged 2026-08-27T19:26:29Z as merge commit `94933adfcf993dbef2e74952662a3eb1281693e2`, re-verified at the archive an ancestor of `origin/main` (`git merge-base --is-ancestor`, exit 0) and a REAL TWO-PARENT MERGE (`ae6a1e7e` + `06ede5b7`) rather than a rewrite — read out of git rather than off the pull request page, which matters more in this packet than most, its whole forcing instance being a pin orphaned by a rewriting landing. GREEN THERE, on the final head `06ede5b70fea0952f8182e38ab3c21362879841c`: `pytest-suite` **success** 19:10:42Z → 19:25:49Z, **6991 passed, 20 skipped, 338 deselected, 28 subtests passed, 0 failed** in 877.32s (14m37s) with the counts read out of the job log of run 33107163953 rather than off a summary, and `wallet-validation` **success** 19:10:41Z → 19:11:01Z — both read back from the check-runs API at this act. THE REALIZATION RODE IN THE PROPOSING PULL REQUEST: the superseding record, the discharge mechanism and the five tests landed together, so there is no second pull request to name and merged-plus-green is one event rather than two. LOCALLY ON THE MERGED TREE at the archive: `OPENSPEC_TELEMETRY=0 openspec validate --all --strict` 76 passed / 0 failed before the act and **75 passed / 0 failed** after (this packet having left the active set), `python3 -m pytest tests/doc-health -q` **1183 passed / 0 failed** before AND after under `set -o pipefail`, `tests/doc-health/test_pin_reachability.py` alone **53 passed**, and the declared pin class reporting **63 sites across 20 members — 49 reachable, 0 orphaned, 1 lost (declared unrecoverable, 0 awaiting a superseding record), 0 inconclusive, 0 uncovered, 0 vanished** with the superseding record read from committed state AT ITS ARCHIVE PATH: the live-plus-archive glob pair OD-1 chose doing precisely the job it was chosen for, so the discharge survives the move that would have broken a single-path citation. RE-MEASURED AFTER `origin/main` WAS MERGED IN TWICE — main gained pull requests #433 and #431 while this act ran, and both sets of numbers are kept rather than the later overwriting the earlier: at the final head, with `origin/main` at `c9a1500e`, validate holds at **75 passed / 0 failed**, `tests/doc-health` reads **1209 passed / 0 failed** on the merged head and **1209 passed / 0 failed** on `origin/main` itself in a second worktree (grown by twenty-six for main's reasons and by none for this act), `test_pin_reachability.py` **53 passed** on both sides, and the declared class grew with main to **65 sites across 22 members — 50 reachable, 0 orphaned, 1 lost (declared unrecoverable, 0 awaiting a superseding record), 0 inconclusive, 0 uncovered, 0 vanished** while the discharge answer did not move. NO CONTRACT BUNDLE IS OWED and it stayed unowed, re-affirmed rather than re-assumed: no edited file is a member of any `contracts/releases/*.digests.yaml` inventory, and this act's own doc-health run reports `release-inventory-drift` at 0 findings before and after. REWRITTEN AT THE ARCHIVE, 2026-08-27, from the forward-looking value that follows, under the `implemented — <evidence>` correction shape ruled 2026-08-22 for register C1 of `docs/archive-record-discrepancies.md` and applied at the sibling archives `2026-08-26-fix-release-reachability-race` and `2026-08-27-harden-ideation-readiness-check`: a value that names its evidence as still owed is self-contradicting on an archived change, because the archive gate admits a change only on the evidence such a value says is outstanding. The authored text is kept VERBATIM rather than discarded, because it is the argument for why no bundle was owed and it is the standard this act was measured against. AS AUTHORED: implemented — the openxFactory main line. This surface cuts NO contract bundle, measured rather than assumed: `grep`ing every `contracts/releases/*.digests.yaml` inventory for `pin_class` and `test_pin_reachability` returns nothing, so no schema moves, no digest set changes, and no release tag is owed. The archive gate is therefore merge-plus-green on main: `python3 -m pytest tests/doc-health` green, `OPENSPEC_TELEMETRY=0 openspec validate --all --strict` green, and the declared pin class reporting itself FULLY VERIFIED over a loss that is still declared, still measured and still reported. THE CHANGE SHIPS ACTIVE and archives only after that.
Status: ratified
