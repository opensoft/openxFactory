...
All three § 5 boxes are now ticked, on Brett Heap's separate word of
2026-09-10T13:06Z, verbatim **"archive 887 and close 857 858 when landed"**,
recorded on PR #887 and on issues #857 and #858. **THE REST OF THAT SENTENCE IS
NOT SUPERSEDED AND IS EXACTLY WHAT THIS ARCHIVE DOES** — *"promotion is a
separate act on a separate word, so openxFactory #857 and #858 close at the
archive and not at this landing"*: this is that separate act, on that separate
word, and `Closes #857` / `Closes #858` are on the archive pull request and on
nothing else. **THE SENTENCE THAT OPENS THIS BLOCK — *"NOTHING IS TICKED THAT
DID NOT LAND"* — IS LIKEWISE NOT SUPERSEDED**: § 5's ticks record acts performed
in the archive pull request itself, and where a box's own text says it ticks by
NAMING a successor, the tick records the NAMING and never the doing. Two
successors are filed UNCLAIMED at this archive — **#893** (§ 5.3) and **#896**
(§ 6.1) — and § 6.2 names none because its own ratified text says none is owed.
Every box's ratified text is carried verbatim beneath its tick clause.

**AND ONE ACCOUNTING SENTENCE OF § 4.9 IS CORRECTED HERE, NOT A RECORD.**
`review/ratification-2026-09-10.md`, `review/verification-2026-09-10.md` and
`review/verification-2026-09-10-post-merge.md` are MOVED BY THIS ARCHIVE AND NOT
EDITED BY IT — the post-merge capture's own `Rerun of:` header keeps its wording,
which is scoped to the tree `2e84325a` it measures. What is corrected is this
file's own § 4.9 clause about that capture; the correction is dated, quotes the
ratified wording rather than deleting it, and is recorded below.
...
- [x] 4.9 **AND RE-RUN A SECOND TIME AFTER THE MERGE FROM `main`**, because
      ...
      2026-09-10 capture keeps `Status: record` and is NOT superseded.
      **ONE ACCOUNTING CLAUSE OF THIS BOX IS CORRECTED AT THE ARCHIVE ACT
      2026-09-10, AND THE RATIFIED WORDING IS QUOTED RATHER THAN DELETED.** As
      ratified this box said the first capture is *"preserved unedited"*. That
      is true of the tree this box MEASURES — `2e84325a`, the merge from `main`,
      where `review/verification-2026-09-10.md` is byte-identical to what
      `c95b9e50` committed — and it is FALSE of `e65aed15`, the commit that
      wrote this box, which in the same commit appended a 23-line
      self-disclosing `## Addendum` to that capture. The measurement, not the
      adjective: `git show --numstat e65aed15 --
      openspec/changes/amend-modified-block-currency-standing/review/verification-2026-09-10.md`
      reads **`23 0`** — twenty-three lines ADDED, ZERO removed — so not one
      original line is edited, moved or deleted, not one measured figure
      changes, and the addendum says so in its own first sentence. The literal
      truth is *"every original line preserved, one dated addendum appended at
      `e65aed15` that only points at the second capture"*. **THE RECORD ITSELF
      IS NOT TOUCHED BY THIS CORRECTION**: it is `Status: record`, it is moved
      by this archive and not edited, and `fam_record_immutability` reports
      nothing on it. Codex raised the imprecision as a P1 on PR #887
      (2026-09-10T13:16:44Z, `review/verification-2026-09-10.md:328-331`); its
      proposed remedy — delete the pre-merge capture and move the pointer out of
      it — was REFUSED with the measurement and the refusal is on that thread,
      while the DESCRIPTION's imprecision, which is this clause, is taken here.
      The re-measure was not a formality: `b91af6ea` brought
...
until 2026-09-10T13:06Z, when Brett Heap gave the separate word this section
waits on, verbatim **"archive 887 and close 857 858 when landed"**, recorded on
PR #887 and on issues #857 and #858. The condition *"when landed"* IS MET AND IS
CITED RATHER THAN ASSERTED: PR **#887** merged into `main` as
**`3b45c037a3ed3db5c04c65ea50f9dd8c38d83d57`** at **2026-09-10T13:51:31Z**. The
...
disposition is what a later reader reads beside it.
`review/ratification-2026-09-10.md` says the same in advance — the archive is a
separate act on a separate word, and #857 and #858 close there — and that record
is `Status: ratified`, is **NOT edited by this archive**, and does not
contradict this act, having named it in advance.
...
- [x] 5.1 **DONE IN THE ARCHIVE PULL REQUEST #899 — THE BLOCK IS PROMOTED AND
      ...
      the pinned `@fission-ai/openspec` artifact — never a bare
      ...
      LANDING plus this task list and waits on no realization evidence; the
      landing is PR **#887** → **`3b45c037`** on `main`, 2026-09-10T13:51:31Z.
...
- [x] 5.2 **DONE IN THE ARCHIVE PULL REQUEST #899 — `Closes #857` AND
      ...
      kept both origin issues open through that landing — exactly as this box
      required. The two issues close ON THE MERGE OF THIS PULL REQUEST, which is
      the orchestrator's act and not this lane's; this box records that the
      closing keywords are placed, not that the issues are shut. The ratified
      text follows unchanged:
      **CLOSE openxFactory #857 AND #858 AT THE ARCHIVE**, not at this
      landing. The pull request body carries `refs`, never a closing keyword,
      for exactly that reason.
- [x] 5.3 **TICKED ON THE RECORDING — THE SUCCESSOR IS NAMED AS openxFactory
      [#893](https://github.com/opensoft/openxFactory/issues/893), IT IS
      UNCLAIMED, AND THE WORK IS NOT DONE HERE.** This is the disposition the
      box's own last three sentences reserve: per the ruling of
      2026-09-06T23:10Z, verbatim *"Tick on the recording"*, the box ticks by
      NAMING a filed successor, and the alternative it also admits — the owner's
      word that the grain is correct as it stands — was not given and is carried
      forward as option 1 on that issue. #893 was filed by this lane at this
...
**DISPOSITION 2026-09-10 — WHY THE TWO OPEN BOXES BELOW TICK AT THIS ARCHIVE
AND WHAT EACH TICK DOES AND DOES NOT SAY.** Both were left open at the landing
of PR #887 **by their own text**, and both name the archive act as the moment
they tick: § 6.1 *"THE BOX TICKS ON THE RECORDING … at the archive act it ticks
by NAMING a filed successor issue, or by the owner's word that a build record
needs no annotation"*, and § 6.2 *"The box ticks at the archive act, on the
recording, if a successor is ever named; nothing is owed by this packet."* The
shape is Brett Heap's ruling of **2026-09-06T23:10Z**, in session, by multiple
choice, recorded on openxFactory
[#692](https://github.com/opensoft/openxFactory/issues/692), option labelled
verbatim **"Tick on the recording"** — *an owed successor's task box ticks once
the successor is NAMED (issue filed), the box text saying the tick records the
naming, not the doing.* **NEITHER TICK CLAIMS THE WORK IS DONE.** § 6.1's names
a successor filed UNCLAIMED at this archive; § 6.2's records that its own
ratified sentence says nothing is owed. `specs/019-modified-block-currency-family/`
is NOT edited by this pull request in either direction, and every ratified
sentence below is carried verbatim beneath its tick clause.

**AND THE SANCTIONED ARCHIVE PATH REFUSES AN OPEN BOX**, with no bypass flag:
`scripts/proposal-support.py` refuses any change whose `tasks.md` still matches
`^- \[ \]` (*"change has incomplete tasks"*), so these ticks are taken in the
commit before the move rather than after it.

- [x] 6.1 **TICKED ON THE RECORDING — THE SUCCESSOR IS NAMED AS openxFactory
      [#896](https://github.com/opensoft/openxFactory/issues/896), IT IS
      UNCLAIMED, AND THE WORK IS NOT DONE HERE.** The box admits two
      dispositions and this is the first of them; the second — the owner's word
      that a build record needs no annotation — was not given and is carried
      forward as option 1 on that issue. #896 was filed by this lane at this
      archive act after a sibling search over `FR-024`, `FR-026`,
      `019-modified-block-currency-family`, `FAMILY_RESOLUTION`, *"advisory at
      launch"* and *"launch severities"*: the only issues those return are #857
      and #858 (both taken by this packet) and the closed #730 / #729 / #357, so
      a new issue was filed rather than an existing name reused. **`specs/019`
      IS NOT EDITED BY THIS PULL REQUEST** — the only `specs/019` edit in this
      packet's history is #858's severable FR-018 commit `2f384fd1`, which
      landed with PR #887 and is already on `main`. The ratified text follows
      unchanged:
      **`specs/019` FR-024 AND FR-026 STILL STATE THE LAUNCH SEVERITIES AND
