...
`openxFactory` now owns repo-boundary governance and shared contract ownership.
...
`openxFactory` using the factory workflow itself.
...
- Move or summarize canonical policy into `openxFactory`.
- Move shared contracts into `openxFactory/contracts`.
...
All migration work will be tracked in `migrate-canonical-policy-to-openxfactory`.
...
Canonical material is copied or summarized into `openxFactory` first. Source
...
- Policy duplication persists temporarily -> Mark `openxFactory` canonical and
...
- Should proof harnesses eventually move to `openxFactory/examples` or a
