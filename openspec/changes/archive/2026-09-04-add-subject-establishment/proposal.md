---
code_surface: none — SPEC AND CAPABILITY TEXT ONLY, and the honest reason is that the two artifact kinds this packet defines have no consumer yet. `openspec/specs/subject-establishment/spec.md` is created on promotion; nothing under `contracts/` or `scripts/` moves, and no file in this repository is edited outside `openspec/changes/add-subject-establishment/`, the staging bookkeeping (`ideation/staging/INDEX.md`, `ideation/README.md`), the DTN register row and section, and the README OpenSpec Records entry. SEPARATELY AND AS CORPUS BOOKKEEPING, this packet touches exactly two test files and neither is a realization surface. `tests/sequenced_after/corpus-ledger.yaml` gains ONE ROW — `add-subject-establishment: {state: active, class: sole, declares: absent, prose: false}` — written by `validate-sequenced-after.py . --seed-ledger --moved-by '#491'` and not by hand, because after `add-per-change-sweep-ledger` (ratified and realized on `main` 2026-09-04) EVERY change in the corpus owes exactly one row; no MOVEMENT LOG entry is owed for it, that packet's restated rule reserving entries for moves the row diff does NOT state. And this packet re-baselines ONE assertion in `tests/doc-health/test_sentinel_vocabulary.py` — the exact pin-site count, most recently 69 → 70 on the 2026-09-03 merge of `origin/main` at `95c2cf6a`, re-derived with `pin_class.verify()` on the merged tree rather than by arithmetic — because a full promotion writes a `supporting-docs/manifest.yaml` carrying a `source_revision` pin and that self-gate compares by exact number. It is a bookkeeping consequence of promoting a staged topic at all, required in the same commit as the manifest, and is not a realization surface: the site joins an EXISTING member class (`proposal-support-manifest`), so the member-count assertion beside it does not move — a precise population figure was carried here at authoring and is deliberately dropped, that member's committed set growing with every promotion in the estate and a number in prose being wrong the moment the next one lands. `ideation/cross-reference.*` is deliberately NOT regenerated, and `tasks.md` § Bookkeeping B.5 records the measurement behind that — the index is revision-addressed and proved at its own pin, and the only mechanical script available is a one-time bootstrap seed that would DOWNGRADE the scorer's output. **THE SCHEMAS ARE DELIBERATELY NOT HERE AND THAT IS OD-1, FLAGGED FOR VETO.** The staged topic's own Exit path names "the two artifact kinds … WITH THEIR SCHEMAS", so this packet departs from its declared exit and says so rather than quietly narrowing it: `contracts/schemas/xfactory-subject-design.schema.yaml`, `contracts/schemas/xfactory-platform-realization.schema.yaml` and a `scripts/validate-subject-establishment.py` are named as the successor `add-subject-establishment-contracts`, to be authored against a domain instantiation rather than ahead of one. A LIVE COUNTER-PRECEDENT EXISTS AND IS RECORDED RATHER THAN OMITTED: the identical decision in the sibling packet `add-credential-escrow-checkout` — its OD-2, "this packet carries no schema surface" — was VETOED by Brett on 2026-08-28 over PR #479, on the reasoning that content Brett had ALREADY RULED the shape of should not wait for a successor. That reasoning does not transfer unchanged here (no ruling has fixed either artifact kind's fields, and the second consumer's mapping is not yet authored anywhere), but a reviewer is owed the fact that the same call went the other way six hours earlier.
target_release: none. Nothing in this packet is a member of any contract bundle, so no cut is owed and none is reserved — established by parse rather than by grep, and RE-MEASURED A SECOND TIME on the merged head of 2026-09-03, `origin/main` at `95c2cf6a`, where the declared bundle has moved four minors and a major under this branch to `contract-v3.2` (`contracts/manifest.yaml:3`): `contracts/releases/contract-v3.2.digests.yaml` was loaded and its 283 entries walked, and NO path this change writes appears among them — not `README.md`, not `ideation/README.md`, not `ideation/staging/INDEX.md`, not `docs/domain-neutralization-candidate-register.md`, and no path under `openspec/changes/add-subject-establishment/`; `contract-v3.0` and `contract-v3.1` were walked as well and read the same. THE VALUE IS THEREFORE UNCHANGED AT `none` AND HAS NOT BEEN RETARGETED — the measurement is refreshed, the declaration is not. The first measurement, kept for the record, was taken on the merged head after `contract-v2.1` was cut under this branch on 2026-08-28: `contracts/releases/contract-v2.1.digests.yaml` was loaded and its 192 entries walked, and NO path this change writes appears among them — not `README.md`, not `ideation/README.md`, not `ideation/staging/INDEX.md`, not `docs/domain-neutralization-candidate-register.md`; the three editorial members `scripts/doc_health/release_inventory.py:62-66` names (`contracts/CHANGELOG.md`, `contracts/manifest.yaml`, `contracts/README.md`) are untouched as well, so the release-inventory-drift family has nothing to report either way. If OD-1 is VETOED this field becomes "next additive contract bundle, allocated AT REALIZATION per `docs/contract-versioning-policy.md` and NOT reserved here" — `contract-v3.2` is the declared bundle as of this branch's 2026-09-03 merge of `origin/main` (`contracts/manifest.yaml:3`; it was `contract-v2.0` at authoring, `contract-v2.1` at the 2026-08-28 merge, and has moved four minors and a major under the packet since — which is itself the argument, and the reason no bundle is spent here) and the `contract-v1.28` renumber sweep is the standing precedent for why a proposal must not spend a minor before merge order is known — and the archive gate grows the cut. As authored, the archive gate is merge plus green PLUS the ruling round (OD-6), which is stricter than what `code_surface: none` alone would require.
Status: ratified
Ratified: 2026-08-28 by Brett — one selection over the orchestrating session's triage survey of the ageing staging topics, verbatim: "Progress both". The survey's Group-4 recommendation — that `subject-establishment` and the escrow topic be progressed — was the ORCHESTRATING SESSION'S wording; the SELECTION was Brett's, and the two voices are kept apart deliberately rather than merged into one quotation. **THE CITATION COVERS THE DECISION TO FILE AND NOTHING ELSE.** It does not cover this packet's requirement text, the capability's name, the decision to carry no schema surface, or any other decision in § Orchestrator decisions below; those are the authoring session's and every one of them is flagged there for veto. The six § Open Questions carry a recommendation and NO decision, and they come back to Brett. No approving OpenSpec change exists to name, so the citation takes the record spelling `sanction-ratified-record-spelling` sanctions for exactly that case, and clears its three-way floor: approver (Brett), date (2026-08-28), and a resolvable record path (this file, § What Brett ruled, and this packet's `.openspec.yaml`). THEN, 2026-09-04, Brett Heap (repository owner), in session, recorded on PR #491, verbatim: "accept all, recommendations stand" — a SECOND AND SEPARATE ACT, the RULING ROUND over the six § Orchestrator decisions (OD-1 … OD-6, all ACCEPTED AS PROPOSED, OD-1 accepted with the #479 counter-precedent disclosed and not followed) and the six § Open Questions (OQ1 … OQ6, each RULED: recommendation stands), and THEN the ratification of the packet on that basis. record: `review/ratification-2026-09-04.md`. **RATIFICATION AUTHORIZES PROMOTION OF THE SPEC DELTA AND PERFORMS NO FURTHER REALIZATION** — no schema lands, no validator is written, and `tasks.md` § Archive gate G.1 (OD-6's stricter gate) is satisfied by this act together with the merge of PR #491.
Proposed: 2026-08-28
...
2026-07-30 (that change's task 2.3). This packet is the first artifact that
