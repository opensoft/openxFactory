...
target_release: none — moves no contract bytes; the schema shape it uses was published at DTN-004 and registered at contract-v1.33 by add-client-identity-roster; examples and validator are content-addressed by commit per contract-v1.33
target_release_note: The original value read "next additive contract bundle (allocated at realization per docs/contract-versioning-policy.md)" — AUTHORING BOILERPLATE, ruled mis-authored by Brett in session on 2026-08-25, because this change moves no bytes under `contracts/` and so never owed a bundle at all; the phantom obligation traces to a stale handoff assumption carried forward at authoring time, never to a finding, and the stale PR `#177` (closed 2026-08-25 as superseded) acted on that boilerplate by allocating a `contract-v1.32` cut this change had no standing to cut.
Status: ratified
