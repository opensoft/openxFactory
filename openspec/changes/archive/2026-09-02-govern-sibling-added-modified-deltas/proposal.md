...
and this section re-measures them at this branch's catch-up merges with `main`,
whose `main`-side parents are `7f656980` and, at the second merge of the same
day, `9af98c4d`; the figures below are the SAME under both reads.
...
A sibling's block is `ADDED`, or a `RENAMED` naming that title as its `TO:` half,
so it is never collected into the group at all —
the group is size one FOREVER, not merely today.

| order | what happens |
| --- | --- |
| the basis sibling — adding, or renaming to the title — archives first (safe) | the requirement enters canon, `resolve` returns `canon`, the carriage arms run — a real backstop |
...
**The population, re-measured at the catch-up merges.** Run with the family's own
`resolve` at `7f656980`, the `main`-side parent of this branch's FIRST catch-up
merge of 2026-08-31, and read again UNCHANGED at `9af98c4d`, the `main`-side
...
first two readings, nor at the second catch-up merge — `9af98c4d` reads
20/16/4, the same four pairs, the same zero markers and the same four `ratified`
adders — so the four pairs above are the four #502 measured. Run over this
...
- **`release-realization` gains the rule, by MODIFIED block, and its antecedent
  ...
  active ratified change ADDS — or RENAMES a promoted requirement TO — references
  that change and declares its deltas
  ...
  and the order still holds — a RENAMING basis reaching this clause only through
  what it modifies beside the rename, a `## RENAMED Requirements` block carrying a
  pair of titles and no scenarios of its own.
- **`document-lifecycle` gains ONE ADDED requirement**: the pairing is declared
  ...
  every marker it is not a carriage unit. **THE ` — <reason>` TAIL IS REQUIRED
  AND NONEMPTY, and a paragraph carrying the prefix alone is a DEFECTIVE
  DECLARATION rather than a non-marker** — recognition ends at the closing colon,
  so such a paragraph is a marker that fails the form rather than a body unit,
  and reading it the other way would report the block UNDECLARED and hand a block
  that carries a declaration the remedy for one that carries none. The tail is
  required of THIS FORM ALONE: the other two carry their whole declaration in the
  units they name, while this one names none, so the reason is the only place it
  says WHY — and the only clause the unratified disclosure below can live in, a
  marker with no reason disclosing nothing and being able to disclose nothing.
  **AT MOST ONE such marker SHALL stand
  ...
  looking declared. The proposal-level cross-reference
  stays owed and is explicitly not a substitute. **A basis that is not ratified
...
- **`doc-health` gains TWO ADDED requirements**: the pairing check that
  ...
  first of the FOUR grounds; or a basis that neither adds nor renames to the
  title, or that IS the carrier itself; OR a `by` identifier that
  is not the change carrying the block; OR NO REASON AT ALL, the required tail
  absent or empty, read after the `by` and BEFORE the disclosure so that a
  prefix-only marker is reported for the tail it lacks rather than for a word it
  has no clause to carry — and UNDISCLOSED, a marker naming a basis
  ...
  DIRECTION IS STATED**: the collision is the rename's TARGET, the `FROM:` half
  being a title canon is expected to carry.
...
- **Conversion without pairing was considered and is FORBIDDEN.** It is the one
  ...
  ever, on a breach this packet's own rule forbade. (This pair's basis is an
  ADDITION; the class reads a rename's `TO:` title on the same terms, and no such
  pair exists in this repository today.)
...
- **Predicted severity movement: +4 `warning`, 0 `error`, 0 `critical`, 0
  ...
  pairing class reports four and the misdeclared, self-referential and
  undisclosed states report zero. **THE UNDISCLOSED STATE'S POPULATION IS ZERO
  AND MEASURED**: all four adding siblings are `ratified` (§ Why), so no marker
  the § 6 sweep writes owes the disclosure at all, and the state launches empty
  on this tree in either order. The collision class reports **ZERO — measured,
  not assumed, and RE-MEASURED ACROSS BOTH BASIS FORMS after the class was widened
  to renames**: of 159 active `## ADDED Requirements` blocks none names a
  ...
  test. **THE REASON GROUND MOVES NO FIGURE**: it adds a
  GROUND to an existing state rather than a class, and its launch population is
  zero by an EMPTY SET — this tree carries no marker of the form at all, § 6's
  sweep not having run, so no marker can be missing its tail and the four the
  sweep writes are authored under the rule rather than measured against it.
  **THE HOLD'S WIDENING TO UNRATIFIED BASES MOVES NO FIGURE EITHER**: it
...
unsafe order — an active ADDED block, or an active RENAMED block's `TO:` title,
for a title canon now carries — which
...
- **OQ-2 — whether a `Modified over` marker should survive promotion.** This
  ...
  basis promotes.
- **OQ-3 — two or more active changes writing one title, by addition or by
  rename.** The rule as written
