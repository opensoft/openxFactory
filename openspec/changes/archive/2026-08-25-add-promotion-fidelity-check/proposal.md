...
**D3 IS SPENT — the flip was ruled and is now taken (task 4.2).** The launch
was advisory and the flip was a ruling, exactly as this decision said it had
to be: Brett ruled "ENFORCING, SEQUENCED" on 2026-08-24 (task 4.1's
four-question round, PR #315) and the sequence was honoured before the flip
was built. `_LAUNCH_SEVERITY` is `error` and `FAMILY_RESOLUTION` carries
`"promotion-fidelity": CONTESTED`, moved in ONE commit, and the two structural
tests that pinned the advisory launch now pin its opposite — the same two
halves, asserted in the same two places, because a half-flip in either
direction is still a family nobody chose. The `contested` class stops being a
back door the moment the family gates: a finding that disappears without a
citation SHOULD be an error when the remedy is a governance act. What
`auto-fixable` was honest about — PR #85 proved the remedy is a byte-level
application of already-ratified text — is now carried by the action line
rather than by the class, which is where D4 already put the judgement.
Evidence: tasks §4.2 and §4.3.
