...
Origin: `openxFactory:adhoc:2026-09-16-repromote-engineering-vocabulary` — a BOX of an active ratified packet, not an `ideation/staging/` topic. FILING WAS NOT RATIFYING — stated in the past tense because the later acts have since been given, and this record is read after them. AT ITS FILING this packet carried `Status: draft` on all three lifecycle documents, admitted no text to canon, and performed neither ratification nor archive; both were later acts on Brett Heap's word. **RATIFIED 2026-09-18** (#656 comment [`5728607038`](https://github.com/opensoft/openxFactory/issues/656#issuecomment-5728607038), ruling R-A) and **ARCHIVED** by [PR #1103](https://github.com/opensoft/openxFactory/pull/1103), where the fifteen reached canon. The drafting provenance in `.openspec.yaml` is deliberately NOT restated here: it is the frozen declaration the archive's origin-retention arm reads, it describes the filing truthfully as of the filing, and approval was ADDED beside it rather than substituted for it.
