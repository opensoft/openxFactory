...
target_release: implemented — merged on the openxFactory main line as PR #221 (merged 2026-08-20T01:23:54Z, rebase-merge; the branch's commits landed on main as `1a7c2a1`, `17bba49`, `8c123bf`, `ca9fcaf`, `6c3d87c`) and green there, re-verified at `7312c25` rather than taken from the PR run: `openspec validate --all --strict` 64/64 exit 0, `tests/ideation-dashboard` + `tests/proposal-support` 3729 passed / 14 skipped exit 0, `tests/doc-health` 691 items with no genuine failure, and the realization proof a full lap on a scratch tree — forward, demote, forward again. No contract bundle was cut — the surface is code-only. Authored as `implementation_pending — the requirements land now; the change archives only on merged code with green realization evidence, because all four parts are mechanism corrections`, and that merge-plus-green condition is what PR #221 discharged; rewritten 2026-08-22 by `archive-register-rulings` off this change's own tasks.md §7.2 and banner (register C1)
Status: ratified
