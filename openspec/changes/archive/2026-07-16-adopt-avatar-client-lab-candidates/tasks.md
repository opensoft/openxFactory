...
- [x] 6.1 In codexFactory `apps/avatar-client-lab/`: run `sync_contracts.dart` against the `contract-v1.12` checkout; vendor the new artifacts; pin the register + derivation table as `vendored_evidence_inputs`; retire the interim P10 checklist + faithfulness guard (contract-pin.md rule 5); un-skip the two BLOCKED-on-P8 assertions in `p8_takeover_recovery_replay_test.dart`. — commit `0dd2729` on `task/adopt-v112-lab-resync`; pin baked openxFactory `01960b13` (main moved additively past `afbfae0` before sync; manifest cross-check clean, 44 members).
- [x] 6.2 Wire gate (vi) to test `deriveAvatarState` against the vendored table and resolve the gate (ix)(a) combination half from the table's named combinations (retiring the interim-invariant scoping); `dart run melos run ci` — nine gates green. — commit `ed278ea`; full ci exit 0; no app-behavior change forced (selector already matched the ratified table); all ten media.states kernel-evidenced.
- [x] 6.3 Record realization evidence (resynced pin refs, green gate run) on this change; update the P-ledger rows (P1, P7, P8, P10, P11, P12, P13 → landed) in the 002 feature's plan.md via the codexFactory session that owns it. — plan.md rows flipped in `d4d053b`; evidence recorded here + in codexFactory PR #17 body.
- [x] 6.4 Notify/unblock `implement-avatar-client-lab` §9 (its 9.3 pin-resync is satisfied by 6.1; its 4.4 manifest registration satisfied by 5.1); archive THIS change per release-realization once 6.2's green run is recorded. — DONE: codexFactory PR #17 ("Realize contract-v1.12: avatar-client-lab pin resync + gate wiring + P-ledger close") merged by Brett (Tier-1 approval) at merge commit `0fed12c` (2026-07-16); realization complete (6.1-6.3 evidenced, nine gates green) → this change archived per release-realization. Swept `implement-avatar-client-lab` 4.4 (manifest registration satisfied by 5.1, `d8cd3c2`); its 9.3 left unticked — 9.3's "routine submodule-pointer commit" clause still needs the xFactory aggregation repo to bump the codexFactory submodule to `0fed12c` (not yet done).
