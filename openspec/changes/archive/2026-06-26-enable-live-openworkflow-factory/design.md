# Design: Live openWorkflow Factory Stack

## Baseline

`openWorkflow` is canonical for workflow policy, contracts, and reference
...
keeps `openWorkflow` as the policy and contract source of truth.
...
Install repos must record the `openWorkflow` commit and contract manifest they
implement. Validation must check:

- the referenced `openWorkflow` commit exists;
