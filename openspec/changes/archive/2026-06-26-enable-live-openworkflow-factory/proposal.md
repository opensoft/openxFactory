...
`openWorkflow` now owns the canonical factory workflow, policy, contracts, and
...
the work needed to pin those proofs to `openWorkflow` contracts, make the
...
- Require Hermes and Omnigent install repos to pin the `openWorkflow` contract
...
- No movement of runtime adapters into `openWorkflow`.
