...
The `doxBench model catalog and provider boundary` block below is CURRENT CANON
plus THIS change's own additions and nothing else. It deliberately does NOT
carry `add-doxchat-model-intake`'s additions: the fold direction was reversed on
2026-08-26 (tasks 1.2/1.2a) because intake is unrealized with a real code
surface and cannot archive for a long time, so this change archives FIRST and
intake — the LATER archiver — now declares its own delta relative to THIS
change's outcome, which is the direction `release-realization/spec.md:64-74`
asks for. Do not re-fold intake's text in here.
