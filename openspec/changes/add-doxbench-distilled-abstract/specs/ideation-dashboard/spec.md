...
Generation of a model-derived abstract SHALL be invoked EXPLICITLY by a human control acting on the currently selected subject, and MUST NOT be triggered by a selection change, a mount-time seed, a scope opening, or any other side effect — the docs wheel notifies its consumer on every notch and at mount, so a selection-triggered design would dispatch a model call for every document a reader spins past. An in-flight generation SHALL be cancellable and SHALL state the expected wait with the adapter-declared timeout ceiling as its visible bound. A RE-GENERATE control SHALL be offered against the subject's current content digest. Every generation SHALL carry the subject path and content digest it was generated for, and WHEN a generation resolves against a subject the pane no longer has selected, the result MUST be discarded unrendered and the not-yet-generated caption MUST show for the current subject. An abstract already generated in the session SHALL survive leaving and re-entering the tile, keyed by subject path and content digest, so returning to a document does not spend a second call on an answered question. Where the gate capability is absent on the local console, the generation control SHALL be ABSENT rather than present-and-refusing, and any already-generated abstract SHALL remain readable with its normal caption.
...
- **WHEN** a generation resolves and the pane's selected subject is no longer the subject it was generated for
- **THEN** the result MUST be discarded unrendered
...
- **THEN** the region MUST show a cancellable in-flight state stating the expected wait bounded by the adapter-declared timeout
...
- **WHEN** an abstract request would be dispatched through the chat-turn assembler or its packet purpose
- **THEN** it MUST be rejected — the request declares its own purpose and its own assembler
...
### Requirement: The abstract cache is a separate bounded store keyed by content digest
An abstract cache SHALL be a SEPARATE, separately bounded store and MUST NOT share the chat surface's turn-idempotency ledger, whose per-process instance is bounded by entry count and total bytes and would evict chat records under abstract churn — a scope larger than that bound is the ordinary case, not an edge case. The cache key SHALL include the subject's CONTENT DIGEST as well as its path, so a regeneration after an edit is a NEW key rather than a same-key conflict; a store that refused a changed digest under an unchanged key would refuse every regeneration after every edit. The cache SHALL admit at most ONE in-flight generation per key, with a concurrent request for the same key attaching to it rather than dispatching a second time, and SHALL replay a completed result for an identical key without a second dispatch. Eviction SHALL be deterministic and MUST NOT be ordered by wall-clock time, and a re-dispatch after eviction SHALL be stated expected behaviour rather than an error. Nothing in the cache SHALL be authoritative, and its contents MUST NOT outlive the session or be written to any corpus, snapshot, register, or gate artifact.
...
### Requirement: The two abstracts share one region budget and one accessible name each
The abstract region SHALL present EXACTLY ONE abstract at a time, switched by an explicit control, with the DETERMINISTIC abstract as the opening view, because the upper half of the docs split carries a measured fixed height that two abstracts cannot both occupy and the deterministic one is the only one that always exists. Each abstract SHALL carry its own caption declaring who derived it and what it is not: the model-derived abstract as model-derived, non-authoritative and regenerable; the deterministic abstract as derived from the document's own headers and never as a distillation; a stale abstract as describing an earlier version of the document, with the source digest stated; an ungenerated abstract as not yet generated; and, on the hosted plane, as unavailable on that plane. The model-derived and deterministic captions SHALL be BOTH visible text and part of their region's accessible name; the stale, ungenerated and hosted-plane captions SHALL be visible text within an already-named region. Where two abstract regions exist they SHALL carry DISTINCT accessible names under this surface's established idiom — a named `role=region` with no heading of its own and the surface's exact casing — and neither SHALL be announced as the selected document, which names the selected buffer. No caption SHALL describe the model-derived abstract in terms that imply it may be cited.

#### Scenario: Both abstracts are available for one subject
- **WHEN** a subject has both a deterministic and a model-derived abstract
- **THEN** exactly one MUST render at a time, switched by an explicit control
- **AND** the deterministic abstract MUST be the opening view

#### Scenario: A caption is read by assistive technology
- **WHEN** a reader enters an abstract region with assistive technology
- **THEN** the region MUST announce a distinct accessible name carrying its provenance caption
