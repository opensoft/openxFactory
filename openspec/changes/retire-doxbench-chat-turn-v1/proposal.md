...
Status: ratified
Proposed: 2026-09-01
Ratified: 2026-09-01 by Brett Heap (repository owner) — in-session, on the orchestrator's report of the two packets read together; record at `openspec/changes/retire-doxbench-chat-turn-v1/review/ratification-2026-09-01.md`. THE RATIFIED BASELINE IS THIS PACKET AS FILED AT PULL REQUEST #552 TIP `c5169476`, and ratification authorizes the requirement text ALONE — it performs no realization, moves no schema byte, changes no manifest digest, cuts no bundle and moves no CHANGELOG row. It ratifies the ENCODING of the fallback posture, the ruling of #522 having set only its DIRECTION ("redesigned, not deleted"). The record discloses what the act stood on: NO CODEX REVIEW HAS HAPPENED (three requests on this pull request, three provider usage-limit refusals, zero verdicts — a pass is owed and blocked by nothing here); FOUR low-severity Copilot comments stand uncorrected on the ratified head, listed in the record; and `main` carries an inherited `pytest-suite` red from PR #510's carriage-ledger subject, which this packet neither causes nor fixes.
Origin: openxFactory issue #522, entry 3 of three; measured 2026-08-31 in a decision memo named `deprecations-522-memo-2026-08-31.md`, held on the operator's own machine and DELIBERATELY NOT COMMITTED anywhere in this repository — NAMED and not located, Principle IV forbidding a host-absolute path in a committed file; the durable in-repo referents are issue #522 and its 2026-09-01 ruling comment; RULED by Brett Heap 2026-09-01 in session and recorded as a comment on #522 — "execute all three retirements at contract-v3.0, as the memo recommends", entry 3 "EXECUTE, its own slice", with the `serve.py` v1-fallback posture "redesigned, not deleted". The ruling authorizes the proposal; it does not ratify this text — RATIFICATION IS A SEPARATE THIRD ACT, given in session on 2026-09-01 ("ratify #551 and #552") and recorded at `review/ratification-2026-09-01.md`, which keeps the filing, the ruling and the ratification apart rather than collapsing them into one.
---
