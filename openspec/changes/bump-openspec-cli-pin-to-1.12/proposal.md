...
3. **Stale refusal, which is what keeps a disposition list from rotting into a
   suppression list.** A disposition matched by NO finding in the run refuses
   (`pin-disposition-stale`, exit 2). `add-composed-view-authoring` has one open
...
- **`tests/openspec_cli_pin/`** — 78 tests and two captured real-report
  fixtures.
...
- **`.github/workflows/openspec-cli-pin-gate.yml`** — untouched, and correctly
  so: it never carried the version. This bump is the first live test of that
  design decision, and it passes.
