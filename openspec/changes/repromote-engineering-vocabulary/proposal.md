...
* The fifteen are **CARRIED, not authored**: lifted from `openspec/specs/ideation-dashboard/spec.md`
  BY TITLE by a script, with every byte of their promoted text preserved except at the SIX sites
  ...
  reversing the six edits reproduces the promoted bytes exactly, for all fifteen.
