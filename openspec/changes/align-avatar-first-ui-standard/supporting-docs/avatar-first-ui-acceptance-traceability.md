# Avatar-First UI Acceptance Traceability

Status: draft
Kind: register
Captured: 2026-07-10
Proposed by: align-avatar-first-ui-standard

[`avatar-first-ui-acceptance-map.yaml`](avatar-first-ui-acceptance-map.yaml)
owns the eight `AFU-*` requirements and 25 scenarios split from the original
avatar-client program proposal. Standard, schema, template, fixture, and
validator evidence is owned here. Flutter widget, golden, platform
accessibility, and live-provider evidence remains assigned to
`implement-avatar-client-lab` or `avatar-pilot-hardening` and cannot be claimed
by this change.

Task 3.2 reconciles the map with the UI delta and requires every deferred
scenario to retain a named successor and closed default.
