...
active, avatar listening, avatar speaking, control degraded or lost,
governed action pending, and retention active. No status MAY be communicated
by animation, audio, or color alone. Stop and mute SHALL remain reachable
whenever capture is possible. On control loss the UI SHALL display the
distinction between control loss and media loss and present the stop,
reconnect, or handoff options required by `avatar-client-runtime`.

#### Scenario: Microphone capture begins
- **WHEN** disclosure and consent pass and the user starts capture
...
automated evidence, manual evidence, or both. Golden tests SHALL render on
