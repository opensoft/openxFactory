...
- [ ] 1.1 New `scripts/doc_health/lines.py` carrying the line rule: split on CR, LF
...
- [ ] 1.2 Placed in `doc_health`, not `ideation_dashboard`, on the measured
...
- [ ] 1.3 A dedicated module rather than more of `corpus.py`: neither the reader nor
...
- [ ] 2.1 `corpus.parse_status` counts real lines through the primitive.
- [ ] 2.2 `corpus.parse_kind` too — it sits beside `parse_status` with the same
...
- [ ] 2.3 Sweep for any further reader using the same idiom before declaring the set
...
- [ ] 3.1 `ideation_dashboard/round_trip.py` imports the primitive instead of
...
- [ ] 3.2 `gate_console._flip_status` follows whatever `round_trip` re-exports, so
      the writer's behavior is unchanged by construction. Assert that: the two
      demonstrated damages must stay closed.

## 4. Tests

- [ ] 4.1 A U+2028-bearing header parses: the status is found, and the document is
      NOT reported as lacking one. This is the false-finding shape and it is the
      test the change exists for.
- [ ] 4.2 The 15-line window counts real lines — a document whose pseudo-line count
      exceeds the window while its real-line count does not.
- [ ] 4.3 `parse_kind` gets the same coverage as `parse_status`, so 2.2 cannot
      silently regress.
- [ ] 4.4 The `join(split(t)) == t` property travels with the primitive, over the
      same ending shapes it is asserted on today.
- [ ] 4.5 The existing three-way fence agreement test still passes and now holds two
...
- [ ] 4.6 The two demonstrated writer damages stay closed (form feed in a `Status:`
      line; exotic separators overrunning the window).

## 5. Gates

- [ ] 5.1 `OPENSPEC_TELEMETRY=0 openspec validate align-status-reader-to-real-lines
      --strict`, then `--all --strict`, from the `openxFactory/` root.
- [ ] 5.2 `python3 -m pytest tests/doc-health -q` and `tests/ideation-dashboard -q` —
...
- [ ] 5.3 doc-health full run diffed against a baseline: findings MUST be IDENTICAL.
