...
Folded in — HELD, NOT YET ON A WORD: 2026-09-17 the three round-nine NORMATIVE threads, by Brett Heap's word, verbatim **"<ORCHESTRATOR FILLS>"** — recorded at [PR #1069, comment <ORCHESTRATOR FILLS>](<ORCHESTRATOR FILLS>) — `PRRT_kwDOTAvnrs6jYkwR` (the five cross-repository signals' matching rules), `PRRT_kwDOTAvnrs6jafy6` (the population arithmetic's missing third skip term) and `PRRT_kwDOTAvnrs6jbfFz` (the `truncated` predicate and `punctuation-stripped`'s firing condition). **THIS RECORD IS WRITTEN HELD AND BELONGS BESIDE THE `Folded in:` RECORDS ABOVE**; the hand that fills the three placeholders moves it there, and a word that never comes drops it and this whole commit with it. Both are held rather than folded because each puts something NORMATIVE that no earlier word already decided. `PRRT_kwDOTAvnrs6jYkwR`: the five D3(c) signals were promoted with EXAMPLES rather than with grammars, so the delta now fixes, per signal, the repository-name vocabulary and its closure, the case rule, the forge URL's boundary, the path-joined form's name boundary, the decoration admitted around a bare qualifier word, and the parenthetical's whitespace rule — one scenario where each signal fires and one where it does not. THE VOCABULARY IS STATED IN THE REQUIREMENT BECAUSE THIS REPOSITORY CARRIES NONE TO READ, which `design.md` D3(c) records with the three near-misses it examined and the two alternatives it costs. `PRRT_kwDOTAvnrs6jafy6`: the promoted population closure names TWO skip terms where D3(a)'s rules make THREE, so a tracked entry skipped for leaving the repository root belongs to no bucket and the arithmetic does not close; the delta now names that third term, requires it printed even at zero, and declines the alternative of folding it into the non-file term, which would close the arithmetic while hiding the one thing a reader needs from it. MEASURED: the term is ZERO at `b1df95ee` and at this branch's base, this repository tracking no symbolic link at either head. `PRRT_kwDOTAvnrs6jbfFz`: the closed vocabulary gave `fixture-path` a PREDICATE on the second word and left `truncated` a DESCRIPTION (*"the extraction severed mid-path"*) and `punctuation-stripped` no firing condition at all, while `design.md` D4's three measured sub-probes lived only there; the delta now carries those probes verbatim — a longer existing path on the line ending with the token, a following `<`, and a split string literal whose rejoined path resolves — as a union with the trailing-hyphen arm under the occurrence rule ALL, and gives `punctuation-stripped` the only condition a class asserted on a REMAINDER entry can have: the strip was applied, the stripped token still resolved to nothing, and the `truncated` predicate did not fire. MEASURED: the probes are the union that caught 4 of 4 hand-classified tokenization artifacts with zero false positives, each still caught by the probe it was caught by; and `punctuation-stripped` has NO member in the measured tree, the one sentence-terminal full stop there sitting on a token that RESOLVES once the stop is gone. NOTHING ELSE MOVES: the window is still the citing line plus the three above, the set is still those five signals, the population's three exclusions and its containment rule read exactly as they read before, the class vocabulary is the same four labels with the same precedence, and FLAG-never-drop and the INCLUSIVE headline are untouched.
