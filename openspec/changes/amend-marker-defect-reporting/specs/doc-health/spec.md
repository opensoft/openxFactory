...
the block. A MARKER SHALL ITSELF BE REPORTED ON ANY OF THREE GROUNDS, because a
declaration that does not describe the block is a declaration no reader can rely
on: where it NAMES a unit the block still carries; where its REASON quotes a
code span that, after the same whitespace normalization every unit is read
under, matches a unit of the promoted requirement the block does NOT carry — a
would-be declaration standing on the wrong side of the separator, which names
nothing under the rule above and so leaves that unit reported by the arms with
nothing to show that a declaration for it was attempted three words away; and
where it NAMES something matching no unit of the promoted requirement and no
unit of the block, which is a declaration about nothing. The second and third
grounds REPORT THE MARKER AND SUPPRESS NOTHING, and they replace no part of the
resolution above: a reason-quoted span still names nothing, a name matching
nothing still buys nothing, and the units concerned remain subject to the
carriage arms exactly as they were. Two shapes MUST NOT be reported, being
legitimate prose rather than a defect: a reason quoting a unit the marker
already named before the separator, which declares it once and mentions it once;
and a reason-quoted span matching a unit the block DOES carry, where canon
carries it, the block carries it and nothing is being dropped. THE SECOND AND
THIRD GROUNDS ARE READ ONLY AGAINST A MARKER THE BLOCK'S OWN CHANGE DECLARES —
a marker promotes into canon with the requirement that carries it, so a later
block restating that requirement carries a PREDECESSOR'S marker whose named unit
is necessarily no longer a unit of canon, and reporting that would report every
faithful restatement of an amended requirement, forever, growing with each
amendment. All three grounds carry the carriage ledger's `info` band and, like
every other finding of this family, are NOT classified `contested`, for the
reason the advisory-launch paragraph below states.
...
every note, every marker and every scenario above this note stands exactly as
promoted, this packet's own marker below excepted, and the only change this block
makes to promoted text is to ONE body sentence: the one that stated a single
ground for reporting a marker. TWO SCENARIOS ARE ADDED, at the END of the block,
one for each new ground, because a normative rule no scenario exercises is a rule
the next author re-deriving this family has nothing to test against. No promoted
scenario moves, is retitled or loses a bullet — the promoted scenario *A marker's
reason quotes a code span* is carried WHOLE, its third `AND` included, which
keeps the units a reason-quoted span would have named subject to the carriage
arms; the report is added BESIDE that carriage and takes nothing from it. No arm
is removed, no severity changes, no threshold moves, no disposition rule changes,
no class is added, and the set of trees over which this family speaks is not
altered by one line.

A SENTENCE STATING ONE GROUND FOR REPORTING A MARKER IS NOT A SENTENCE STATING
THE ONLY GROUND, AND FOR TWO YEARS' WORTH OF MARKERS IT WAS READ AS BOTH. The
checker conforms to the retired sentence exactly: `suppression`'s third
resolution — a name matching no canon unit suppresses nothing and reports
nothing — was left silent by its own author, in as many words, because "the delta
mandates exactly one reporting case for a marker, and adding a second is an
obligation this feature has no standing to invent", and it was recorded as a
plausible later ruling. This is that ruling, and it takes a SECOND case besides:
a code span an author writes INSIDE a reason, which `amend-marker-reason-boundary`
(2026-09-06) correctly ruled names nothing, and which is therefore invisible
where it happens to be the very unit the block dropped. MEASURED ON THIS CORPUS
ON 2026-09-09, and the measurement is what bounds the second ground rather than
rhetoric: of SIXTEEN unit-naming markers across every promoted specification and
every active delta, EIGHT quote a code span inside their reason — every one of
the eight PROMOTED, and every quoted span reason-prose — so a rule reporting any
span inside a reason would report canon's own blessed form eight times over and
grow with the corpus. ZERO of those quoted spans is a derived unit of the
document that carries it, which is why the narrow ground is silent on all eight
today; and the shipping path carries TWO unit-naming markers over 23 resolved
active blocks, neither of which either ground reports. **BOTH GROUNDS ARE
THEREFORE INERT ON THIS CORPUS AND NEITHER IS IDLE.** The next author whose
reason quotes a unit their block dropped, and the next author who mistypes a
name, are told so instead of being answered with a carriage row that names the
unit and never the declaration.

**Removed from canon by amend-marker-defect-reporting (2026-09-09):** `A marker naming a unit the block still carries declares nothing and SHALL itself be reported, because a declaration that does not describe the block is a declaration no reader can rely on.` — the sentence states ONE ground for reporting a marker and the paragraph above states three: the ground it states, carried into the first of the three with its clause intact and its rationale word for word; a reason quoting a unit the block does not carry; and a name matching nothing at all. The unit is REPLACED rather than deleted, and this reason deliberately quotes no code span anywhere, so the parser derives exactly one name from it and the reason its author wrote.
...
- **WHEN** a unit-naming marker declared by the block's own change carries a reason, and a code span standing after the reason separator matches, after whitespace normalization, a body unit, scenario title or scenario bullet of the promoted requirement that the block does not carry
- **THEN** the run MUST report the marker itself, at the carriage ledger's `info` band, naming the marker and the unit its reason quoted, a would-be declaration on the wrong side of the separator being invisible to a reader who sees only the carriage row
- **AND** nothing MUST be suppressed by that span, and the unit MUST remain reported by the carriage arms, the report standing beside its carriage rather than in place of it
- **AND** a reason-quoted span the marker already names before the separator, and one matching a unit the block does carry, MUST NOT be reported

#### Scenario: A marker names something no unit matches
- **WHEN** a unit-naming marker declared by the block's own change names a code span matching no unit of the promoted requirement and no unit of the block
- **THEN** the run MUST report the marker itself, at the carriage ledger's `info` band, naming what it named, a declaration about nothing being unusable as evidence about the block
- **AND** nothing MUST be suppressed by that name, the report adding a ground for reporting a marker and changing no part of the three-way resolution
- **AND** a marker carried by the block but declared by ANOTHER change MUST NOT be reported on this ground or the one above, a predecessor's promoted marker naming a unit canon no longer carries by construction
