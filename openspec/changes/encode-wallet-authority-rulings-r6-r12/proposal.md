...
target_release: deferred-allocation — a `digest_subject` enumeration member is a contract artifact change, so the realization lands in a contract bundle whose NUMBER `docs/contract-versioning-policy.md` § Bundle Realization Order allocates AT THE CUT (the third value `add-target-release-deferred-allocation` admitted, openxFactory #1022 → `9378eca5`), and every consumer that pins the bundle receives it by advancing that pin. NO BUNDLE NUMBER IS TAKEN OR RESERVED HERE: the cut, the `contract_bundle_version` bump and the `contracts/CHANGELOG.md` line stay the cutting session's act, exactly as `contracts/manifest.yaml`'s own realization notes require — and this declaration is TEMPORARY by construction: it SHALL be resolved to the literal release identifier that cut allocates (or to `implemented`, should the realization end up cutting no bundle) before this packet archives, by a pull request that names the cut it observed (tasks § 5.3). The requirement text itself promotes at archive; the bundle is what carries the enumeration member to readers.
