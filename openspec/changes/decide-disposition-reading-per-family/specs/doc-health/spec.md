...
- **THEN**, PROVIDED the entry also satisfies whatever FURTHER admission predicate that family's own reader declares beyond the cite (a `date` AND a path under the archive prefix for `ratified-provenance`, `families.py:447-456,527-528`; a string `content_sha256` for `neutrality-drift`, `neutrality.py:669-677`), the entry MUST be read over THAT FAMILY'S OWN FINDINGS exactly as that family's own declaration says and by that declaration alone, whether it suppresses or downgrades — an entry failing that further predicate is left to the family-neutral reader below, admitted or not on that reader's own terms and never this one's
