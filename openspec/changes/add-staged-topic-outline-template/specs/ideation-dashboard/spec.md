...
The tab SHALL offer an add-section affordance. A section added through it SHALL be written by the existing `edit-apply` path on the topic's branch session, scoped by the section it targets — the heading, or the `xspec:candidate` fence where the section is a proposal-element block — as the patch's addressing key. It MUST NOT introduce a second write verb: section-scoped patching is a patch-TARGETING detail, not a different kind of action, and `edit-apply` already supplies the branch-scoped, committed, reviewable machinery. Every section added this way SHALL carry its `Added-by:` provenance, whether the author is the human or an agent.
...
- **THEN** the write MUST go through `edit-apply` scoped to the targeted section
