...
- **AND** this reaches no obligation this estate has not already ratified, so it imposes none on any pin's author: `neutral-product-pin`'s *A dispositioned finding is cited, upgrade-coupled, and refused when stale* already requires a disposition to identify ONE finding by repository, item, delta path and finding text, to carry a NON-EMPTY citation and to name its granting authority, and already states that a disposition recording no `cited_to:`, an empty one, or no granting authority makes the pin REFUSED as malformed BEFORE any artifact is fetched — the offline resolver reading the same entries is this grammar catching up with that text
