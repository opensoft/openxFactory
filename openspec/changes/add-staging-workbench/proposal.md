---
code_surface: codexFactory (snapshot generator per-document completeness scoring, the staging-workbench view and its docs/lens/outline panels, the wheel's WHEEL_ACTIONS / ACTION_MOUNTERS workbench row), openxFactory (additive `ideation-dashboard-snapshot` document `completeness` growth)
...
- BOUND the score to information. Completeness never gates: it is not an
  input to the readiness recommendation gate, produces no doc-health
  finding, and no console verb, guard, or lifecycle transition may consult
  it. A red bar is a prompt to a human, never a refusal.
...
- Affected specs: `ideation-dashboard` (three ADDED requirements: the
  completeness signal, the workbench view, the workbench tile action). No
  MODIFIED requirement: the "Snapshot projection contract" requirement
  already prescribes additive field growth by delta as the way a view
  acquires data, and the "Interactivity boundary" requirement already
  bounds a read-only surface — this change adds within both rather than
  restating either.
- Affected schemas: `contracts/schemas/ideation-dashboard-snapshot.schema.yaml`
  (additive `document.completeness`; no `schema_version` bump, no existing
  snapshot invalidated). No change to `ideation-workbench.schema.yaml` —
  see design D5 on the two senses of "workbench".
- Affected code (codexFactory, Speckit-side): `generator.py`
  (`_document_entry` gains the scoring call; one new deterministic scoring
  module with the fixed weights), `snapshot.py` callers unchanged
  (validation stays schema-driven), the new workbench view plus its pure
  model module, `views/wheel-model.js` (`WHEEL_ACTIONS` rows for
  `clusters` / `possibles` / `staged`) and `views/wheel.js`
  (`ACTION_MOUNTERS` entry), `app.js` nav wiring, tests.
...
- NOT in scope: a tunable weight configuration (v1 weights are contract
  constants); completeness for anything other than a document (no cluster,
  possible, or staged-topic aggregate score — see design D3); any
  re-scoring of the cross-reference readiness tiers; new snapshot fields
  for the lens panel.
