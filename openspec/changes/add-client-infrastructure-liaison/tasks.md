...
- [x] 5.1 Register the two schemas (`type: schema`, per-file sha256) and the validator (`type: tool`) in `contracts/manifest.yaml`; CHANGELOG entry; move the contracts/README row out of "Pending Realization"; `contracts/releases/contract-vX.Y.digests.yaml` + annotated tag at the allocated bundle version. — Realized 2026-07-17 as **contract-v1.13**: manifest bumped + both schemas registered with per-file sha256 (`5cc2dd7e…`, `60a64eb8…`), validator registered as commit-content-addressed tool (v1.11 document-cataloging precedent); seventh annotated-tag CHANGELOG entry; contracts/README rows moved from "Contracts Pending Realization" to the realized inventory; `contracts/releases/contract-v1.13.digests.yaml` built via `validate-contract-release.py build` (179 entries, working-tree = release bytes); annotated tag `contract-v1.13` cut at the realization commit and verified with `verify-commit`/`verify-tag`.
