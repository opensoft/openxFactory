...
*Why.* `Omnigent-Install` PR #213 attempted 6.1 and measured that it could not
...
*What #213 did land, and what it did not.* It retired the 50 paths whose
...
*The hold on #213.* Brett also ruled OQ-6 (design § D5's amendment): **"Land it
...
`tests/test_worker_host_manifest.py` as part of its `code_surface`, and #213
retires both, so **#213 is HELD until that change lands** — otherwise the
retirement re-homes an active change by side effect, which the ADDED
requirement forbids. Its repository work is already merged (`Omnigent-Install`
PR #129 `509b7d65`, PR #143 `5b5592e4`); what remains is a HUMAN/VAULT gate, a
...
- [ ] 6.0 **Realize the `Omnigent-Install` → `OmniWorker-Install` pin**, design
      ...
      (PR #214, authored 2026-09-05 by this change's lane; Brett ratifies).
      **6.1 depends on this box**, on the ratified "Copy-first migration"
      ...
      Also depends on #213 landing, which depends in turn on
