...
bounded Actions artifact. Git repository credentials, deploy keys, Git credential
helpers, raw Git repository clones, and Git repository network access on the
worker are prohibited; the credentialed hosted parent owns those reads and
persists no credential into the artifact. "Repository" in this paragraph means a
GIT repository throughout: the worker's push-scoped credential for the single
`ideation-dashboard` IMAGE repository is governed by the paragraph above and is
neither widened nor withdrawn here.
