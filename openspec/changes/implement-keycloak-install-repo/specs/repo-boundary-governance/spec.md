...
released repository named `Keycloak-Install`, created 2026-08-21 by the
`implement-keycloak-install-repo` successor change as
`opensoft/Keycloak-Install` and, when admitted, pinned into the top-level
xFactory aggregation at `installs/keycloak-install`.
