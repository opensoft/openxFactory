...
- a **SHALL** that the conformance test asserts BOTH equalities, each taken
  ...
  actually pin the enrolment, measured by running exactly the captured pinning
  node ids rather than by sweeping the directory holding the conformance test
  itself, and the captured artefacts against the paths their own allowlisted
  regenerations rewrite — with a
...
withdrawal as the baseline, measure the assertions over the captured node list —
which may not be empty — regenerate and diff against the baseline, record), and
