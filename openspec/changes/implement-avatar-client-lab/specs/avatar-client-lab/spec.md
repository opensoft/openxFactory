...
in the private `xfactory-avatar-client` repository, while openxFactory retains
ownership of the neutral contracts, conformance fixtures, and acceptance
requirements. Aggregation-repo pinning MUST be a separate change.

#### Scenario: A neutral contract or fixture is edited in the client repo
- **WHEN** the client repository proposes a change to a neutral contract, registry, or shared conformance fixture
...
- **THEN** it MUST be contributed to openxFactory's shared fixtures, not forked into the client repo
