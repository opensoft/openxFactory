...
- [ ] 2.5 Implement the Dart draft-2020-12 schema-conformance runner over `contracts/avatar-client/fixtures/index.yaml` as the CI type-faithfulness ground truth (decision D1: spike Workiva `json_schema` first; port the keyword subset if it or its license fails).
...
- [ ] 4.2 Add the openxFactory-owned neutral client-lab conformance fixtures and a client acceptance map; reuse `examples/avatar-first-ui/fixtures/` where they apply; contribute new neutral scenarios upstream.
- [ ] 4.3 Seed M0 from the released `examples/avatar-first-ui/fixtures/deterministic/offline-acceptance.yaml`; one loader normalizes the portable-conformance vs UI-example fixture shapes (decision D6).
...
- [ ] 8.1 M0 walking skeleton: one fixture end-to-end (reducer + static avatar on `listening→thinking→speaking` + avatar_stage/conversation_rail + three harness controls) green on three gates — fixtures/index conformance, replay determinism, one VP-01 golden.
...
- [ ] 8.6 Author the golden strategy (decision D5: authoritative goldens on the Linux CI job only; Windows + web run all tests except pixel goldens); every golden flake treated as a determinism bug.

## 9. Realization

- [ ] 9.1 [GATE] Confirm the offline lab is green on the implemented target: `verify_pin` + schema conformance + replay determinism + boundary/forbidden-API gates + keyboard-only F1-F4 + the golden set, on Windows + web (web a11y per decision D4).
