...
| `policies/repository-identity.yaml` | The estate's former-to-current repository mapping — how a FROZEN former repository identity stays interpretable without a provider redirect (which lapses the moment the former owner reuses the name). Carries the file-level `owner_segment_rule` (a transfer moves the OWNER SEGMENT ONLY, so bare names, commits, repository-relative paths and blob digests are untouched), `pending_row_rule` (a row whose `transfer_state` is `pending` declares a ruled change that HAS NOT HAPPENED and must not resolve a live reference), and per row the redirect's lapse condition, the case-sensitivity divergence, the GHCR-lowercased derived container namespace, and `redirect_does_not_cover` — the three shapes that break outright at a transfer (reusable-workflow `uses:` paths, container package namespaces, federated-credential subject strings). Authored by `adopt-codexfactory-repository-identity` after its task 1.1 was AMENDED 2026-09-08; `adopt-medxsoft-repository-identity` appends its two 2026-08-26 MedxSoft rows. Registered in `manifest.yaml` with per-file SHA-256; pending bundle registration | `openxFactory` |
