...
|---|---|---|
| `council-convening/` + `scripts/validate-council-convening.py` | Neutral resolved council-seat contract family for the codexFactory producer and xFactory-Hermes-Install consumer: closed Draft 2020-12 schema at `council-convening/resolved-council-convening.schema.yaml`, complete indexed corpus at `council-convening/fixtures/index.yaml`, and deterministic provider validation via `python3 scripts/validate-council-convening.py --strict`. Realized in the `contract-v2.3` candidate; successor ownership and coordinated hard cutover are defined in `../specs/026-add-resolved-council-seats/contracts/successor-handoff.md` | `add-resolved-council-seats` |
