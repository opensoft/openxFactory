...
| `omnigent/omnigent-domain-overlay.schema.yaml` | Per-domain Omnigent content payload: worker classes on the five neutral archetypes, generalized permission matrix (constitutional `execute_final_action`/`access_secrets` false), credential tiers incl. `never_assignable`, payload sections, routing, stop conditions | `add-omnigent-domain-overlay` |
| `omnigent/omnigent-install-manifest.schema.yaml` | Omnigent install manifest: Hermes runtime manifest digest-pinned as single stack identity, one tenant / one domain / N subject workloads, subject-workload registry, pre-rendered effective-profile provenance | `add-omnigent-domain-overlay` |
| `scripts/validate-omnigent-contracts.py` | Validator: schema conformance, positive examples, marked negative fixtures, credential-tier disjointness, unique worker ids, canonical-vocabulary key lint | `add-omnigent-domain-overlay` |
| `policies/layer-vocabulary.yaml` | Canonical Subject/Tenant/Domain layer vocabulary: names + roles, legacy mapping (`customer→subject`, `client→tenant`), reserved layer terms, frozen-identifier inventory, per-domain alias table | `adopt-subject-tenant-domain-vocabulary` |
