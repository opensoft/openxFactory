...
def _scratch_checkout(base: Path) -> tuple[Path, Path]:
    ...
    _run(["git", "-c", "init.defaultBranch=main", "init", "--bare",
          str(remote)], cwd=base)
    _run(["git", "-c", "init.defaultBranch=main", "clone", str(remote),
          str(checkout)], cwd=base)
    ...
    # scope projection.
    (checkout / "ideation" / "staging" / "ideation-governance" /
     "detail.md").write_text(
    ...
    _run(["git", "-c", "user.email=t098@example.invalid",
          "-c", "user.name=T098", "-c", "commit.gpgsign=false",
          "-c", "core.hooksPath=/dev/null",
          "commit", "-m", "scratch fixture"],
         cwd=checkout)
...
class _Recorder:
    ...
    def wire(self, page, host_port: str):
        ...
        page.on("requestfailed",
                lambda r: self.failed.append(r.url))
        page.on("request", lambda r: self.foreign.append(r.url)
                if host_port not in r.url else None)
...
def main() -> int:
    ...
    try:
        ...
        entry = doxbench_model.ModelCatalogEntry(
            model_id="fixture-model", label="Fixture authoring model",
        ...
        httpd = serve_mod.build_server(
            web_dir, snapshot_path, checkout,
            host="127.0.0.1", port=0, head=tree_before, actor="t098",
        ...
        with sync_playwright() as p:
            ...
            catalog_hits = []
            page.on("response", lambda r: catalog_hits.append(r.status)
                    if "/workbench/model-catalog" in r.url else None)
            ...
            def open_workbench(tile_text="ideation-governance"):
                ...
                page.wait_for_selector(
                    ".doxbench-canvas:visible", timeout=15000)
                try:
                    page.wait_for_function(
                        "(t) => (document.querySelector("
                        "'#staging-workbench-root') || {textContent: ''})"
                        ".textContent.includes(t)", arg=tile_text,
                        timeout=8000)
                except Exception:
                    seen = page.evaluate(
            ...
            def submit_session_form(label_text, submit_text, fills=None,
                                    selects=None):
                """Open one session-bar affordance form and submit it. Returns
                (verdict, outcome_text) after either verdict renders."""
                page.locator(".swb-sessionbtn",
                             has_text=label_text).first.click()
                ...
                handle = page.wait_for_function(
                    ...
                    "  }"
                    "  return false;"
                    "}", arg=panel_before, timeout=15000)
            ...
            def storage_names_ref(ref):
                return page.evaluate(
                    ...
                    ".includes(ref); } catch { return false; } })", ref)
            ...
            try:
                page.wait_for_function(
                    "document.querySelectorAll('.doxchat-model option')"
            ...
            try:
                ...
                note = page.eval_on_selector(
                    ".doxchat-failure", "e => e.textContent") or "(empty)"
                raise SystemExit(
            ...
            page.select_option(".doxbench-document-picker",
            ...
            save_btn = page.locator(".doxbench-canvas button.doxbench-save:visible").first
            if not save_btn.is_enabled():
                caps_json = page.evaluate(
                    "fetch('/capabilities').then(r => r.json())")
                raise SystemExit(
            ...
            def session_commits():
                ...
                shas = subprocess.run(
                    ["git", "log", "--reverse", "--format=%H",
                     branches[0], f"^{tree_before}"], cwd=checkout,
            ...
            print("STEP 8 OK: ordered Save — Outline then Document, two "
            ...
            def fill_and_hold(tab_label, text):
                ...
                page.click(
                    f".doxbench-canvas [role=tab] >> text={tab_label}")
                for _ in range(6):
                    ...
                    if held == text:
                        return
                raise SystemExit(
                    f"the {tab_label} edit never held against the refresh")
            ...
            (session_wt / "ideation" / "staging" / "ideation-governance" /
             "detail.md").write_text(
                "# Rewritten behind the buffer (smoke)\n", encoding="utf-8")
            _run(["git", "-c", "user.email=actor2@example.invalid",
                  "-c", "user.name=Actor2", "-c", "commit.gpgsign=false",
                  "-c", "core.hooksPath=/dev/null",
                  "commit", "-m", "second actor moves the document base",
                  "--", "ideation/staging/ideation-governance/detail.md"],
                 cwd=session_wt)
            save_btn = page.locator(
                ".doxbench-canvas button.doxbench-save:visible").first
            save_btn.click()
            refusals = []
            for _ in range(20):
                page.wait_for_timeout(500)
            ...
            page.wait_for_timeout(400)
            print("STEP 8b OK: partial Save — Outline committed, Document "
            ...
            page.fill(".doxbench-canvas textarea:visible",
                      "# Outline " + marker + "\n")
            ...
            print(f"STEP 10 OK: session verbs offered ({verbs}); open-pr on "
            ...
            print(f"STEP 10e-1 OK: pull request opened for {branch_a} "
            ...
            _run(["git", "-c", "user.email=t098@example.invalid",
                  ...
                  "-m", "external merge of the session branch (smoke)"],
                 cwd=merge_clone)
            ...
            _run(["git", "-c", "core.hooksPath=/dev/null",
                  "pull", "--ff-only", "origin", "main"], cwd=checkout)
            ...
            assert ("MERGED, so the session ENDED" in outcome
                    or "session merged" in outcome), outcome
            ...
            httpd = serve_mod.build_server(
                web_dir, snapshot_path, checkout,
                host="127.0.0.1", port=server_port, head=head_after_pull,
                actor="t098", repository="fixture-repo",
                pull_request_factory=FakePullRequests,
                model_port_factory=lambda: port)
            ...
            verdict, outcome = submit_session_form(
                "abandon this session", "abandon-session",
                fills={"input": "smoke: exercising the abandon-cleanup live "
                                "variant (T098)"})
            ...
            print(f"STEP 10f OK: {branch_b} abandoned — worktree/registry "
        ...
        ok = (not recorder.page_errors and not recorder.failed
