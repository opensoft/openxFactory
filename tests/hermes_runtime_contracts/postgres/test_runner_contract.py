...
def test_compose_is_internal_has_no_host_port_and_uses_throwaway_storage() -> None:
    ...
    assert networks and all(network.get("internal") is True for network in networks.values())
...
def test_json_run_uses_pinned_image_and_redacts_ephemeral_secret(
    ...
    assert all(secret not in argument for command in _commands(log_path) for argument in command)


def test_failure_still_tears_down_containers_volumes_and_networks(tmp_path: Path) -> None:
    repository = _copy_harness(tmp_path)
    fake = _install_fake_docker(tmp_path)
...
def test_image_lock_refresh_is_explicit_and_uses_official_source_tags(
    ...
    commands = _commands(fake[1])
    for major in ("15", "16"):
        assert any(command[-1] == f"postgres:{major}" and "pull" in command for command in commands)
        assert any(
