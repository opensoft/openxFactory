    ...
    if ids is not None:
        try:
            ...
            if not all(
                ...
                raise StaticProofError("pytest ID requires unsupported escaping") from None
            if len(set(identifiers)) != len(identifiers):
                raise StaticProofError("pytest parametrization IDs must be unique") from None
...
def _is_id_scalar(value: StaticValue) -> bool:
    return value is None or isinstance(value, (int, float, bool)) or (
        isinstance(value, str) and _safe_identifier(value)
    )
