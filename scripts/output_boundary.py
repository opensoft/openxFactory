...
(`split-opendox-two-layer-product` § 2.1, design D2). It used to live at
`scripts/ideation_dashboard/boundary.py`, and `scripts/doc_health/` imported it
from there — `derive_possibles.make_boundary` and
`ideation_readiness.make_boundary`, lazily, so the cycle would not bite at
import time. That was the ONLY edge pointing that way, and it made the two
packages mutually dependent: `ideation_dashboard` imports `doc_health` in
twenty-three places (which is lawful and stays), while `doc_health` reached
back for exactly this one class. A seam cannot be drawn through a cycle, so the
guard moved out of both packages to the one place both already resolve from,
and neither package now owns it. It imports NOTHING from either side — that is
the property, not an accident of the current body, and
`tests/doc-health/test_import_direction.py` asserts it by parsing rather than
by reading.
"""
