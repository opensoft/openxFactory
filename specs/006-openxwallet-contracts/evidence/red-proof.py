#!/usr/bin/env python3
"""Red-proof harness for the openxWallet negative corpus.
...
tripping something incidental while its own suite stays green.
...
SCRIPT = ROOT / "scripts" / "validate-openxwallet.py"


def load():
    spec = importlib.util.spec_from_file_location("validate_openxwallet", SCRIPT)
...
def main() -> int:
    ...
    for code in codes:
        ...
        print(f"  {'RED  ' if errors else 'GREEN'}  neutered {code:38s} "
              f"-> {errors} error(s)")
        if not errors:
            silent.append(code)
    print()
