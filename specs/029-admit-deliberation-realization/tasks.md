...
- [X] T003 `contracts/clearing/permitted-operations.registry.yaml`: ENTRY NUMBER
...
- [X] T004 `scripts/validate-clearing-dispatch.py`:
      `RATIFIED_OPERATIONS = frozenset({"readiness-diagnostic", "deliberation"})`
      and the constant's comment rewritten. *(copy 1 of 5)*
- [X] T005 `tests/clearing/test_register_closure.py`: the INDEPENDENT `RATIFIED`
      copy gains the member — **not** imported from the validator; the
      independence is the control. *(copy 2 of 5)*
- [X] T006 `.github/workflows/clearing-dispatch-gate.yml`: the literal grep
      `\(1 registered operation\)` → `\(2 registered operations\)`, and the
      comment above it rewritten to say what moved it. *(copy 4 of 5)*
- [X] T007 `tests/clearing/test_clearing_gate_wiring.py::test_the_assertion_pins_the_registers_literal_member_count`:
      the pinned literal moves with T006. *(copy 5 of 5)*
- [X] T008 The validator's register-read note pluralizes correctly so the gate's
      new literal is the one the reader actually prints.
- [X] T009 [P] `tests/clearing/test_register_closure.py`: entry-two twins of
...
- [X] T010 [P] `examples/negative/register-carrying-an-unratified-operation.yaml`
      → `coding`, comments rewritten to argue about `coding`. *(ratified 2.4)*
- [X] T011 [P] `examples/negative/manifest-naming-an-unregistered-operation.yaml`
      → `coding`, comments rewritten. *(ratified 2.4)*
- [X] T012 `tests/clearing/test_register_closure.py::test_deliberation_is_refused_by_name`
      RENAMED `test_coding_is_refused_by_name` and re-pointed, docstring argument
      intact. *(ratified 2.4)*
- [X] T013 [P] `examples/dispatch-record-refused.example.yaml` → a refused claim
...
- [X] T015 Author `contracts/clearing/deliberation-return.schema.yaml` per
      [data-model.md](./data-model.md) § 2 and
      [contracts/deliberation-return.md](./contracts/deliberation-return.md).
- [X] T016 `scripts/validate-clearing-dispatch.py`: add the file to
...
- [X] T017 EXTRACT the `VERDICT_WORDS` name scan into one shared function and
...
- [X] T018 [P] `contracts/clearing/examples/deliberation-return.example.yaml` —
      the positive, validating clean.
- [X] T019 [P] `examples/negative/deliberation-return-that-does-not-match-its-shape.yaml`
      declaring `# expected_failure: schema`.
- [X] T020 [P] `examples/negative/deliberation-return-carrying-a-verdict.yaml`
      declaring `# expected_failure: clearing-report-carries-a-verdict`.
- [X] T021 `tests/clearing/test_schemas.py`: `test_the_family_ships_five_schemas`
      → SIX, renamed, with the new filename in the exact list; and
      `test_the_corpus_covers_every_shipped_kind` gains the new kind.
- [X] T022 [P] `tests/clearing/` gains assertions for the new shape: the binding
...
- [X] T023 `contracts/clearing/dispatch-record.schema.yaml`:
...
- [X] T024 `tests/clearing/test_dispatch_record.py::test_the_refusal_grounds_are_exactly_the_two_the_realization_emits`
      → the five, renamed, with the docstring's argument carried forward.
- [X] T025 [P] A packaged record naming each new ground validates clean — proven
...
- [X] T026 `contracts/manifest.yaml`: the new row `clearing-deliberation-return`
      ...
      research.md § O7.**
- [X] T027 `tests/clearing/test_clearing_manifest_rows.py`: `EXPECTED_ROWS` 6 → 7,
...
- [X] T028 ~~Build the release digest inventory~~ **NOT DONE, DELIBERATELY.** An
      ...
      § O8.
- [X] T029 [P] `contracts/clearing/README.md`: the register section stops saying
...
- [X] T030 [P] `contracts/README.md`: the two clearing index rows move.
...
- [X] T033 Prove each of the FIVE frozen copies fails ALONE — revert exactly one,
      five times, observe red, restore. *(ratified 2.6)*
- [X] T034 Prove each of the THREE pinned numerals fails alone — the schema list,
      the manifest row count, the row digests. *(ratified 2.6)*
- [X] T035 Green: `python3 scripts/validate-clearing-dispatch.py .` (0/0, 2
