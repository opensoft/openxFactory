...
- [ ] T003 [P] Add `hosting_declaration_path(root)` to
...
- [ ] T004 `HOSTING_REL` stops being the declaration's path and becomes
...
- [ ] T005 `_refuse_unusable_declaration()` gains the example refusal, worded like
...
- [ ] T006 `scripts/validate-notebook-projection-hosting.py`: precedence becomes
...
- [ ] T007 `.gitignore` the workspace configuration file and state in the ignore
...
- [ ] T008 [P] `test_the_env_var_resolves_the_declaration` — an absolute path in
      `$XFACTORY_NOTEBOOK_HOSTING_DECLARATION` binds the run. (packet 2.4)
- [ ] T009 [P] `test_a_workspace_relative_env_var_resolves_from_the_root` — the
      same declaration reached by the relative spelling binds identically.
- [ ] T010 [P] `test_the_workspace_configuration_resolves_the_declaration` — the
      `.xfactory/notebook-hosting.yaml` arm binds the run. (packet 2.4)
- [ ] T011 [P] `test_the_env_var_wins_over_the_workspace_configuration` — D-1's
      precedence, asserted rather than assumed.
- [ ] T012 [P] `test_a_configured_path_that_does_not_exist_is_undeclared` — the
      uninitialized-submodule case the packet's OQ-A table calls correct.
- [ ] T013 [P] `test_a_resolved_declaration_is_validated` — the validator's
...
- [ ] T014 Make `_declare_hosting()` in `tests/notebooklm/test_sync_notebooklm_books.py`
      ...
      operator move safe.
- [ ] T015 Run the whole `tests/notebooklm` suite and confirm no existing
...
- [ ] T016 [P] `test_absent_configuration_is_undeclared_and_does_not_break` — the
      clone case: reported as a transition state, run continues unbound. (packet 2.4)
- [ ] T017 [P] `test_a_configured_path_resolving_to_the_shipped_example_is_refused`
      — the fail-closed arm; the message names the file and the remedy. (packet 2.4)
- [ ] T018 [P] Re-aim and rename `test_the_committed_record_conforms` to
...
- [ ] T019 Rewrite `examples/notebook-projection-hosting.yaml` as a SYNTHETIC
...
- [ ] T020 Mark it in the record — `hosting.instance: example` — at the two-space
      indent the sync's narrow reader parses, never in a comment. (packet 3.2, OQ-C)
- [ ] T021 Keep the SHAPE: same keys, same order, same comment structure, same
...
- [ ] T022 Take the actor names to role placeholders, keeping
...
- [ ] T023 Add the pointer the public tree is allowed to carry: a comment saying a
...
- [ ] T024 Prove it: `python3 scripts/validate-notebook-projection-hosting.py`
      exits 0 over the synthetic instance. (packet 3.5)
- [ ] T025 Leave `examples/lifecycle-notebook-workspaces.yaml` ALONE — its two
...
- [ ] T026 [P] `tests/notebooklm/test_nlm_auth.py`: `STORED` and `LEGACY` become
...
- [ ] T027 [P] `tests/notebooklm/test_sync_notebooklm_books.py`:
...
- [ ] T028 [P] `tests/notebooklm/test_validate_hosting.py`: `BASE`, `ENTRY`,
...
- [ ] T029 Re-run the whole suite and check the `pytest-suite` pins: SKIPPED must
...
- [ ] T030 Sweep: `git grep -nE "<the four addresses>"` over `tests/`, `examples/`,
...
- [ ] T031 `docs/lifecycle-notebook-projection.md` § *The declaration* and § *The
...
- [ ] T032 [P] `docs/notebook-projection-migration-runbook.md` and
...
- [ ] T033 Tick the packet's own `tasks.md` boxes for work that will be TRUE when
      this pull request merges, and say in the pull-request body which ticks depend
      on the merge and which remain the operator's.
- [ ] T034 `specs/031-configured-notebook-hosting-identity/quickstart.md`: the
      operator's two commands and the one file they write.

## Phase 7: The cross-repository half (packet Group 5 — OPERATOR)

- [ ] T035 Verify the destination repository is PRIVATE before any real identity is
      committed to it. **Gate: if it is not private, stop.**
- [ ] T036 Prepare the hermes-install pull request placing the live record INTACT at
...
- [ ] T037 Label that pull-request body **OPERATOR step — Brett places/merges** and
