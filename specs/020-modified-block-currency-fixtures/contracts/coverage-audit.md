...
| **A13** | **3.8** — a MODIFIED title resolving to an active sibling's ADDED COMPARES nothing (pending, not absent); the same title with no sibling and no canon requirement fires | `test_a_title_pending_on_a_sibling_s_addition_compares_nothing_and_reports_its_pairing`, `test_a_title_resolving_to_nothing_is_reported`, `test_a_capability_with_no_promoted_spec_at_all_resolves_to_nothing` | **satisfied** | Nothing, and one case beyond § 3.8: a capability with NO promoted spec at all is a distinct code path (`promoted()` returns `None` rather than a dict missing the title) and is asserted separately. **ROW RE-AIMED 2026-09-01 by `govern-sibling-added-modified-deltas`**, which renamed the first test with the claim it makes: the COMPARISON half of § 3.8 is unchanged and the block is no longer DROPPED before its pairing is checked |
