...
**Decision**: Validate the YAML mapping, normalize it to canonical JSON, hash it, and load it into staging; SQL never parses arbitrary YAML. The mapping pins source database/schema, fixed v1 tables, snapshot/WAL identity, subject/admin/principal mappings, and approving authority. A default Customer is legal only when all governed ancestry proves one subject.

Cutover starts `SERIALIZABLE`, locks the fixed v1 table list in bytewise order before the first query with `SHARE ROW EXCLUSIVE`, computes counts/digest, migrates, quarantines, reconciles, appends success evidence, and commits. The dataset-digest tool follows the ratified table/row/column/value framing exactly and has golden vectors shared by PostgreSQL 15/16.

Use append-only migration attempts/events. Record STARTED before the main transaction, SUCCEEDED with the authoritative commit, and FAILED/ABANDONED from recovery after rollback. Unique `(migration_id, source_snapshot, mapping_digest)` makes identical retry converge and changed input fail.

**Rationale**: A failure record cannot survive inside the transaction it describes if that transaction rolls back. A separate attempt/event ledger preserves crash evidence without weakening atomic authoritative state.
