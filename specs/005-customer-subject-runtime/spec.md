...
2. **Given** a v1 database with two legacy subject values and a complete approved mapping, **When** migration runs against a frozen source snapshot, **Then** every row ID is preserved, mapped to the correct layer, and reconciled by reproducible counts and dataset digest.
...
- Migration loses its connection after writing some statements, or a concurrent v1 write occurs during cutover.
...
- **Migration Mapping / Ledger**: The approved frozen-source mapping and immutable reconciliation outcome for a v1-to-v2 cutover.
...
- **SC-006**: Clean initialization and repeated initialization produce an identical verified operational contract, and 100% of drifted-object fixtures block readiness.
- **SC-007**: One-subject and two-subject migrations preserve 100% of source row IDs and counts, reproduce the source digest, and leave zero authoritative row for unmapped or unverifiable evidence.
- **SC-008**: 100% of ambiguous/default-map, concurrent-write, altered-map/snapshot, reconciliation, drifted-DDL, and quarantine-boundary negatives fail without partial authoritative state.
...
- **SC-014**: A pre-commit migration crash leaves zero partial authoritative rows, and an identical migration ID/snapshot/map retry converges exactly once; changed-map or changed-snapshot replay fails.
...
- Existing v1 consumers remain supported during the additive bridge; making v2 the default belongs to a later governed major release.
