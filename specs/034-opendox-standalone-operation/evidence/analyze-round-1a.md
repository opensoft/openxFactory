...
- Also for the second pass, with openXdox-code `e28930bf` installed in its
  own venv: `snapshot.find_validator(start)` answers `None` for a start in a
  host's tree, outside the product, and the product's own
  `scripts/validate-ideation-dashboard-contracts.py` for a start inside it or
  for no start. That script's `SCHEMA_FILENAMES` lists the family's ten.
