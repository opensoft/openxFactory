...
- [x] CHK004 Does every one of the now-39 written FRs carry an explicit cross-reference (a box number, a Q-number, or a ruling id) a reviewer can follow to its origin, or do some still assert their MUST with no inline tag? [Traceability, Spec Functional Requirements] — CLOSED 2026-09-09 (round 3). All ELEVEN remaining untagged FRs were tagged: FR-022 *(box 4.8c; clarify Q3)*, FR-035 *(box 5.6)*, FR-040 *(boxes 0.1, 1.1-1.3, 5.1, 5.5, 5.6, 6.1-6.4, 7.1-7.3; architect ruling Q1)*, FR-041 *(architect ruling Q5)*, FR-042a *(checklist finding, evidence CHK031)*, FR-042b *(checklist finding, tooling CHK042)*, FR-042 *(architect ruling Q4)*, FR-043 *(architect ruling Q10)*, FR-044 *(architect ruling Q1, the `3b530009` form)*, FR-045 *(architect ruling Q10; tooling CHK036/CHK037)*, FR-045a *(checklist finding, evidence CHK017)*. VERIFIED mechanically: a regex over every `- **FR-nnn**` heading in spec.md returns ZERO untagged. The same pass also fixed the out-of-sequence FR-005, which now reads FR-001..FR-007 in ascending order.
...
- [x] CHK008 Is "DISTINCT finding codes per leg" (FR-010) precise enough to determine, without reading the User Story 2 Acceptance Scenarios, that the ANCHOR leg shares ONE code across both its digest and locator halves while the LINKAGE leg splits into TWO codes (one per half) — an asymmetry the ratified packet's task 3.1 also carries but FR-010's prose does not surface? [Ambiguity, Spec FR-010, US2 Acceptance 1–2] — resolved: FR-010 now spells out all four codes by name with the rationale for each grouping (`custody-chain-unanchored` as ONE code "because an unanchored chain is one defect however it fails"; `custody-chain-broken-link`/`custody-chain-locator-gap` as TWO "because the linkage halves are independently meaningful"), and additionally rules the simultaneous-failure case (both linkage codes fire).
- [x] CHK009 Is "what it checked and what it did not" (FR-013) given the same itemized content the ratified delta's own scenario "The neutral pass is not a currency claim" specifies (anchoring, linkage in digest AND locator, order, enumerations, entry closure, the unmoved pin, `path_only` digest equality, any withheld outcome, and nothing about the target's bytes), or is FR-013 a strictly weaker paraphrase that an implementer could satisfy with a vaguer line? [Completeness, Spec FR-013, Ratified delta line 280–284] — resolved: FR-013 now enumerates all eight items verbatim plus "nothing about the target's bytes", and adds "A report line that says less than the requirement enumerates is a weaker claim than the one that was ratified."
...
- [x] CHK012 Is "the orchestrator" (FR-030: "the orchestrator re-measures at every merge-from-main and reports") a role defined anywhere in spec.md's Key Entities or elsewhere, distinct from "the LANE" that FR-030's next sentence names as the one who claims the number? [Ambiguity, Spec FR-030, Key Entities] — passes on balance: "the orchestrator" is not in spec.md's own Key Entities glossary (which defines only the entry, the chain, WITHHELD and the candidate commit), but the term is used consistently and only in the one FR-030/plan.md pairing, with no competing or contradictory usage, so the residual ambiguity is minor and does not rise to a requirements defect the way the other findings in this checklist do.
...
- [x] CHK014 Do FR-004/FR-006 and Clarifications Q4's ruling ("Confirmed. Only `contract_schema_version` moves... `$id` frozen... the `contracts/manifest.yaml` row's `schema_version: 1` frozen") describe the SAME set of frozen identity fields? [Consistency, Spec FR-004/FR-006, Clarifications Q4] — resolved: a NEW FR-006 *(clarify Q4)* now states "THREE identity fields MUST NOT MOVE, and the diff MUST prove it" and names all three — `$id` (schema line 7), the record envelope's `const: 1`, and the manifest row's own `schema_version: 1` (manifest line 2041) — citing the manifest's own comment that it "mirror[s] that const, not the schema file's contract_schema_version" (confirmed present at `contracts/manifest.yaml:2058`).
- [x] CHK015 Do FR-021's fixture list and FR-010/FR-011/FR-014/FR-015's validator-behavior requirements name the SAME seven finding codes Clarifications Q10 rules "adopted... verbatim"? [Consistency, Spec FR-010/011/014/015/021, Clarifications Q10] — resolved: re-searched spec.md; all seven codes now appear literally and each now sits inside its own FR's text (not only in Acceptance Scenarios): `custody-chain-unanchored`, `custody-chain-broken-link`, `custody-chain-locator-gap`, `custody-chain-out-of-order` all named in FR-010; `custody-pin-rewritten` in FR-011; `custody-path-class-digests-differ` in FR-015; `custody-content-class-withheld` in FR-014.
...
- [x] CHK025 "A re-derivation does not amend the instrument" (line 68) — traceable to any FR in spec.md's Functional Requirements? [Traceability] — resolved: a NEW FR-007 *(ratified delta, MODIFIED requirement, scenario "A re-derivation does not amend the instrument")* now requires "no `amendments` machinery of any kind... the realization of that scenario is the ABSENCE of code, and the absence MUST be asserted by diff rather than assumed."
...
- [x] CHK046 Is Q4's full four-fact ruling ($id frozen, envelope const:1 frozen, manifest row schema_version:1 frozen, only contract_schema_version moves) encoded completely across FR-004/FR-006 (see CHK014)? [Fidelity, Spec FR-004/FR-006, Clarifications Q4] — resolved alongside CHK014; NEW FR-006 carries all three frozen facts verbatim.
...
- [x] CHK057 Does SC-010 appear in its correct numeric position among SC-001..SC-009? [Clarity, Spec Success Criteria] — resolved: spec.md now lists SC-001 through SC-009 in order, then SC-010 last.
...
Re-read the updated `spec.md` (552→720 lines), `plan.md`, `tasks.md`,
`research.md` and `clarify-questions.md` in full and re-checked all 8
previously unticked items against the new text.

**Tally**: 64 passed / 1 unticked / 0 dispositioned (total 65).

**Resolved (7)**: CHK008 (FR-010 now names all four codes and rules the
both-halves-fire case), CHK009 (FR-013 now enumerates the ratified eight
items verbatim), CHK014/CHK046 (new FR-006 carries all three of Q4's frozen
facts), CHK015 (all seven finding codes now appear inside their own FR's
text), CHK025 (new FR-007 requires the absence of `amendments` machinery,
asserted by diff), CHK057 (SC-010 now sits after SC-009 in sequence).

**Still open (1)**:

- **CHK004** — narrowed but not closed. Every schema (FR-001–007) and
  validator (FR-010–016) requirement now carries a tag, but 11 of the 39 FRs
  still carry none: `FR-022`, `FR-035`, `FR-040`, `FR-041`, `FR-042a`,
  `FR-042b`, `FR-042`, `FR-043`, `FR-044`, `FR-045`, `FR-045a` — the § 4 tail
  and the whole bookkeeping/evidence block. "Every one of the FRs carries a
  cross-reference" remains false as a general claim, though the gap shrank
  from 28-of-32 to 11-of-39.
