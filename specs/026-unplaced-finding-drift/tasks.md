...
- [x] T014 [US1] Add `_shape()` to `scripts/doc_health/modified_block_currency.py`. **AMENDED 2026-08-28 (`6d100e51` / PR #461), and what landed is the amended rule**: mask the `repr` spans with a left-to-right consumer, then match the registered arm templates first-match-wins, the shape BEING the matched template's id; a text no template claims falls back to the lexical mask (quoted spans, then digit runs, two DIFFERENT placeholders), fail-closed. As originally written this box asked for the lexical mask ALONE, which is now the fallback only — packet § 2.4, research R3 (superseded), plan § OPEN-1 (RULED)
...
- [x] T023 [P] [US4] Write `test_two_unplaced_findings_of_one_shape_are_one_remedy` in `tests/doc-health/test_modified_block_currency_reporting.py` — AMENDED scenario 3, first half: findings from the SAME arm template, differing only in the values it interpolates → ONE finding naming the count. Joined after the amendment by `test_two_findings_of_one_template_differing_in_an_unquoted_field_are_one_shape`, which varies the promoted-spec path and the unit-kind list — the two unquoted fields the superseded mask split on — packet § 2.6(3)
- [x] T024 [P] [US4] Write `test_two_unplaced_shapes_are_two_remedies` in `tests/doc-health/test_modified_block_currency_reporting.py` — AMENDED scenario 3, second half: findings from DIFFERENT arm templates, differing in the fixed prose the mask leaves standing → TWO findings. Joined by `test_two_findings_of_different_templates_are_two_shapes` — packet § 2.6(3)
