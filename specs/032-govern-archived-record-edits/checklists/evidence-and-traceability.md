...
- [ ] CHK003 Is "re-run each named command at the named head and reproduce each
...
- [ ] CHK004 Are the four § 4 gate families (4.1, 4.3, 4.4, 4.5) each mapped to
      a distinct, separately-labeled evidence entry, so no gate's result can be
      inferred from another's? [Coverage, Tasks T014-T017]
- [ ] CHK005 Is the pinned CLI's prefix path required as part of the
...
- [ ] CHK006 Is the evidence file's exact path specified, including that a copy
      lives inside the packet's own `evidence/` directory as well as this
      feature's? [Traceability, Spec FR-013]
- [ ] CHK007 Is the evidence file's non-authority stated explicitly — that it is
...
- [ ] CHK008 Is the mechanism by which the evidence file owes no lifecycle
...
- [ ] CHK009 Is a `review/`-homed record distinguished from an
...
- [ ] CHK010 Is the evidence file's required header content specified as a
...
- [ ] CHK011 Is "a box is ticked in the same commit that records its evidence"
      stated as a rule about COMMIT boundaries, rather than about ordering
      within a working tree? [Ambiguity, Spec FR-018]
- [ ] CHK012 Is the failure mode FR-018 exists to refuse — a tick landing ahead
...
- [ ] CHK016 Is the distinction between an INTERIM gate result and a
...
- [ ] CHK017 Is the disposition of an interim result specified once a
      final-head result exists for the same gate — superseded-but-retained, or
      removed — rather than left unstated? [Gap, Tasks T029]
- [ ] CHK018 Is the trigger for re-running a gate at a new head specified
...
- [ ] CHK019 Is the MODIFIED-block currency measurement (task 4.2) required to
...
- [ ] CHK020 Is a way specified for a reader to tell "this measurement is
...
- [ ] CHK021 Is "capture the return code AND the summary line to a file"
...
- [ ] CHK022 Is the failure mode of a piped `tail` — reporting the pipeline's
...
- [ ] CHK023 Is "recorded verbatim" (FR-011) quantified as the literal captured
      text, rather than a paraphrase or a summary composed after reading the
      output? [Measurability, Spec FR-011]
- [ ] CHK024 Is the doc-health finding-set diff required to be recorded as the
...
- [ ] CHK025 Is a rule stated for distinguishing, in the record, a count that
...
- [ ] CHK027 Is the specific fact that licenses this rule — the approval's
...
- [ ] CHK028 Are the exact fields a citation-only note must carry specified
...
- [ ] CHK029 Is "cite records rather than paraphrase them" distinguished from
...
- [ ] CHK030 Is the pinned-target measurement's required wording specified
...
- [ ] CHK037 Is the § 4.2 measurement's own reproduction script (the Python
