...
- [x] CHK008 Is "the pinned OpenSpec CLI 1.12.0, reached only through
...
- [x] CHK011 Is a response specified for the case where
...
- [x] CHK013 Is WHEN the baseline must be captured specified — before the
      ...
      Tasks T004]
- [x] CHK014 Is the consequence of capturing the baseline AFTER the edit (rather
...
- [x] CHK015 Is the baseline's own currency addressed — if `main` advances
...
- [x] CHK016 Is the mechanism for obtaining the `main` checkout specified
...
- [x] CHK017 Is "running a gate twice at the same head yields the same result"
...
- [x] CHK020 Are idempotency expectations stated for the doc-health
...
- [x] CHK028 Is what "the feature stops" means operationally specified for a
      ...
      table]
- [x] CHK029 Is a gate that FAILS distinguished, in the requirements, from a
...
- [x] CHK036 Is the pinned CLI's own installation and availability (the
...
**Re-evaluated after amendments (round 3)**: Key Entities' gate set now names the § 4.2 currency measurement explicitly; new FR-024a states every § 4 gate is expected to be idempotent at a fixed head (generalizing FR-028's doc-health-specific proof to the whole gate set).

**Tally**: 36 passed / 0 open / 0 dispositioned (total 36).

All items pass as of this round.
