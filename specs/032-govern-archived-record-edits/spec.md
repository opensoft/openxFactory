...
1. **THE CORPUS'S CONVENTION IS MEASURED, NOT ASSUMED.** Twelve Speckit feature
   ...
   expression over every file in each tree. Not one was repointed at its packet's archive. The
   one repoint in the tree's history (`3d7b8f3b`, `specs/029`) moved a SINGLE
   pointer to a ratification record, in a file whose only other citation was
   already authored in archive form; it is a pointer fix, not a sweep.
