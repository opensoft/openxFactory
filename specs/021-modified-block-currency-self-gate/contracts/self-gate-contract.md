...
| `test_the_resolution_ordering_and_marker_classes_read_zero_over_the_real_tree` | no finding matches the unresolved rule text (`resolves to no promoted requirement`) or the marker action (`_MARKER_ACTION`); each class named separately. **AMENDED 2026-09-05, TWICE THE SAME DAY:** the ordering class (`the ordering of MODIFIED blocks for`) stopped reading zero over this tree — `split-opendox-two-layer-product`'s ratification made it the second ACTIVE RATIFIED writer of `neutral-product-pin`'s *An external neutral product is pinned by commit and digest, never by tag* beside `add-openspec-cli-pin` — and it is asserted against the NAMED EXACT SET `_ORDERING_SUBJECTS`, on `_LEDGER_SUBJECTS`' discipline, rather than as a band. It READS ZERO AGAIN and that set is now EMPTY, on `_PAIRING_SUBJECTS`' precedent: the ordering declaration Brett Heap ruled (*"declare and land"*, 2026-09-05T12:49Z, `opensoft/openxFactory` issue #656) put `add-openspec-cli-pin` in the later writer's `proposal.md`, which is the band's own written retirement condition. `_ORDERING_TITLE` is KEPT so the first fresh subject fails BY NAME. **AMENDED AGAIN 2026-09-16:** that set is NO LONGER EMPTY — `scope-pinned-arm-root-naming` (PR #1052, `f8eba345`) and `adopt-entry-grain-dispositions-form` (PR #1050, `fa39141c`) landed ratified two minutes apart, each a writer of `document-lifecycle`'s *Prose tagging marker hygiene* with neither proposal naming the other, and `_ORDERING_TITLE` named both rather than leaving an unnamed row. They are NAMED and not resolved: the resolution is the later writer's `proposal.md` naming the earlier, inside those packets, and whoever lands it removes both names here in the same commit — which `ordering_gone` enforces | invert one to `assert len(...) == 1` — fails; and a positive control asserts the three rule-text probes are the module's own strings, so a typo'd probe cannot read zero vacuously | — |
