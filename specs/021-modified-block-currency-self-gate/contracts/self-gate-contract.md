...
| `test_the_resolution_ordering_and_marker_classes_read_zero_over_the_real_tree` | no finding matches the unresolved rule text (`resolves to no promoted requirement`), the ordering rule text (`the ordering of MODIFIED blocks for`) or the marker action (`_MARKER_ACTION`); each class named separately | invert one to `assert len(...) == 1` — fails; and a positive control asserts the three rule-text probes are the module's own strings, so a typo'd probe cannot read zero vacuously | — |
