...
- [x] CHK003 Are the additions to the ratified field list (legend, `duty_separation_rationale`, `exceeds_governed_unit`, and the gate pass's `client_tenant` and `provider_object_ref`) each justified in the requirement that adds them, so a reviewer can see they are not scope creep? [Completeness, Spec §FR-001, §FR-002] — AMENDED by gate rulings G2 and G4: the enumeration grew by two, and FR-001 now carries the justification for each — `client_tenant` because the ratified client-resident refusal has no comparand without it, `provider_object_ref` because the shared-material disjunct otherwise reads tenant identifiers and refuses FR-024's ratified non-finding. Both are declarations of a fact the rule needs, the same footing as the three before them.
...
- [x] CHK027 Is the treatment of promoted spec text consistent across all modified promoted capabilities — never edited here, and rewritten at archive ONLY for the requirements a delta restates? [Consistency, Spec §FR-025, plan §decision 15] — AMENDED by gate rulings G5/G7: the parenthetical "rewritten at archive" was true of capabilities and false of requirements. The archive replaces exactly the requirements a delta carries, wholesale, and leaves every other promoted requirement untouched — so a declared MODIFIED capability rewrites nothing by itself, and an omitted scenario inside a restated requirement is DELETED. The consistent treatment is therefore twofold: leave promoted files unedited AND make each delta carry every promoted requirement the change alters (`review/amendment-record-2026-08-15.md`).
