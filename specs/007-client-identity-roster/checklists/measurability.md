...
- [x] CHK1212 Is the misplacement criterion measured by fixtures placed where the rule is hardest — outside `credentials/` entirely AND nested inside `credentials/client-identity-roster/<sub>/`? [Measurability, Spec §SC-005] — AMENDED by gate ruling G3: the hardest case is now the INSIDE-BUT-NESTED one, not the outside-`credentials/` one. Under the pre-gate directory-prefix reading of FR-036 a nested instance was tolerated by the sweep and missed by the flat validating glob — covered by nothing — while any reading catches the outside-`credentials/` case. FR-036 is now an exact-path (direct-child) predicate and repo fixture 7 measures the nested arm.
