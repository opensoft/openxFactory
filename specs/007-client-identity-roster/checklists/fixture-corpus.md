...
- [x] CHK406 Are the counts of packaged negatives internally consistent across plan.md's structure listing, its Cluster C prose, and tasks.md's authoring tasks? [Consistency, plan §Cluster C, tasks §4.1–4.4] — FIXED: re-counted after the three additions — 31 packaged (19 + 8 + 3 + 1), 34 rule-homes, 36 refusing probes, ~35 packaged fixture files; the FR-016 table stays 16 named / 16 homed. RE-COUNTED after the gate pass: the PACKAGED count is unchanged at 31 (rulings G1–G4 add no packaged file), the repo-shaped corpus grows from six to EIGHT (fixture 7, nested misplacement, G3; fixture 8, an ended instrument cited by a non-retired entry, G1), so refusing probes go 36 → **38** while rule-homes stay 34 — both new fixtures are second probes of rules already homed. The doc-health corpus grows by the equal-`provider_object_ref` fixture (G4) and still contributes ONE rule-home.
