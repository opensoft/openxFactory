...
- [X] T027 [US3] Implement acceptance-map parity check `AFUV-PARITY-ACCEPTANCE` in `scripts/validate-avatar-first-ui.py`: consume `examples/avatar-first-ui/avatar-first-ui-acceptance-map.yaml` read-only; assert 8 requirements / 25 scenarios; and resolve every fixture's scenario `evidence_ids` against the map via `evidence_id_template: TEST-{scenario_id}` (no hand-authored `TEST-` IDs) per spec §SC-008 (I1 disposition)
