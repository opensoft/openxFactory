...
> **A RECORD, NOT THIS BRANCH'S BODY.** This is PR #584's captured narrative,
> kept unedited as the record of the realization round. That realization landed
> from a sibling session as **PR #587**, squash `3fa222f3`, and this feature was
> then re-scoped onto `fix/spent-state-containment-hardening` — see
> [`spec.md`](./spec.md) § *RE-SCOPING NOTE* and
> [`evidence/hardening-red-first.md`](./evidence/hardening-red-first.md). The
> declared single red described below belongs to PR #584 and NOT to the
> hardening branch, whose self-gate is green because the declaration is already
> live on `main`.
