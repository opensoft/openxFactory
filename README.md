...
- [prepare-openspec-1.12-readiness](openspec/changes/prepare-openspec-1.12-readiness/proposal.md)
  ...
  *"start the 1.12 upgrade fixes"* — an ADMISSION TO THE QUEUE, not a
  ...
  `9e869acc`, same tree, two tools: the PINNED entrypoint
  `scripts/validate-openspec-cli-pin.py --all --strict` reads **90 passed /
  0 failed** and `1.12.0 --strict` reads **48 passed / 42 failed of 90**.
  **Not one of the 42 was introduced by any lane**: 1.12.0 added a
  ...
  `## 4.` header was never written). **AFTER: 91/0 under the pin — prior count
  plus this change — and 89 passed / 2 failed under 1.12.0.** THIRTY-NINE real
