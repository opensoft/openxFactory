...
- [govern-archived-record-edits](openspec/changes/archive/2026-09-09-govern-archived-record-edits/proposal.md)
  ...
  `opsXfactory-1`), through the PINNED OpenSpec CLI `@fission-ai/openspec@1.12.0`
  — `openspec archive govern-archived-record-edits --yes`, rc 0, `specsUpdated:
  true`, `added 2, modified 1, removed 0, renamed 0`. On the
  `release-realization` rule its own front matter names: `code_surface: none`, so
  the packet archives **ON LANDING** rather than on merged-plus-green realization
  evidence. Ratification landed at PR
  ...
  `9a67d42c`, which is this archive's base.
  **PROMOTED — ONE `## MODIFIED` AND TWO `## ADDED`, ALL THREE BYTE-IDENTICAL TO
  THE ARCHIVED DELTA.** `document-lifecycle` goes **18 → 20** requirements; the
  ...
  ``**Removed from canon by …**`` marker is owed; the 17 requirements the delta
  does not name are byte-identical before and after; the file's whole diff is
  **+238 / −0** and the only change outside the three blocks is the blank line
  the serializer inserts before `## Requirements`. No file is added under
  ...
  **BOX 4.2 WAS RE-RUN AS THE PRECONDITION AND CANON HAD NOT MOVED**: 5,815
  characters, 0 canon lines removed — character-for-character the figure this row
  states below, which is why the block could be carried forward as ratified
  ...
  **THE SEVEN PACKET FILES ARE PURE RENAMES** (`git diff -M --numstat` reports
  ...
  edit to "a task record or its tick state" on an archived file, so § 4.2, § 6.1
  and § 6.2 were ticked in the commit BEFORE the move, while the packet was still
  live — the shape `refresh-install-repository-enumerations` used at `ca4a1558`.
  **ONE BOX ARCHIVES OPEN AND IS NAMED RATHER THAN CLOSED QUIETLY**: § 6.3,
  `[OPERATOR] The archive word`, for which no recorded word exists at the archive
  commit; Brett Heap's approval of the archive pull request is what discharges
  it.
  **THE ROW THIS REPLACES IS CARRIED FORWARD BELOW RATHER THAN REWRITTEN**, its
  two dated amendments included — the archive moves the row and edits none of
  what the row already said. **EXACTLY TWO BYTES-LEVEL CHANGES WERE MADE TO IT,
  BOTH PATH REPOINTS AND NEITHER A CLAIM**: the row's own link, above, and the
  one citation of `review/ratification-2026-09-08.md` below, each now naming the
  archive path the file actually sits at. Every other character reads exactly as
  it read under *Active changes*:
  — authored 2026-09-08, **`Status: ratified`** (2026-09-08, Brett Heap
