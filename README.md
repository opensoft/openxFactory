...
- [add-signed-execution-chain](openspec/changes/add-signed-execution-chain/proposal.md)
  — **RATIFIED 2026-08-29 by Brett Heap** (`review/ratification-2026-08-29.md`), and its **code surface is now REALIZED**: `contracts/signed-execution-chain/` (four record kinds plus the one digest construction), the packaged corpus, `scripts/validate-signed-execution-chain.py` as the NAMED READER, and the running `signed-execution-chain-gate` check. Three boxes stay deliberately open and are not the author's to close — making the check REQUIRED in the ruleset (4.5, an operator act, without which the capability confers and refuses nothing and says so about itself), its evidence (4.6), and registration at the next additive bundle cut (4.7). Exit 1 of
