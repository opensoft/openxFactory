...
- [add-notebook-hosting-credential-custody](openspec/changes/archive/2026-08-31-add-notebook-hosting-credential-custody/proposal.md)
  ...
  so that a dependent packet could archive at all.** Ratified 2026-08-23 (Brett
  Heap, in-session; `review/ratification-2026-08-23.md`) with two rulings in one
  read — ratify the requirement set, and the **residency redirect ACCEPTED**:
  neutral obligations here, the live binding instances in the consuming installs.
  **REALIZED 2026-08-26** by PR **#395** ("Realize custody §1–§3: the binding
  named, the secrets refused by name"), merge `1dd822ea`, re-verified an ancestor
  of `main` at the archive act; **`target_release: none` and it held — `git diff
  origin/main -- contracts/` is EMPTY and the bundle stays `contract-v2.5`**, no
  CHANGELOG entry, no manifest row, no inventory, no tag. Every §1–§3 box was
  checked against the tree rather than trusted: the `custody:` block naming the
  BINDING and never the secret (with `session_state_in_custody: false` and
  `self_hosted` left free of the obligation), `_check_custody` plus the
  secret-shaped refusal running over the whole hosting block, the both-directions
  tests, and the three documentation surfaces including the rule that **the `nlm`
  profile is refreshable session state and is NOT a custody subject**. **THE
  ORDER IS THE POINT.** `add-binding-consumer-identity` carries a MODIFIED block
  over `Each consuming system reaches a shared operated identity through its own
  binding` — a requirement THIS packet ADDS — so `openspec archive` aborted on
  that packet with *"MODIFIED failed for header … - not found"* while this one
  stayed active. Its §7.2 pre-archive `grep` read **0** before this act and **1**
  after. Brett ruled **Route 1** on 2026-08-31: the basis archives first, the
  requirement enters canon, and the dependent block becomes a block over canon
  like any other — rather than converting that block to ADDED, which
  `govern-sibling-added-modified-deltas` (ratified and merged the same day,
  `8f986c39`) calls **a breach** unless taken as a paired act. **PROMOTION
  VERIFIED BYTE-FOR-BYTE with no scenario-completeness exposure to measure**: the
  packet is **pure ADDED** across both deltas — no MODIFIED, REMOVED or RENAMED
  block anywhere — so no canon scenario set was being restated. Measured anyway:
  **3 requirements in, 3 out, all three bodies byte-identical (3/3), 14 scenarios
  in, 14 out**; `credential-contracts` **7 → 9 requirements, 24 → 34 scenarios**;
  `lifecycle-notebook-projection` **+1 requirement**; neither ADDED title existed
  in canon beforehand, checked by `grep`. `doc-health --family
  promotion-fidelity` → **No findings**. `.openspec.yaml` moved rather than being
  lost, proved by blob `36d76fdc` identical across the move. **FIVE BOXES SURVIVE
  UNTICKED BY DECISION and each carries a restated disposition**: §4.1 the
  declined fixture (shipping one would trip `shared-secret-identity`, and
  relaxing that check is its own change), §4.2 and §4.3 the residency statements,
  **§4.4 an OPERATOR ACT** — the password's placement into
  `kv-opensoft-xfactory-qa`, which nothing in this repository performs or
  witnesses — and §6.2, which attaches to `add-notebook-projection-identity`'s
  archive and not to this one. **§4.5 DID close**: the named successor it owed is
  `add-binding-consumer-identity`, whose `consumer:` block is published and cut
  as `contract-v2.4`, so the box is ticked on the shipped shape rather than on
  the successor's existence.
