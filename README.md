...
- [add-neutral-product-standalone-operability](openspec/changes/add-neutral-product-standalone-operability/proposal.md)
  ...
  openDox-code and openXdox-code; `target_release: implemented`** (the two code
  legs: the assembly root gains README documentation and openxFactory manifest
  annotations, and neither is a runtime artifact) — the requirements land now,
...
- [add-target-release-deferred-allocation](openspec/changes/add-target-release-deferred-allocation/proposal.md)
  ...
  **ALL TEN OQs = (a), THE RECOMMENDED OPTION, SO THE DELTA MOVED NOT ONE BYTE**
  ...
  packet names and does not take."*). It admits **`deferred-allocation`** as the
  THIRD `target_release:` value — a contract bundle a change realizes into whose
  NUMBER `docs/contract-versioning-policy.md` § Bundle Realization Order
  allocates AT THE CUT, so that no number exists to be named at proposal time,
  and twelve active packets were departing from one ratified rule in order to
  obey another. Two `## MODIFIED Requirements` blocks over `release-realization`
  — *Realization axis declaration* (pre-text written from
  `add-structured-scope-substrate`'s OUTCOME, that change being ACTIVE and
  ratified over the same title, so `sequenced_after: [add-structured-scope-substrate]`
  is declared as *Ordered deltas and branch vocabulary* requires) and
  *Realization axis vocabulary is gated* — plus four rules the new value needs
  and canon does not have: it is available only where a bundle is cut; it MUST be
  resolved to a literal release before the packet archives; the resolving pull
  request MUST name the cut it observed; and the archive is REPORTED and still
  never judged, the promoted *"SHALL refuse nothing there"* admitting no
  exception for this value. The realization rides in the SAME pull request, the
  precedent's shape: `scripts/target_release.py` admits the token, the report
  counts it, and archived records still carrying it unresolved are counted and
  refused never. **`CLOSED_REGISTER` DOES NOT MOVE AND NO ENTRY IS ADDED** — this
  admits a VALUE, which is the distinction the promoted requirement draws when it
  says admitting a new value *"SHALL be a change to this specification rather than
  an addition to the register."* The twelve standing entries are NOT swept: each
  retires on its own packet's correction, the entry deleted in that same pull
  request by the stale-entry refusal that already exists. **THE FIRST CONSUMER IS
...
- [adopt-entry-grain-dispositions-form](openspec/changes/archive/2026-09-18-adopt-entry-grain-dispositions-form/proposal.md)
  ...
  *"ratify 1052 when green, then 1050"*) and from the realization's landing
  word
  ([#1045 5717398099](https://github.com/opensoft/openxFactory/issues/1045#issuecomment-5717398099),
  *"merge 1094 when green after 1089"*). **`code_surface:` NON-EMPTY**
  (`scripts/doc_health/pin_shapes.py` and its two test files under
  `tests/doc-health/`), so under `release-realization` this packet archives
  ONLY on MERGED-PLUS-GREEN REALIZATION EVIDENCE and never on landing: the
  filing and ratification landed as PR
  [#1050](https://github.com/opensoft/openxFactory/pull/1050) →
  `fa39141cec2b`, the ratifier's partition amendment as PR
  [#1088](https://github.com/opensoft/openxFactory/pull/1088) → `893abe97`,
  and **THE REALIZATION AS PR
  ...
  WITH `main` GREEN AT THAT COMMIT** — the evidence this archive stood behind.
  **AND IT CARRIES ITS RATIFIER'S OWN AMENDMENT.** On 2026-09-17, answering
  the RULING NEEDED the packet's record posted on 2026-09-16 — an overlap
  Copilot found between the already-promoted positive-resolution scenario and
  this packet's own new refusal scenario — Brett Heap ruled, verbatim
  *"Apply the partition"*, openxFactory
  [#1045 5714433011](https://github.com/opensoft/openxFactory/issues/1045#issuecomment-5714433011),
  that the positive scenario *A marker names a capability of a pinned neutral
  product*'s WHEN gain ONE clause — every OPTIONAL member the matched shape
  admits that is PRESENT and for which that shape's own in-tree verifier
  exposes a PURE, SOURCE-FREE guard must conform AT ITS ENTRY GRAIN to that
  guard, today `dispositions:` alone — with its disjointness bullet gaining
  the matching exclusion. Recorded as an AMENDMENT BY THE RATIFIER at
  [`review/ratification-2026-09-16.md`](openspec/changes/archive/2026-09-18-adopt-entry-grain-dispositions-form/review/ratification-2026-09-16.md)
  § Addendum, 2026-09-17, in the form
  `add-requirement-ref-resolution-integrity`'s 2026-09-01 amendment set.
  **THE PROMOTION, MEASURED:** the one `## MODIFIED` block is BYTE-IDENTICAL
  to canon after the archive — `document-lifecycle` *Prose tagging marker
  hygiene* **45,963 bytes, sha256 `2099a22f69cb23b0…`** on BOTH sides, canon
  `--numstat` **+11 −2**, the requirement's scenarios **24 → 25**. What
  promoted is EXACTLY the ratifier's amendment above plus this packet's own
  new scenario *A pin record's optional dispositions member carries a
  malformed entry* (its title and its seven bullets) — the two units the
  carriage-ledger arm reported as UNCARRIED against the ACTIVE block were that
  same amendment, and the promotion is what discharges them. **ORIGIN
  RETAINED** — the archive tool's own line, "declaration unchanged since the
  ratifying commit `fa39141cec2b`". **THE `tasks.md` § 3 VERIFIER LINE
  CITATIONS STAND, AND THE RE-POINT IS RECORDED RATHER THAN APPLIED:** they
  were measured at the realization's base `4b53ea99`, where
  `scripts/validate-openspec-cli-pin.py`'s `pinned_dispositions` begins at
  `:787`; PR [#1056](https://github.com/opensoft/openxFactory/pull/1056) →
  `3e32d987` moved that file by +7 lines and it now begins at `:794` — RULED
  STAND by the lane at PR #1094, on the principle
  `docs/document-lifecycle.md:556` states for archive directory names, history
  being history and every citation standing; the live fact is already carried
  where it is ASSERTED, in `tests/doc-health/test_pin_shape_adapter.py`.
  `tasks.md` § 4.1–4.3 tick on this recording, § 4.3 having RETIRED the
  packet's `_LEDGER_SUBJECTS` row in
  `tests/doc-health/test_modified_block_currency_self_gate.py` (thirteen →
  twelve unique) on the row's own stated condition. openxFactory
  [#1045](https://github.com/opensoft/openxFactory/issues/1045) closes on this
  archive, by the lane's hand after the ARCHIVED record — this pull request
...
- [gate-code-surface-declarations](openspec/changes/archive/2026-09-16-gate-code-surface-declarations/proposal.md)
  ...
  **`code_surface: openxFactory` IS NON-EMPTY**, so under `release-realization`
  ...
  Brett Heap, verbatim *"ratify"*, at
  [#1018](https://github.com/opensoft/openxFactory/pull/1018#issuecomment-5649742729);
  ratifying commit `b4e0087aaf48`, **ORIGIN RETAINED** and unchanged since).
  **THE MERGED HALF, IN HAND:** the realization pull request
  [#1029](https://github.com/opensoft/openxFactory/pull/1029) — *"Realize
  gate-code-surface-declarations § 3: gate the `code_surface:` head"*, the
  reader `scripts/code_surface.py`, the house validator
  `scripts/validate-code-surface.py`, the closed register
  ...
  **THE PROMOTION, MEASURED:** the three `## ADDED` requirements — *Code-surface
  ...
  in a closed register* (eighteen scenarios, 10 + 4 + 4) — are BYTE-IDENTICAL to
  the text now standing in `openspec/specs/release-realization/spec.md`: 18,033
  bytes, sha256 `a4322cda8b7dd5f9…` on both sides, `diff` exit 0, canon
  `--numstat` +247 −0. **NO PROMOTED BYTE IS EDITED** — the delta is ALL-ADDED
  over three titles novel to the corpus, so *Realization axis declaration* and
  *Realization archive gate* are untouched and no `sequenced_after` is owed.
  **NO ORDERED-DELTA CHILD DEPENDS ON THIS CHANGE** (searched
  `openspec/changes/` for a `sequenced_after:` naming it; none).
...
- [add-declared-former-id](openspec/changes/archive/2026-09-15-add-declared-former-id/proposal.md)
  ...
  **`code_surface: openxFactory` IS NON-EMPTY**, so this archive rests on
  MERGED-PLUS-GREEN REALIZATION EVIDENCE and never on landing or on
  ratification. **THE FOUR REALIZATION MERGES:** PR
  [#1038](https://github.com/opensoft/openxFactory/pull/1038) →
  `701c8fded24963a855d7f34155821cd4ec70c100` (slices 1–3 — the `former_ids:`
  declaration and its reader, the baseline resolved across every declared
  identity, the fail-closed history reads), PR
  [#1037](https://github.com/opensoft/openxFactory/pull/1037) →
  `8f3937584da8c4ba734ec087c9d613552a9cb217` (slice 5 — the identity-based
  reference resolver and the named consumer it was built for), PR
  [#1039](https://github.com/opensoft/openxFactory/pull/1039) →
  `92d519b0c9e700f288c7d8e19cdf0b8312353f60` (slice 4 — the landing validator
  `former-id-arrival-gate`), and PR
  [#1041](https://github.com/opensoft/openxFactory/pull/1041) →
  `79ac94b74ec6418b9fb149f9ba70767ac2635773` (slice 6 — the
  `docs/document-lifecycle.md` amendment and the ticks).
  **THE GREEN RUN, WITH THE THREE MERGES THAT HAVE NO DECIDED RUN OF THEIR OWN
  NAMED RATHER THAN GLOSSED.** `pytest-suite`'s concurrency group is
  `pytest-suite-${{ github.ref }}` with `cancel-in-progress: true`, and on
  `main` that ref is one string, so each landing cancels the previous main run:
  only `701c8fde` has a decided green run of its own
  ([34897189477](https://github.com/opensoft/openxFactory/actions/runs/34897189477),
  `selected=7626 passed=7620 skipped=6 failures=0 errors=0`), while
  `8f393758`, `92d519b0` and `79ac94b7` were each CANCELLED (runs 35011209877,
  35012492979, 35025437085). **#1037 and #1041 close on the house fallback**:
  their pull requests' own green runs
  ([34905703945](https://github.com/opensoft/openxFactory/actions/runs/34905703945),
  [35023330889](https://github.com/opensoft/openxFactory/actions/runs/35023330889))
  ran on merge refs `34ab52f3` and `018aa2ed`, whose trees
  (`1ee37ec0…`, `cce45860…`) are MEASURED EQUAL to those merge commits' trees.
  **#1039 DOES NOT, AND IT IS SAID SO**: its merge ref `a7fc93e9` was built on
  `74348374` before #1037 landed, tree `bd318880…` against `92d519b0`'s
  `8857c32d…`. What stands behind it is `main`'s own next decided run
  [35014513335](https://github.com/opensoft/openxFactory/actions/runs/35014513335)
  at `8944758c` — `92d519b0`'s first-parent child, slice 4's whole surface
  byte-identical between the two trees — and `main`'s newest decided run
  [35026939157](https://github.com/opensoft/openxFactory/actions/runs/35026939157)
  at `b3a75537`, which has all four merges as ancestors
  (`selected=7814 passed=7808 skipped=6 failures=0 errors=0`).
  **THE PROMOTION, MEASURED BYTE-FOR-BYTE:** one `## MODIFIED` and three
  `## ADDED` requirements into
  `openspec/specs/release-realization/spec.md`, **thirty-one scenarios**
  (7 + 8 + 10 + 6), canon `--numstat` **+352 −0** — a PURE INSERTION, which is
  D5's own claim proved rather than restated: *Origin retention at archive*
  carries every promoted sentence and all three promoted scenarios verbatim and
  only adds. Each block is byte-identical between the archived delta and canon:
  *Origin retention at archive* 5,944 bytes sha256 `cf633d606db7f08c…`;
  *A moved packet declares the identity it was ratified under* 6,574 bytes
  `88c1b0352afe3a1c…`; *An undeclared rename arrival is refused at its landing*
  7,901 bytes `1ee95c2e459552b4…`; *A packet reference resolves by identity,
  not by path* 4,879 bytes `24312b4fa514f8db…`.
  **§ 4.5 IS NOT PERFORMED, AND THE PACKET ARCHIVES MEETING ITS OWN DEGRADED
  CASE RATHER THAN EVADING IT.** Registering `former-id-arrival-gate` as a
  required check is Brett Heap's console act; re-read live at the archive,
  2026-09-15T23:35:31Z, `repos/opensoft/openxFactory/rules/branches/main`
  returns three `required_status_checks` rules (21957695, 22551797, 21538893)
  naming it nowhere. He ruled the archive may proceed anyway —
  ...
  [#1053](https://github.com/opensoft/openxFactory/issues/1053), § 6.2 taken
  elsewhere at PR [#1027](https://github.com/opensoft/openxFactory/pull/1027) →
  `04340d77`. **openxFactory
  [#1003](https://github.com/opensoft/openxFactory/issues/1003) WAS ALREADY
  CLOSED** at 2026-09-15T21:24:38Z, eleven seconds after slice 6's merge, on
  his word *"Close when #1041 lands"* — earlier than `tasks.md` § 7.2's own
  first sentence put it, which that box records rather than reconciles.
  **No ordered-delta child depends on this change** (no active packet declares
  it in `sequenced_after:`), and the archive relocation is EXCEPTED BY ID at
  the arrival gate this packet itself built — the first real archive to be
  judged by it.
...
- [extend-prose-tagging-target-to-pinned-capabilities](openspec/changes/archive/2026-09-15-extend-prose-tagging-target-to-pinned-capabilities/proposal.md)
  ...
  **SEPARATE ARCHIVE WORD** — verbatim *"archive it"*, 2026-09-15, in session to
  ...
  and mirrored on
  [#745](https://github.com/opensoft/openxFactory/issues/745#issuecomment-5686413478)
  — given AFTER, and apart from, his *"realize it"*
  ([#992 5662218844](https://github.com/opensoft/openxFactory/issues/992#issuecomment-5662218844))
  and *"merge 1040 when green, then post the records"*
  ([#992 5670841396](https://github.com/opensoft/openxFactory/issues/992#issuecomment-5670841396)).
  **`code_surface` IS NON-EMPTY**, so this archive rests on MERGED-PLUS-GREEN
  REALIZATION EVIDENCE AT CANON'S GRAIN. **THE EVIDENCE, CITED:** realization
  **PR [#1040](https://github.com/opensoft/openxFactory/pull/1040) →
  `74348374bcf999b80ac2e442b3fd3bf98473a20a`** on `main`, 2026-09-14T22:14:26Z
  (D-5's ONE pull request — the fail-closed pinned-target arm, the pure shape
  adapter `scripts/doc_health/pin_shapes.py`, the shared containment helper
  `scripts/pin_containment.py`, tests for § 3.3 (a)–(p) including the two-leg
  equivalence test over the five real pin records, the four markers
  retargeted, the docs section and the INDEX sentence; eight review rounds;
  REALIZED record
  [#992 5671554584](https://github.com/opensoft/openxFactory/issues/992#issuecomment-5671554584)),
  and `main`'s OWN `pytest-suite` run
  ...
  2026-09-14T22:14:29Z to 22:34:47Z — so no tree-equality argument is needed
  and none is made. **THE PROMOTION, MEASURED:** both `## MODIFIED` blocks are
  BYTE-IDENTICAL to canon after the archive — `document-lifecycle` *Prose
  tagging marker hygiene* 40,464 bytes, sha256 `bb106900ac5ce1c7…` (canon
  +447 −1), and `doc-health` *Tag hygiene enforced by reference* 3,686 bytes,
  sha256 `16c290ab796ea56f…` (+36 −4). **NO ORDERED-DELTA CHILD DEPENDS ON THIS
  CHANGE** (searched `openspec/changes/` for a `sequenced_after` naming it;
  none). **ITEM (7) OF `split-openxwallet-repo`'S ENTRY BELOW IS DISCHARGED by
  this change** (annotated there). **THE HONEST OUTCOME STANDS ON THE RECORD:**
  the four `error`-band `tag-hygiene` findings the packet was filed for CHANGED
  FORM — unresolved PINNED targets naming `contracts/openxwallet-pin.yaml` and
  the publisher's remedy — and stay FOUR until openXwallet's pin record
  publishes a `capabilities:` enumeration, the publisher's act, recorded as
  prose and not filed. All § 4 boxes tick on this recording; § 3.7 was ticked at
  `c111fec0`; the origin declaration is retained (`ORIGIN RETAINED`, unchanged
  since the ratifying merge `e6e1c968`, with act 5's tightening accepted by the
  first entry of `openspec/origin-dispositions.yaml`).
...
- [amend-repo-boundary-governance-scope-first-line](openspec/changes/archive/2026-09-11-amend-repo-boundary-governance-scope-first-line/proposal.md)
  ...
  evidence — *"its code_surface is `none` and it archives when its artifacts
  land, as before"*, that capability's realization gate binding only *"A change
  with a **non-empty** code surface"*.
  **THE EVIDENCE, CITED RATHER THAN ASSERTED:** ratified and landed by **PR
  [#937](https://github.com/opensoft/openxFactory/pull/937) →
  `61cee60d85ec53a1107033fe0aea55ffe8987fa0`** on `main`, 2026-09-11T03:43:30Z
  (ratifying commit `54bb36b3`, the last commit to touch `.openspec.yaml`;
  records
  `openspec/changes/archive/2026-09-11-amend-repo-boundary-governance-scope-first-line/review/ratification-2026-09-11.md`
  `Status: ratified` and the gate capture `verification-2026-09-11.md`
  `Status: record`, both MOVED and untouched by this archive), with
  `code_surface: none` and `target_release: implemented`. PR #937's green
  `pytest-suite` run
  [34557680707](https://github.com/opensoft/openxFactory/actions/runs/34557680707)
  on its head `54bb36b3` is named as a courtesy; **no run on the merge commit is
  owed for an empty code surface**. The word's condition — *"when landed"* — was
  ALREADY MET when the word was given, the landing preceding it by some six and
  a half hours.
  Origin: openxFactory
  [#931](https://github.com/opensoft/openxFactory/issues/931), **CLOSED at this
  archive act and not at that landing** — which is what the proposal pull
  request's `refs #931`, carrying no closing keyword in its body or in any
  commit message on its branch, was for; its `closingIssuesReferences` was `[]`
  at the merge.
  **WHAT IS NOW CANON.** The `## MODIFIED` block is promoted BYTE-IDENTICAL into
  `openspec/specs/repo-boundary-governance/spec.md` — **34 lines, 3,404 bytes,
  sha256 `50ced1fa72901a62…`**, extracted programmatically from both sides and
  equal; and that hash is the GENERATOR'S OWN OUTPUT, the string the delta's
  header recorded before the ratification and the bench reviewed, so canon
  carries the produced string and not a transcription of it. It amends *Install
  repository scope*: the first body line now opens *"The following install
  repositories SHALL be scoped to subsystem install, operations, backup,
  restore, upgrade, verification, and disaster recovery:"* where canon opened
  with the enumeration *"`Hermes-Install`, `Omnigent-Install`,
  `Keycloak-Install`, `OpenXPKI-Install`,"* and deferred its verb and its `SHALL`
  to line two. **FOUR WORDS ARE ADDED AND NONE IS REMOVED** — the five
  repository names keep canon's order, canon's spelling and canon's serial
  comma, 215 characters becoming 251 — and **NO BEHAVIOUR MOVES**: the same five
  repositories are scoped to the same seven activities, and the admission
  sentence, the index sentence and all three routing scenarios are canon's own
  bytes.
  **AND THE INHERITED MARKER LEAVES THE PROMOTED FILE HERE, STATED RATHER THAN
  LEFT IN THE DIFF** (`design.md` **D2b**): the `Removed from canon by
  refresh-install-repository-enumerations (2026-09-08):` marker that stood at
  `:66` of this requirement is not restated by the block and so is not written
  back — canon's own *"A marker is NOT a carriage unit, in either direction …
  The durable record of a deletion is the archived delta"*
  (`openspec/specs/doc-health/spec.md:1801-1805`), that durable record being
  `openspec/changes/archive/2026-09-09-refresh-install-repository-enumerations/specs/repo-boundary-governance/spec.md:37`.
  That predecessor's two OTHER markers in this file, at `:32` and `:87`, are
  untouched (three → two), and this packet's own marker takes the retired
  sentence's place. `git diff -w --numstat` equals `git diff --numstat` at 4/3,
  so **ZERO lines changed by whitespace only**; spec count **62 → 62**, so no
  codexFactory floor advance is owed.
  **AND THE FAILURE #931 REPORTS IS CLEARED HERE, MEASURED ON BOTH SIDES RATHER
  THAN PREDICTED:** `openspec validate repo-boundary-governance --strict --type
  spec` on the 1.2.0 binary goes from *"Specification 'repo-boundary-governance'
  has issues"* / `✗ [ERROR] requirements.1.text: Requirement must contain SHALL
  or MUST keyword` / exit **1** on `main` `22efcbe8` to ***"Specification
  'repo-boundary-governance' is valid"*** / exit **0** here, and `--all
  --strict` drops `spec/repo-boundary-governance` from its failure set (`99
  passed, 3 failed (102 items)` → `99 passed, 2 failed (101 items)`; that
  removed line is the only difference between the two sorted failure sets). On
  the PINNED **1.12.0** — the binary the required gate runs, which never
  reported this at all — the failure set is UNCHANGED either side, the two
  entries being the dispositioned exceptions `tasks.md` § 6.5 names.
  **THE RESIDUES ARE NAMED, NOT DISCHARGED.** `tasks.md` § 6.1's successor is
  filed UNCLAIMED as
  [#955](https://github.com/opensoft/openxFactory/issues/955) — a later
  amendment of a requirement whose promoted text already carries a UNIT-NAMING
  marker has exactly two options, drop it or accept a ground-three
  marker-defect finding, with no third today FOR THAT FORM and the PAIRING form
  deliberately not reached — and § 6.8's as
  [#956](https://github.com/opensoft/openxFactory/issues/956) — `target_release:
  none` is outside `release-realization`'s ratified vocabulary, carried by 33 of
...
- [add-release-tag-gate](openspec/changes/archive/2026-09-05-add-release-tag-gate/proposal.md)
  ...
  *"ruleset updated, do the evidence and archive it"*, given after his
  ratification the same day — *"ratify 668, land it when green"*. Proposed
  2026-09-04 and **ADMITTED** the same day on *"do your recommendation"*
  (openxFactory issue **#664**, option 2 of three) — origin metadata only, not a
  ratification; **RATIFIED 2026-09-05** (records
  `review/ratification-2026-09-05.md` and `review/verification-2026-09-05.md`,
  archived with the packet), **REALIZED by PR #668**, merge **`7ee0e73d`**
  (2026-09-05T02:13:55Z), and **GATED** by the `[OPERATOR]` act recorded at
  `review/realization-evidence-2026-09-05.md`.
  **THE RELEASE-TAG ZERO-FINDINGS PIN LEFT THE REQUIRED SUITE FOR A GATE ON THE
  CUTTING PULL REQUEST.** `tests/doc-health/test_release_tag_publication.py`
  asserted, inside the required `pytest-suite`, that this repository reads ZERO
  `release-tag-publication` findings — but a new bundle's annotated tag is
  published AFTER the cutting pull request merges, at the merge commit, by a
  second actor (measured 6s/23s/44s/**69s** after the merge commit for
  `contract-v3.1` through `contract-v3.4`), so the legitimate window between the
  two acts reddened EVERY open pull request: PR #628 declared `contract-v3.3`
  untagged at 22:27Z on 2026-09-03 and every lane failed on that one test for
  five and a half hours. **ONE requirement MODIFIED** (`doc-health` §
  *Release-tag publication*, restated in full with all 24 promoted scenarios and
  adding six; it drops no canon unit, so no `Removed from canon by` marker was
  owed) and **NO new file under `openspec/specs/`**, so **no codexFactory floor
  advance** — the promoted-capability count is unchanged at 59.
  `.github/workflows/release-tag-gate.yml` runs on EVERY pull request (**no
  `paths:` filter**, so the required context always reports; **not on
  `push: main`**, which would re-create the same red) and
  `scripts/validate-release-tag-gate.py` short-circuits green unless the pull
  request touches `contracts/manifest.yaml` or `contracts/releases/**`,
  otherwise running THE SAME FAMILY over the merge tree by overriding one seam
  method — **one clock, not two**. The bar is the one the pin held, *no `error`,
  no `warning`*, **moved and not weakened**. **D1 and D2 were put forward as
  veto points and NEITHER was vetoed:** a cutting pull request is NOT required
  to carry its own tag (it cannot yet; the family's own *declaring commit is
  still the published tip* scenario already answers it) and the obligation is
  RECORDED instead; `gate-version-reuse` stays, scoped to the moved-declaration
  case. **REQUIRED ON `main` BY RULESET `21538893`** ("openxFactory wallet-gate",
  `updated_at` 2026-09-05T04:36:49Z), which now carries four contexts —
  `wallet-validation`, `pytest-suite`, `lane-line`, `release-tag-gate` — with
  `release-tag-gate` also listed by `rules/branches/main`; **no new ruleset was
  created**, the context joining the one already carrying `pytest-suite`. First
  green run under the required regime: **`33945259576`** on the archiving pull
  request **#672**, `isRequired: true`, `SUCCESS`, short-circuiting. **STILL
  OWED, and named rather than closed:** the accepted `release-inventory-drift`
  `error` on `docs/contract-versioning-policy.md` — a digested member of the
  published `contract-v3.4` bundle — persists until the next contract cut
  re-digests it, and the first pull request the gate judges FOR REAL is the next
  one touching the release surface (PR #653 is not it).
...
- [split-openxwallet-repo](openspec/changes/archive/2026-08-28-split-openxwallet-repo/proposal.md)
  ...
  **Successors registered**, named here in one place because the README is the
  ...
  corpus** — **DISCHARGED 2026-09-15 by
  `extend-prose-tagging-target-to-pinned-capabilities`** (ratified by PR #1019 →
  `e6e1c968`, realized by PR #1040 → `74348374`, archived above): the four
  markers now carry `target=pinned:openxwallet/openxwallet`, the TRUE form, and
  their four findings changed form and stay four until openXwallet's pin record
  publishes a `capabilities:` enumeration — the publisher's act, not this
  repository's. Its origin, kept as written: added at the archive itself,
  because the act produced it: deleting
