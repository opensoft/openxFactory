...
- **TRANCHE ONE RAISED 2026-08-29** as the OpenSpec change
  `add-signed-execution-chain`, carrying `Status: draft`. It ADDS the neutral
  capability `signed-execution-chain` with ten requirements covering links 1-3
  — the ratification as a verified, key-attributed grant exercise; ratification
  and CHAIN ENROLLMENT as one signed act; the chain identity as the digest of
  the signed ratification — plus the hash-linking rule and the off-chain
  evidence plane, which authoring showed links 1-3 need in order to be
  stateable at all (a refinement offered to Q4, not a closing of it). Two
  compositions were CORRECTED against ratified text rather than carried from
  this fragment's shorthand: link 1 is an EXERCISE with verified proof of
...
- **NOTHING MOVED OUT OF STAGING, AND TRANCHES TWO AND THREE ARE NOT RAISED.**
  A partial promotion of ZERO files: both documents stay here because both are
  load-bearing for the later tranches. The primary fragment gained only the
  `Staging ID:` header the origin contract requires at the proposal
  transition; `chain-selection-study.md` was not edited at all. **Q6 (the PHI
  reading, gating tranche three) and Q7 (where an attestation signature
  physically happens, gating tranche two's contract text) remain OPEN and
  untouched**, reserved for Brett's clarify sitting, and the raised packet
  carries a repeatable check that no tranche-one requirement changes meaning
  under either answer.
