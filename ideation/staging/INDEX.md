...
- Open questions (none blocking, all `open`): (1) may an openxWallet grant
  ...
  answer is narrower than the question expects: `scope` is CLOSED
  (`additionalProperties: false`, required `acts` + `authority_tier`) with
  nowhere to write a provider or an external role, and `audience` requires a
  `wallet_ref`, so an ordinary human grantee cannot be the audience.
