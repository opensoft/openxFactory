...
- The PHI architecture is the part that must not be hand-waved: **raw PHI never
  ...
  off-chain custody with patient-held keys; the chain carries commitments, consent
  records and their revocations, and the execution anchors. Hospitals and insurers
...
- A conflict inside the vision itself, recorded rather than smoothed: a broken
  ...
  tranche three.
- 6 open questions, none blocking: wallet-presentation mechanics; the exact
  ...
  ruling rather than applying it, raised for Brett rather than assumed.
...
- Exit path: OpenSpec change(s) in three tranches. **Tranche 1 — signed
  ...
  what keeps the anchor choice reversible over a 10-year horizon.
...
- Readiness: RATIFIED 2026-08-26 as `split-openxwallet-repo` (PR
  ...
  repository, Q5 is `wallet-vN.M` with no range in the pin). Realization
  proceeds through Speckit features, one per `tasks.md` group in the design's
  Migration Plan order — OpenSpec ratified the boundary and Speckit builds it —
  starting with the change's own remaining packet bookkeeping (Amendment 2 to
  `docs/openxdox-naming.md`, the xFactory working-rule #1 amendment) and P5a.1,
  LedgerxFactory's three-candidate finder, which lands before the carve
