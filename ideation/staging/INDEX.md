...
| [derived-third-party-model](#derived-third-party-model) | ADDED `derived-third-party-model` (neutral template schema + invariant vocabulary + validator rules); DTN-014 | 1 | Ready to iterate — pattern named 2026-07-23 with three domain proofs (Medx ratified templates conform as-is; Adx + Ledgerx staged topics); exit gated on one of the Adx/Ledgerx object-model changes landing |
| [codexfactory-domain-hermes-content](#codexfactory-domain-hermes-content) | codexFactory `hermes/domain/` content (changes A + B) + Omnigent overlay extension in lockstep | 1 | Change A COMPLETE — ratified + archived 2026-07-22 (`archive/2026-07-22-add-domain-hermes-roles-and-policies`; canonical spec `domain-hermes-content`). Change B AUTHOR-READY — all 4 gating decisions made 2026-07-22 (client seat CPL, project seat intent-owner symbolic, council_small drawn-by-trigger, spend value client-owned + seeded finding_class vocab); drafted in the topic's `openspec/` (proposal-b/tasks-b); Change B RAISED + IMPLEMENTED 2026-07-23 (`add-domain-hermes-councils-and-memory` on codexFactory main 0939dc9: mixes, both councils, escalation, memory boundaries, catalog; strict + validate-docs green) — pending ratification/archive; topic completes when B archives |
...
- Repository context: openxFactory (neutral template schema + invariant
  vocabulary + validator rules); proof domains MedxFactory (ratified
  `templates/{dream-object,simulation-scenario}.yaml`), AdxFactory
  (`ideation/staging/adx-persona-simulation/`), LedgerxFactory
...
- Source: cross-domain modeling session with Brett Heap 2026-07-23 — the
  ...
  health, with the Medx dream-object precedent recognized as the
  original instance. Registered as DTN-014.
- Claim: three domains independently converged on the same object shape —
  a governed model of a third party the subject cares about but who holds
  no stack authority — with six shared invariants (non-authoritative by
  construction, per-fact provenance + assumption register, read-only vs
  the truth store with zero action authority, per-subject isolation,
  human-gated promotion, calibration-derived confidence written only by a
  designated workflow) and exactly four domain dials (third-party
  identity synthetic↔real, truth store Hermes-memory↔external-enforcement,
  calibration source, promoting authority). Promote the invariant
  vocabulary + a conformance declaration (`conforms_to`) + validator
  rules; domains keep their own templates.
- Files:
  - [derived-third-party-model.md](derived-third-party-model/derived-third-party-model.md) — primary: the pattern, dial table, 4 claims, open questions, exit.
- Open questions: delta shape (new capability vs `xfactory-domain-factory-model`
  guidance + schema); scenario object as own conformance kind or profile;
  naming (dream object models the subject themself, not a third party);
  whether person-modeling constraints are a neutral dial or domain policy.
- Exit: one openxFactory change (`code_surface: openxFactory`); archives
  when ≥2 domains declare conformance and validate green — Medx
