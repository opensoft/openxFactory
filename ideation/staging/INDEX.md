...
- Open questions: **two await Brett** — Q6 sequencing against the five active
  `ideation-dashboard` changes (recommend four preparatory slices now, and
  require each active change archived, re-homed or waived before the carve; the
  one genuine conflict is `add-nightly-dashboard-refresh` adding 7 `doc-health`
  requirements for the dashboard while Q4 separates them, which needs a decision
  rather than a merge), and Q7 repository ownership, license and visibility plus
  what Amendment 3 must state (recommend both repos under `opensoft`, do NOT
  pursue the `opendox` org, decide license and visibility AT CREATION — and note
  that Q5's "a student could use openDox" gives visibility a product answer, not
  only a legal one: a student cannot use a private repository). **Four secondary
  questions travel into the change**: Q8 REMOVED-with-a-per-requirement-successor
  -map across three destinations rather than a stub; Q9 a three-part behavioural
  floor in place of byte identity (test-count arithmetic summing to the
  pre-split count, a neutral conformance corpus, a snapshot-equivalence run);
  Q10 whether a consumer may pin openDox directly (recommend both, with the layer
  declared — otherwise Q5's standalone test is unsatisfiable); Q11 whether the
  lifecycle vocabulary is parameterized (recommend yes: openXdox carries the
  engine, each descendant declares its own words, openxFactory's nine become
  `codexDox`'s declaration) — Q11 is the one whose answer most changes what gets
  built.
- Exit: one OpenSpec change on the `split-openxwallet-repo` shape adapted for a
  two-repository, non-byte-identical extraction of an APPLICATION with a third
  descendant layer, declaring a code surface across at least six repositories so
  it archives only on merged plus green realization evidence. **Four preparatory
  slices do NOT depend on the proposal and are worth taking first**: land Q4's
  one-way dependency and its small neutral module; name the corpus adapter's
  operation signatures; harden the apply lane (Q1 promotes a path with ONE
  dispatch in its history to the only governed write path); and work the three
  domain mappings out far enough to extract the common core — the direction's own
  homework. What must be true before the proposal: Brett answers Q6 and Q7, with
  Q7 needed before either repository is CREATED rather than before the proposal
  is written. Q6's active-change wave must be resolved, re-homed or waived by a
  recorded ruling before the carve phase, and the unpromoted
  `ideation-intent-plane` capability promoted or its non-promotion recorded
  before the per-requirement map is authored.
