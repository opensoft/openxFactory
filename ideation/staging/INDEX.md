...
| [qualify-avatar-live-voice](#qualify-avatar-live-voice) | ADDED `avatar-live-voice` (incl. the reserved AVC-09/AVC-10 contracts) | 2 | Blocked — 5 open questions (credential custody + spend cap and activation-gate scope hardest); also gated on a released client from the lab. PO decision memo staged 2026-07-16 |
...
- Files:
  ...
  - [fork-decision-memo.md](qualify-avatar-live-voice/fork-decision-memo.md) — PO (Brett) decision memo for the five blocking forks: per-fork source constraints, options + tradeoffs, labelled recommendations (not rulings), unblocks/forecloses, and named gaps → assumptions to ratify; plus a "Rulings requested" table, latent decisions surfaced (repo-locus, pilot-hardening carrier, production-default status), and ruling mechanics.
- Open questions (blocking): credential custody + spend cap; latency-budget derivation; the activation-gate scope; data-control/consent for evaluation audio; canary/rollback shape. Also gated on a released, code-signed client from the lab. The fork-decision-memo records per-fork recommendations for Brett to rule on (unblocks the topic's Exit when all five are ruled).
