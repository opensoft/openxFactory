...
| [codexfactory-domain-hermes-content](#codexfactory-domain-hermes-content) | codexFactory `hermes/domain/` content (changes A + B) + Omnigent overlay extension in lockstep | 1 | Change A COMPLETE — ratified + archived 2026-07-22 (`archive/2026-07-22-add-domain-hermes-roles-and-policies`; canonical spec `domain-hermes-content`). Change B AUTHOR-READY — all 4 gating decisions made 2026-07-22 (client seat CPL, project seat intent-owner symbolic, council_small drawn-by-trigger, spend value client-owned + seeded finding_class vocab); drafted in the topic's `openspec/` (proposal-b/tasks-b); **gate CLEARED 2026-07-23** — the omnigent overlay realization landed in codexFactory (archetype-shaped, change A's directed_by + both new workers preserved); change B is ready to raise |
