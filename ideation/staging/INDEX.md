...
- Exit: three OpenSpec changes in dependency order — (1) neutral
  ...
  under `repo-boundary-governance`. Exits 1 and 2 may be authored in parallel,
  but the neutral contract ratifies first. EXIT 1 EXECUTED — proposed and ratified
  2026-08-21 as `add-identity-brokering`; its repo admission landed as an
  ADDED per-repo requirement on the avatar-client template (naming successor
  `implement-keycloak-install-repo`) rather than the shared MODIFIED
  enumeration delta, so the sibling `add-trust-anchor` cannot collide with it
  at archive time.
...
- Exit: three OpenSpec changes — (1) neutral `trust-anchor` capability in
  ...
  under `repo-boundary-governance`, moving the QA deployment topology
  manifests in per R2. EXIT 1 EXECUTED — proposed and ratified 2026-08-21 as
  `add-trust-anchor`; its repo admission landed as an ADDED per-repo
  requirement on the avatar-client template (naming successor
  `implement-openxpki-install-repo`) rather than the shared MODIFIED
  enumeration delta, mirroring the sibling's collision avoidance. The
  `add-openxpki-qa-image-pipeline` amendment rides that change's tasks §2 as
  the time-critical item.
