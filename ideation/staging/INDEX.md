...
- Files:
  - [notebook-access-wallet-governance.md](notebook-access-wallet-governance/notebook-access-wallet-governance.md)
    ...
    resolving), 4 idea notes, 3 conflicts, 6 open questions each with
    Context/Recommended answer/Explanation/Disposition status, exit.
- Open questions (none blocking, all `open`): (1) may an openxWallet grant
  scope an EXTERNAL provider's resource — its `scope` is open by construction
  so such a grant would validate today, but the capability's eighth promoted
  requirement bounds it as "an authority control, never an identity
  substrate"; recommended answer keeps the wallet holding the AUTHORITY TO
  GRANT while the provider's ACL remains the access; (2) whether the ratified
...
- Exit: iterate until all six questions carry a disposition other than `open`,
