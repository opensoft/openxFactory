...
- **Q2 is RULED.** The ratified prose defines ASSEMBLY as "the code that
  assembles a review team, shared across the family" (`proposal.md:608`), which
  reads as ONE shared repository, while an electing project needs a
  PER-PROJECT root to clone. Brett Heap ruled, 2026-09-02: "yes, assembly is per
  project." The ASSEMBLY leg IS the per-project root repository the engineer
...
- Exit: TWO acts, and the first is HALF DONE. FIRST, openRepoShape is created and
  ...
  repository topics, any regenerated template repository). Gate: Q2 is RULED
  (Brett Heap, 2026-09-02); a digested openRepoShape release is still needed
  before the change can pin it.
