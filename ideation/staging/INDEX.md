...
| [qualify-avatar-live-voice](#qualify-avatar-live-voice) | ADDED `avatar-live-voice` (incl. the reserved AVC-09/AVC-10 contracts) | 2 | **UNBLOCKED 2026-08-26 — all five forks AND all three latent decisions RULED by Brett in session**, recorded in the staged `fork-decision-memo.md` (staged 2026-07-16): Fork 1 credential custody + spend cap = **Option C** layered hard-closed (broker session hard-kill on the existing duration/quota terminal + kill switch + lease revocation; the dedicated spend-capped internal-live OpenAI project is the hard per-tenant/ring stop; async Usage Meter + alerting for visibility; durable synchronous per-tenant counter deferred); Fork 2 latency budgets = **Option C** two-tier with the materiality threshold **RATIFIED AS PROPOSED** (>15% relative OR >150 ms absolute, whichever greater, on first-playable-after-authorized and sideband-ready; hard gate p50+p95 on Windows desktop + web canvas at nominal network; per-profile numbers in AVC-10/AVC-09 evidence; p99 + teardown recorded, not gated); Fork 3 activation-gate scope = **Option C** (the kernel's four-element ring is the binding exit contract, conditions 1-5/7/8 reused as the mapped preflight+canary checklist, condition 6 reinterpreted as no-material-regression vs the direct-provider reference with the number owned by Fork 2, all replace-the-primary semantics reserved to the GPT-Live adoption change); Fork 4 consent + evaluation audio = **Option C** (synthetic evaluation corpus; opt-in canary may carry real consented users on the single candidate with strictly ephemeral processing — `ephemeral_presentation` + `structured_record` only, nothing retained, nothing shadowed; retained-real corpus + retention-class unreservation deferred); Fork 5 canary + rollback = **Option B** (vendor-org + one internally-staffed domain sandbox on synthetic/internally-consented audio only; written revoke-vs-block policy — safety/integrity breaches auto-abort WITH active-lease revocation, latency-budget breaches auto-block-new letting legs drain, quality/cost operator-triggered; rollback target disable-voice → text/handoff). Latent decisions: (1) this topic OWNS the `xfactory-avatar-client` extraction from codexFactory `apps/avatar-client-lab`, matching the Exit's `code_surface` (adding the repo to the aggregation stays a separate reviewed change); (2) CONFIRMED `avatar-pilot-hardening` is the carrier of the Fork 1 durable-counter and Fork 4 retention-unreservation deferrals; (3) CONFIRMED internal-live qualification does NOT auto-promote `gpt-realtime-2.1` to production default primary. Ruling on each option also ratifies that option's named-gap assumptions per fork. **The Exit is now UNBLOCKED** — the `qualify-avatar-live-voice` change is to be created carrying these eight rulings as locked decisions; the released, code-signed client from the lab remains a realization-evidence gate, not a proposal gate |
...
- Files:
  ...
  - [fork-decision-memo.md](qualify-avatar-live-voice/fork-decision-memo.md) — PO (Brett) decision memo for the five blocking forks: per-fork source constraints, options + tradeoffs, labelled recommendations, unblocks/forecloses, and named gaps → assumptions to ratify; plus the "Rulings requested" table, latent decisions surfaced (repo-locus, pilot-hardening carrier, production-default status), and ruling mechanics. **NOW CARRIES THE RULINGS** — all five forks and all three latent decisions ruled by Brett in session 2026-08-26 and written into the table and the latent-decision items. The memo stays `Status: staged`: a ruling here is a staging decision, not an OpenSpec ratification, so no promotion is claimed.
- Open questions (blocking): **ALL FIVE RULED 2026-08-26 (Brett, in-session), none open** — credential custody + spend cap = Option C (layered hard-closed: broker session hard-kill on the existing duration/quota terminal + kill switch + lease revocation; the dedicated spend-capped internal-live OpenAI project as the hard per-tenant/ring stop; async Usage Meter + alerting; durable synchronous per-tenant counter deferred to pilot-hardening); latency-budget derivation = Option C two-tier, with the materiality threshold RATIFIED AS PROPOSED (>15% relative OR >150 ms absolute, whichever is greater, on first-playable-after-authorized and sideband-ready; hard gate p50+p95 on Windows desktop + web canvas at nominal network; per-profile numbers recorded in AVC-10/AVC-09 evidence; p99 + teardown recorded, not gated); activation-gate scope = Option C (four-element ring binding, conditions 1-5/7/8 as the mapped preflight+canary checklist, condition 6 reinterpreted as no-material-regression vs the direct-provider reference, replace-the-primary semantics reserved to the GPT-Live adoption change); data-control/consent for evaluation audio = Option C (synthetic evaluation corpus; opt-in canary may carry real consented users on the single candidate under strictly ephemeral processing, `ephemeral_presentation` + `structured_record` only, nothing retained and nothing shadowed; retained-real corpus + retention-class unreservation deferred to pilot-hardening); canary/rollback shape = Option B (vendor-org + one internally-staffed domain sandbox on synthetic/internally-consented audio only; written revoke-vs-block policy with safety/integrity auto-abort WITH active-lease revocation, latency-budget auto-block-new letting legs drain, quality/cost operator-triggered; rollback target disable-voice → text/handoff). Each option ruling also ratifies that option's named-gap assumptions as recorded in the memo's per-fork lists. Three latent decisions ruled the same day: this topic OWNS the `xfactory-avatar-client` extraction from codexFactory `apps/avatar-client-lab` (aggregation registration stays a separate reviewed change); `avatar-pilot-hardening` CONFIRMED as the carrier of the durable-counter and retention-unreservation deferrals; internal-live qualification CONFIRMED as NOT auto-promoting `gpt-realtime-2.1` to production default primary. The released, code-signed client from the lab remains a realization-evidence gate, not a proposal gate.
- Exit: **UNBLOCKED 2026-08-26** — create `qualify-avatar-live-voice` (`code_surface: openxFactory, xfactory-avatar-client`) carrying the eight rulings above as locked decisions, moving this folder's staged files into the change's `supporting-docs/`; archives only on merged + green internal-live realization evidence, never on landing alone.
