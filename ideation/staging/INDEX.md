...
| [layer-content-materialization](#layer-content-materialization) | ADDED neutral `hermes_domain_overlay` contract + `overlay_path` (openxFactory); hermes-install seeding increment 2 (`layer_content` kernel + materialization) | 1 | First exit COMPLETE (`contract-v1.15`, archived 2026-07-23). Second exit IMPLEMENTED 2026-07-23: feature 003 complete on branch `003-layer-content-materialization`, **PR #6 open** (hermes-install) — all gates green (236 unit + 53 pg incl. 7 new materialization tests; parity vs the canonical validator; live codexFactory overlay passes). Merge + ratify + archive next |
