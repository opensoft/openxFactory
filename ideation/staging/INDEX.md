...
| [project-repo-schema](#project-repo-schema) | ADDED a neutral `project-repo-schema` capability in openxFactory (the doctrine, the repository-naming convention, the assembly-root submodule shape and its double pin, the one-command bootstrap contract, and the lockstep-validator obligation) + MODIFIED `ideation-dashboard` (the project register gains an optional per-project `schema` election, a per-repository `role` of spec/code/assembly, and a manifest-derivation rule — all still conferring nothing) | 1 | Registered 2026-09-02 from Brett Heap's four-part questioning and his ruling "stage the project-schema topic too". **The ERGONOMICS half only** — the sibling topic `wallet-carried-work-authority`, staged the SAME DAY and landed on `main` first (PR #585), carries every authority question, and this topic defers to it by name. **The forcing finding: the recommendation has NO SPEC TEXT.** `add-wallet-carried-review-authority` (ratified 2026-08-23) recommends a SPEC/CODE/ASSEMBLY shape at `proposal.md:600-617` and rules that "Electing the schema changes no gate, no floor, no grant, and no clearance eligibility" — but that change's own `specs/` deltas are exactly two (`review-authority-intake`, `roles-authority-model`) and repository-wide greps for "recommended project schema", "three-repositor" and the governance sense of "ASSEMBLY" resolve only to that one proposal plus its `tasks.md:811-814`. The delta is therefore **ADDED, not MODIFIED** — a PA who elects the schema today has nothing to elect against. **TWO RULINGS LANDED THE SAME DAY, and one act was performed**: Brett ruled "name it openRepoShape and make it public", then "apache 2.0, create it", and `opensoft/openRepoShape` NOW EXISTS (https://github.com/opensoft/openRepoShape — PUBLIC, Apache-2.0, default branch `main`, initial commit `228dd9f`, README seeded `65880cb`; the standard's CONTENT — policy file, templates, scaffold, bootstrap, `AGENTS.md` — is NOT in it yet, and `65880cb` is the earliest pinnable ref). That settles Q9's name, visibility and licence and performs the first half of the exit path. Bookkeeping recorded in the fragment: the seed push to `main` bypassed the org ruleset "Changes must be made through a pull request" under the operator's bypass right, so the repo inherits that ruleset and later content lands by PR. **TWELVE open questions, ONE of them blocking**: Q2 — the ratified prose defines ASSEMBLY as "the code that assembles a review team, shared across the family", which reads as ONE shared repository while an engineer needs a PER-PROJECT root to clone; the corpus has no third mention, and **the exit change cannot be proposed until Brett rules it**. Recommended: the per-project root IS the assembly leg and PINS the shared review-team code at a sha, exactly as the aggregation already consumes `review-lane-reusable.yml@dc21767…`. The other eleven carry recommendations and reasoning: leg naming `<Project>` / `<Project>-spec` / `<Project>-code` with a shape-based disambiguation from `open<Product>`, `<Domainx><Product>` and `<X>-Install` (Q1); submodules over a manifest because the atomic pin is the only object a merge gate can check (Q3); detached HEAD paid by bootstrap-driven tracking branches rather than `.gitmodules` `branch=`/`update=merge`, which would make the pin advisory (Q4); the assembly root's own `project.yaml` as the SOURCE with the register row DERIVED from it, since a new org has no register (Q5); a four-way ownership split — openRepoShape mechanics, openxFactory doctrine + pin + register delta, codexFactory engineering overlay, OpsxFactory org naming and topics (Q6); the lockstep validator shipped in the template asserting gitlink == pin-file commit == workflow ref, which is the aggregation's own 2026-08-25/26 defect (seven pin-syncs, `validate` red on every PR for a day, xFactory #146) written down as machinery (Q7); a schema-neutral bootstrap so a one-repository project is not second-class (Q8); a pilot permitted to start against the STAGED fragment provided the reference is recorded, on INDEX.md's own draft-proposal-convention precedent (Q10); a concrete NEW-ORG flow — fork openRepoShape (fork, not template, so upstream drift stays comparable), run `scaffold-project.py`, clone with `--recurse-submodules`, `make bootstrap` (Q11); and neutral-only CI with the codexFactory review lane and wallet authority as OPT-IN OVERLAYS that attach to an existing shape, never the reverse (Q12). Six conflicts recorded and NOT resolved, including the co-residence preference the ratified D1 leaned on (unchanged — the schema stays elective) and the narrowing of "codexFactory may recommend — and scaffold" to an overlay now that the neutral scaffold lives in openRepoShape |
...
- **RETIRED FROM STAGING 2026-08-31 — the exit was TAKEN and the change is
  ARCHIVED** at `openspec/changes/archive/2026-08-31-add-notebook-projection-identity`.
  The exit above described one combined change; that is exactly what ran.
  Both MODIFIED deltas are PROMOTED into canon —
  `lifecycle-notebook-projection` (+3 added, ~2 modified) and
  `credential-contracts` (~1 modified) — measured rather than assumed: 6 delta
  requirements in, 6 out, **all six bodies byte-identical**, 32 delta scenarios
  carried. Ratified 2026-08-23, realized at squash `40b33845` (PR #277), the
  migration run 2026-08-24 (PR #289: 7 books, 626 managed sources), parity
  PROVEN against the corpus scan under the amended
  `add-projection-title-uniqueness` rule 2026-08-25, and the seven legacy books
  retired BY RENAME 2026-08-26 with nothing deleted.
  **TWO THINGS THE ARCHIVE DID NOT CLOSE, named here rather than left in the
  packet alone:**
  - **§ 4.7 was RE-SCOPED, not completed.** Brett Heap ruled 2026-08-31 (in
    session, by explicit multi-choice) that the **Gmail grantee is DROPPED and
    the second reader is not wanted**, so the task's satisfying set is the seven
    verified `brett.heap@farheap.com` editor grants — **7 of 7**. The ruling is
    consistent with two prior ones rather than new: the 2026-08-27 wind-down of
    the legacy books **by owner deletion** and the 2026-08-24 **sole-grantor**
    posture (RESTRICTED, org-visible rejected). The provider's account-level
    refusal of `brett.heap@gmail.com` — both spellings, `API error (code 7)`,
    isolated against the farheap address succeeding on all seven in the same
    session — stands as a **finding**, and nobody is owed the act it blocked.
  - **§ 5.1 archives STANDING AS A DISPOSITION, deliberately unticked.**
    Unattended re-authentication does not exist: Google's sign-in for
    `xFactor001@opensoft.one` is interactive, `nlm login` is broken UPSTREAM by
    the notebook.google.com rebrand (the CLI's `_is_notebooklm_url()` allow-list
    excludes the new host, so `is_logged_in()` reports false for a browser that
    IS signed in), and `add-notebook-hosting-credential-custody` states in
    ratified text that custody **does not deliver automation**. Carried to the
    named successor **opensoft/openxFactory#537**;
    `docs/notebooklm-sync-open-item.md` stays OPEN and points there. § 4.3's
    code gap (`ensure_workspace_record()` returns without registering a
    replacement `provider_notebook_id`) is filed at
    **opensoft/openxFactory#536**.
  **The topic folder `ideation/staging/notebook-projection-identity/` is
  RETAINED as provenance, not deleted** — unlike
  `substantive-review-lane-questions`, which was named-not-linked because its
...
- **State: FULLY RULED, DRAFTING GREEN-LIT, and the packet is RATIFIED.** Brett
  ...
  the packet the same day** (record:
  `openspec/changes/add-signed-execution-chain/review/ratification-2026-08-29.md`).
  Ratification authorizes tranche one only and **performs no realization** —
  that sentence is about the RATIFYING ACT and it stands. **THE LATER COMMISSION
  IT NAMED HAS SINCE BEEN CARRIED OUT**, in its own pull request and by its own
  act: `contracts/signed-execution-chain/` (the four record kinds plus the one
...
- Repository context: SPLIT ACROSS TWO HOMES, which is the sequencing problem.
  `opensoft/openXwallet` owns the grant schema, the custody registry, the
  distinct-holder constraint schema and `scripts/validate-openxwallet.py` — the
  object grammar and the act readers are ITS deltas and cannot be authored here.
  `openxFactory` owns the seam: `governance/review-authority/`, the consuming
  capability `work-authority-intake` and its required check, and
  `contracts/openxwallet-pin.yaml`, which consumes openXwallet at a commit plus
  eight per-file `sha256`s under `neutral-product-pin` (tag-only refused; the
  `contract_bundle_tag: wallet-v1.3` line is a label beside the commit, never the
  referent).
- Source: Brett Heap's ruling of 2026-09-02, in session with Claude. Two coherent
  options were put to him — (1) keep grants repo-granular and make the three-repo
  schema the default wherever spec and code authority diverge, which requires
  revisiting co-residence; (2) keep co-residence and extend the grant schema with
  path-scoped objects plus `author` and `merge` acts, then let the staged
  `signed-execution-chain` topic carry the enrollment-to-merge gate. **He chose
  option 2 and said "stage it as a topic".** Reasoning recorded by the session:
  option 2 matches the ratified doctrine that layout confers nothing; option 1
  quietly re-imports layout as an authority carrier. Origin context is the
  ratified `add-wallet-carried-review-authority` — its "recommended project
  schema" section and its design D1 — plus the live grant and register under
  `governance/review-authority/`.
- Files: 1 — `wallet-carried-work-authority.md` (the primary fragment; conforms
  to the staged-topic outline template, with the three required sections, seven
  open questions each carrying Context / Recommended answer / Explanation /
  Disposition status, and `xspec:candidate` fences on Why / What changes /
  Impact targeting `roles-authority-model`, `review-authority-intake` and
  `neutral-product-pin`. `openxwallet` and `work-authority-intake` are
  deliberately NOT fenced, on `openxwallet-neutral-home`'s recorded precedent:
  neither resolves as a capability in this repository, so fencing them would emit
  tag-hygiene unresolved-target findings).
- The claim that makes it worth raising: the ratified demotion of the
  SPEC/CODE/ASSEMBLY three-repository schema — adoption is a human's per-project
  decision and "changes no gate, no floor, no grant, and no clearance
  eligibility" — is TRUE IN THE PROSE AND FALSE IN THE MACHINERY. Grant objects
  are repository-granular in practice, the schema has no path or branch
  narrowing, and `review` is the only act with a reader, so the only way to give
  one holder spec authority and another code authority over the same project is
  to put spec and code in different repositories. The schema that confers nothing
  is therefore the sole carrier of a distinction the doctrine calls essential,
  and a project that declines it cannot express that distinction at all. The
  topic fixes this on the AUTHORITY side — path-prefix objects, `author` and
  `merge` readers, an openxFactory intake — rather than on the layout side, which
  is exactly the difference between the two options Brett ruled on.
- Exit: two sequenced OpenSpec changes, in this order —
  `extend-openxwallet-object-scope` in openXwallet (object grammar, prefix
  containment in the attenuation rule, the `author` and `merge` readers, negative
  fixtures for a widening prefix / a glob / a negation, and a contract release),
  then `add-wallet-carried-work-authority` in openxFactory (the capability, its
  register, its reader, its required check, the exercise record, the
  drift-checked projection into branch protection, and the digest-pin bump as its
  own first task). What must be true before the FIRST is proposed: the
  predecessor's S3 exercise recording and S5 revocation lifecycle have landed, or
  the first change is explicitly lockstepped to them with the dependency declared
...
- Repository context: FOUR homes, and one of them was created on the day the
  topic was staged. `opensoft/openRepoShape`
  (https://github.com/opensoft/openRepoShape — PUBLIC, Apache-2.0, created
  2026-09-02, seeded at `65880cb`) owns the MECHANICS: the repository-naming
  policy data, the assembly-root templates, `scaffold-project.py`, `bootstrap`,
  the lockstep pin validator and an agent-facing instruction file. openxFactory
  ratifies the DOCTRINE (`docs/project-repo-schema.md`), pins openRepoShape under
  `neutral-product-pin`, and carries the register-election delta. codexFactory
  realizes an ENGINEERING OVERLAY on the neutral scaffold — review-lane caller,
  ...
  enforcement, repository topics, and any regenerated GitHub template repository,
  because GitHub administration was located there by the two exit changes of the
  superseded `github-administration-plane` topic. The aggregation layer keeps the
  `project-register.yaml` instance it already owns.
- Source: Brett Heap in session 2026-09-02, in four parts, quoted verbatim in the
  fragment. He asked why GitHub has no folder to group repositories and whether
  spec and code should be submodules of the assembly; asked where the convention
  and scaffold should be documented given that a project might start "before
  codex is ready to support all this or even outside codex"; described the
  new-org flow he wants ("fork the 'codeXfactory-repo-shape' repo from opensoft
  and then tell my ai to scaffold a new project based on that"); and ruled twice
  on the answer — "name it openRepoShape and make it public", then "apache 2.0,
  create it". The topic itself was commissioned by his ruling "stage the
  project-schema topic too". Origin provenance is the ratified change
  `add-wallet-carried-review-authority` (`proposal.md:600-617`), carrying his
  2026-08-22 ruling that codexFactory may "recommend and run a 3 repo project
  schema. But let the human project manager decide."
- Files: `project-repo-schema.md` (the primary fragment; conforms to the
  Staged-Topic Outline Template — pre-document idea notes, conflicts, and twelve
  open questions each carrying Context / Recommended answer / Explanation /
  Disposition status).
- **The claim that makes it worth raising: the recommendation exists and is
  unactionable.** `add-wallet-carried-review-authority` recommends the
  SPEC/CODE/ASSEMBLY shape, rules who elects it (`PA`, `CA`-constrained,
  `PM`-sequenced), and rules that electing it "changes no gate, no floor, no
  grant, and no clearance eligibility" — and then supplies no name form, no
  schema field, no validator and no scaffold. Verified 2026-09-02: no
  `project-repo-schema` capability exists under `openspec/specs/`; that change's
  own `specs/` deltas are exactly `review-authority-intake` and
  `roles-authority-model`; and greps for the schema's own vocabulary resolve only
  to that one proposal plus `tasks.md:811-814`. So the delta is ADDED rather than
  MODIFIED, and the topic's whole job is to make an existing, ratified, elective
  recommendation possible to act on without making it worth anything.
- **Q2 is the blocking ruling.** The ratified prose defines ASSEMBLY as "the code
  that assembles a review team, shared across the family" (`proposal.md:608`).
  Read as ONE shared repository, an electing project has SPEC and CODE and no
  root — and therefore no answer to the question Brett actually asked. Read as a
  PER-PROJECT root, the sentence describes the content such a root pins. Both
  readings fit the rest of the document and the corpus contains no third mention.
  Recommended: the per-project root IS the assembly leg and PINS the shared
  review-team code at a sha, exactly as the aggregation already consumes
  `review-lane-reusable.yml@dc21767…`. The exit change may not be proposed until
  it is ruled.
- **The lockstep invariant is carried as a cost, not hidden.** The aggregation's
  ...
  written down nowhere; seven consecutive pin-syncs from 2026-08-25 moved the
  gitlink alone and left `validate` red on every aggregation PR until 2026-08-26
  (xFactory #146), unnoticed because `validate` runs on pull requests only. Q7
  proposes shipping the assertion as a validator in the assembly template, so the
  cost of the submodule answer is paid once at scaffold time.
- Sibling: [`wallet-carried-work-authority`](wallet-carried-work-authority/wallet-carried-work-authority.md)
  was staged the SAME DAY and landed on `main` first (PR #585). It carries path-scoped grant
  objects and the `author`/`merge` acts, removing the hidden coupling under which
  repo-granular grants made the three-repo split load-bearing for authority
  legibility. This topic is the ERGONOMICS half and defers every authority
  question to it by name; the bootstrap's grant readout depends on its `author`
  act and degrades rather than fails until it lands.
- Exit: TWO acts, and the first is HALF DONE. FIRST, openRepoShape is created and
  cut — the public Apache-2.0 repository EXISTS as of 2026-09-02; what remains is
  its content and a first release with per-file digests, landing by PR under the
  inherited org ruleset. SECOND, one openxFactory change (working name
  `add-project-repo-schema`) that ratifies the doctrine doc and the ASSEMBLY
  clarification, adds the openRepoShape pin under `neutral-product-pin`, and adds
  the register election fields plus the manifest-derivation rule. Realizations
  follow in codexFactory (engineering overlay) and OpsxFactory (org naming,
  repository topics, any regenerated template repository). Gate: Q2 ruled before
  the change is proposed, and a digested openRepoShape release before it can pin.
