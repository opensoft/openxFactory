...
| [client-layer-tuning](#client-layer-tuning) | MODIFIED client scaffold (`roles/` + FAO + `cost_reporting_steward`); ADDED client content schemas + `validate-client-content`; wizard verb in hermes-install | 1 | **All three exits IMPLEMENTED + LIVE-PROVEN 2026-07-24** — 2a `add-client-layer-tuning-contracts` (contract-v1.17), 2b `add-client-layer-defaults` (codexFactory), 2c `add-client-tuning-and-seeding` (hermes-install: wizard + unified client seeding). The opensoft tenant is tuned (ratified envelope, empty park-map) and SEEDED LIVE (phase-2 evidence note, order invariant passed). Pending: ratification of all three; topic completes on archive |
