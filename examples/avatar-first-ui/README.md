...
- [fixtures/](fixtures/README.md) — deterministic UI fixtures checked offline by
  ...
  each failing exactly one stable `AFUV-*` error), and five deterministic
  fixtures — the offline-acceptance `listening` baseline, a
  `listening → thinking → speaking` extension, and (avatar-client-lab task 4.2)
  the governed-denial, control-lost-failure, and consent-withdraw-mid-speech
  worked-scenario seeds — plus the requirement→evidence map.
